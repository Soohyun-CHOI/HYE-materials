...
**Invoices**: Invoice ID (HYE-INV-YYMMDD-##, top-level), Vendor Invoice Code (human-entered), Vendor (link), Issue/Due Date, Amount Due (labeled "Vendor's Stated Total" — compared against but never overwritten by the calculated total), Shipping Fee, Tariff (optional, toggle-revealed), Paid(+Date), File (attachment, required).
