...
- **Every string a screen can render** → `docs/briefs/strings/`, one file per screen beside its brief (#288). Re-made from the code by `scripts/screen-strings.mjs` rather than maintained by hand; `--check` reports where a file and the screen disagree.
