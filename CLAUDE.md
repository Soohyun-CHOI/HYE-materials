...
**Known, lower-probability residual risk**: `withKeyLock` only serializes calls within a single process/function invocation. It does **not** coordinate across separate serverless invocations — two genuinely simultaneous API requests (e.g. two different Vercel function instances) touching the same PR/PO/material key could still race. This is judged low-probability for this app's actual usage pattern: the signing-chain design (`Current Signer Step`) already serializes PR editing to one signer at a time, so concurrent multi-item writes to the *same* PR/PO from *different* requests shouldn't normally happen. Revisit if this ever becomes a real (not theoretical) problem — e.g. a distributed lock, or optimistic-conflict detection with reconciliation on write.
