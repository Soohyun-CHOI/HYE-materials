...
- Reference usage: `app/admin/jobs/new`, `app/admin/vendors/new` — minimal Admin-only forms wired straight to the existing `createJob`/`createVendor` (`lib/airtable/jobs.js`/`vendors.js`), no new business logic. Each page pairs with an `actions.js` Server Action for the actual mutation; **the Server Action calls `requireAdmin()` again independently of the page**, since Server Actions are callable directly (e.g. via devtools) regardless of what the page rendered — a page-level check alone only gates the UI, not the mutation.
