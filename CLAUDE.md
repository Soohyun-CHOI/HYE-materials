...
**Phase 3** (Invoice handling) — in progress. Done: #14, #46, #51, #48, #57, #84, #17 (decision), #15, #16, #91, #92, #93 (decision), #96, #99.
...
**Items locking + reset-warning consistency** (#99, InvoiceForm.js) — Items stays visible but faded/disabled (`itemsReady`: at least one PO chosen, and every currently-selected PO's items have finished loading) rather than hidden, so the invoice's overall shape is visible immediately; this also structurally prevents both the transient zero-option dropdown during loading and Item Name being freely editable before any PO exists. Separately, `autoInsertedItemsRef` snapshots `items` every time it's set *automatically* (initial default, PO-forced defaulting, post-load auto-fill, detection auto-fill) but never on a user-driven change (add/remove/edit a line, pick a PO Item) — comparing current `items` against this snapshot is the single "has the user actually edited anything" signal, replacing the old itemName/qty/unitPrice truthiness check that couldn't tell an auto-filled value from a typed one. Both the direct PO-change path and the Vendor-change path (which previously reset PO/items silently, no warning at all) now share this same check via `confirmIfDirty`.
