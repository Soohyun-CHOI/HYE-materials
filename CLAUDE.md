...
- `lib/toolRegistration.js` — registering tool items (#338): the key two typed names share to be one tool, the per-submission ceiling, the actor's-own-jobs rule, and every word the screen says. Applied by the action, previewed by the form. **A tool item's minted ID is printed, so the submission names every one it created and never a count.**
- `lib/toolItemView.js` — what one tool item's page shows (#340): which of a `Tool Log` row's five facts appear, and every word the screen says. **Four are on every row and `Notes` is the one that drops.** Nothing here compares `Status`/`Job` against the log — no app path can make a cache and its source differ.
