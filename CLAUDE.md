...
**Not wrapped, and why (#147).** These are the exemptions, listed with reasons in `scripts/tests/verify-authz-structure.mjs` and enforced there. The list is meant to stay short for two reasons: every entry is a precedent a future author can copy, and an exemption is verified more weakly than a wrapper (see the scope note below).
...
**It is a standing check, not issue evidence.** That is why it lives in a file with no issue number, runs with no env vars, no Airtable, no dev server and no module loader, writes nothing, and exits non-zero when it fails: `node scripts/tests/verify-authz-structure.mjs`. Run it after adding or moving an endpoint. `scripts/tests/verify-authz.mjs` wraps it as Part A and adds the runtime evidence (Parts B–E: the Blob host predicate, PO-generation idempotency against a throwaway PR+PO, the wrappers' control flow, and real HTTP 401/403 refusals); that one needs `.env.local`, the loader, and a dev server for Part E, and it exits 2 if a part could not run so an incomplete run never reads as a pass. Adding an endpoint without a gate fails the structure check without anyone remembering to register it, which is the property the old check didn't have.

---

## Utility scripts (scripts/)

- scripts/tests/ — temporary/verification. The fixtures such a script creates are deleted from Airtable after use, within the run that created them. That is a statement about a script's own fixtures, not a standing licence to clean the base — see the dummy-data note below.
