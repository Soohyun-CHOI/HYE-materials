...
**Metadata API limits, measured.** A `multipleRecordLinks` field CAN be created, and a table CAN be created; a table CANNOT be deleted (404, no endpoint). An existing single select's option list cannot be PATCHed at all (422). `prefersSingleRecordLink` is refused on both CREATE and UPDATE (422), so single-record links are enforced by this app and checked on the DATA rather than on a schema property. Creating a link field auto-creates the symmetric field on the far table, named after the source table, and that name can be PATCHed afterwards.
...
- `lib/poUnsigned.js` — `isPOUnsigned` and the signal wherever an order is offered for an invoice (#198): the picker's option label and `UNSIGNED_COPY`. The judgment runs server-side; a client reads only the `unsigned` boolean.
...
- **Resend's domain is verified, so mail delivers to any address.** It was sandbox-only for most of this project's life, and several decisions elsewhere were made under that constraint; the constraint is gone.
