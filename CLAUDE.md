...
The rule is a convention, and it holds whatever does or doesn't enforce it — but note what the verification actually proves. `scripts/tests/verify-authz-134.mjs` Part A is a substring match: it asserts that `requireAdminApi(` appears in a route's source before that route's first real-work needle, over a source with block comments and whole-line `//` comments removed. So the helper being *mentioned* satisfies it — a trailing comment on a code line survives the stripping — and so does a genuine call whose refusal Response is then discarded, which is the caller-obligation failure described above. Passing Part A is evidence the name is present and ordered first, not evidence that the route re-authorizes.
