...
**Material Categories**: HQ's four-level tree, one row per path (#354). `Category Label` (primary, formula; the rule is `lib/materialCategory.js`), `Level 1–4 Code` / `Level 1–4 Category` (all singleLineText; **a code is text and never a number**), `Item Name` (singleLineText — the readable name, **stored and never composed**: eight templates produce it, so a hand-added path gets a label for free and no name, #415), `Materials` (reverse-link, #356). Reference data, loaded re-runnably from a committed CSV, keyed on `Level 4 Code`.
