...
- Verified end-to-end against the real Airtable base and a real browser session, including the full pause/resume path (signer 1 returns to the Requester → Requester edits a Qty → resumes to signer 1, whose status resets from `Returned` to `Pending` → signers 1, 2, 3 approve in order → `PR.Status: "Approved"`) and a forced-failure rollback test on Approve. Not yet covered by an automated test: the "not your turn" guard was exercised structurally (every step's UI only ever showed action buttons to the correct current signer) rather than via a deliberate bypass attempt.
