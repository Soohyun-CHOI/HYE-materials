...
- **Columns: `PO ID · Vendor · Job / Discipline · Total · Status`.** No requester column: a PO carries no requester of its own (#138) and the PR list already answers that question, so adding it would spend a column and a query on a fact belonging one document upstream. Job and Discipline both come from the parent PR, since a PO has neither field — `Job` through the PR's lookup and `Discipline` through its link, rendered as one cell separated by a middot, which is /prs's own shape for the pair. **The header read `Job / Line` until #280**; the widths below are the widest CELL's and are unaffected, but the heading itself grew six characters — checked in a browser on that branch and still one line.
...
- **ROWS CARRY PR ID, Job / Discipline, Vendor — and nothing the table below
