...
- **Columns: `PO ID · Vendor · Job · Total · Status`.** No requester column: a PO carries no requester of its own (#138) and the PR list already answers that question, so adding it would spend a column and a query on a fact belonging one document upstream. `Job` comes from the parent PR, since a PO has no such field — through the PR's own lookup, which reaches it via the discipline. **The header read `Job / Discipline` until #314 and `Job / Line` until #280**, one cell from two fields separated by a middot, which was /prs's shape for the pair. #314 took the discipline off every list: a discipline is how a request is FILED rather than where the material went, so it sits on the documents that hold one, and #314 added it to `/pos/[poId]` because this column was the only place an order's discipline appeared anywhere in the app. **The removal is measurable and was measured** — `getAllDisciplines()` was the page's seventh operation and the page is six now, verified in the browser at 9 total against 10.
- **SORTED BY `PO ID` DESCENDING, SERVER-SIDE, AND THERE IS NO DATE COLUMN — the same shape as `/invoices`**, which sorts by `Invoice ID` and shows only dates that are other facts. A PO ID is `HYE-PO-YYMMDD-##`, fixed width and zero-padded, so a plain string sort gives chronological order and the within-day sequence; `InvoiceForm.js` already states that property of this exact format. A `Created` column was there first, on the argument that a list should show the field it sorts by — **`/prs` is the counter-example that retired it**: it sorts by `Created At` and has no date column at all, and `/deliveries` hides its `Created At` tie-break too. Showing the sort key is not this repo's rule. **Consequence on the demo base:** hand-made fixture IDs like `HYE-PO-TESTQA-01` have no date segment, so `T` sorts above any digit and they head the list. Unreachable in production — `mintDailyId` (#164) can only mint the dated format — and the ID prefix is the one thing nobody can backdate, which is why it is the safer key than `Created Date` (the demo seeds backdate that field after generation).
...
- **SIX OPERATIONS, NONE PER ROW — but stepped, not constant.** `requireUser` (1 find) + `getAllPOs` + `getAllVendors` + `getAllJobs` + `getPRsByRecordIds` + `getApprovedPRs` (#176's, the sixth). **`getAllDisciplines` was the seventh until #314**, and it bought one thing: the Discipline NAME for a column and for the two strips that copied its shape. Nothing on the screen reads a discipline now, so the level is not fetched — the read went with the render rather than outliving it, which is what `offline/job-column.mjs` holds. Four of those are selects, which Airtable pages at 100 rows, and the batched PR read chunks at 50 ids — so the count rises one query per 50 or 100 records and never with the number of rows rendered. `getLinkedRecords` is used nowhere here: it re-finds the parent on every call, which is why `/prs/[prId]` reads one PR five times. `#190`'s counter measures selects per PAGE for exactly this reason.
- **Table widths are declared (#166's rule) but measured for the columns of the day.** The invoice table's 52rem came from its own seven and does not transfer. Measured at 14px/20px Arial plus the 8px `pr-2` when the table had five: Job / Discipline 184, PO ID 149, Vendor 124, Status 117, Total 79 — 653px of the 832px available. Three columns are bounded by construction and take what they need; **the remaining 27.5rem was split between the two nobody controlled**, Vendor and the Job / Discipline cell, both human-entered — 192px and 248px, the larger share going to the cell that carried two values and a separator. **PO ID was first sized by counting characters against the invoice list's ID and 38 of 40 rows wrapped**, because a PO ID carried a four-digit year then (the one exception to the 2-digit convention) and rendered at 141px; **Vendor takes all the slack**, being the only column whose content nobody here controls. **#313 TOOK TWO CHARACTERS OFF THAT STRING AND THE COLUMN WAS NOT RE-CUT** — the 141px is what an 18-character id measured, the id is 16 characters now, and re-measuring to reclaim the difference is the design pass's re-cut rather than a migration's. The figure is left as measured and labeled as of its day, which is the same treatment the census dates in `id-generation.md` get.
  - **THAT PAIR IS DOWN TO ONE SINCE #314.** The column heads `Job` and carries a job code, which is a house format of about ten characters and bounded by construction like the PO ID beside it — so it narrowed 12.75rem to 5.75rem, the width `/deliveries` and `/invoices` also declare for the same value, and Vendor is now the only column here whose content nobody controls. **Nothing was redistributed**, which is #235's and #311's call applied in the other direction: a column carrying one value takes one value's width, and handing 7rem to a neighbor is the pixel judgment those two declined to make twice. The row is 58.25rem against the page's 52 and still scrolls inside its own container — measured in a browser at 932px, `Job` one line at 92px.
...
### The payment column (#311)

`/pos` carried an order's status, vendor, job and total and nothing about what had been charged against it or paid. The order's own page already folded the invoices charging it and marked each one, so the fact existed a level down; this lifts it.

- **`paid` IS NOT A FACT AN ORDER HOLDS, WHICH IS THE WHOLE ISSUE AND NOT A CAVEAT ON IT.** An order is charged by several invoices and an invoice charges several orders — both are real on this base: `HYE-INV-260819-10` charges two orders, and `HYE-PO-260819-19` and `-22` are each charged by two invoices. So the column states something about the DOCUMENTS: every invoice charging this order is paid, or some are, or none is. What it may not state is that the order has been paid for.
- **#233's RULE BINDS HERE, AND WHAT IT FORBIDS IS A FIGURE RATHER THAN THE MARK.** `lib/poDocuments.js`'s header says no entry may hold a document's own total, because a money figure beside a purchase order invites addition against the order's total and one invoice can charge two orders. The mechanism is the ADDITION, and `Paid` is a per-document boolean with no partial-payment field behind it, so a paid invoice is paid in full and the aggregate is exact. What the rule does reach: **this axis carries no figure at all** — no amount, no count of invoices, no day count on the badge. The first is #233's outright; the second and third are its reasoning one step on, since a count belongs to a set the row does not show and a day count belongs to one invoice while the badge is about a set.
- **THE EMPTY SET IS THE TRAP AND IT SAYS NOTHING.** "Every invoice is paid" and "none is paid" are both vacuously true of no invoices, so an order nothing has charged would read as whichever the arithmetic reached first. It gets the dash. A withdrawn order lands there without a clause of its own — it cannot be invoiced, so nothing charges it.
- **A PARTLY-INVOICED ORDER CAN READ `Paid`, AND THE COLUMN BESIDE IT IS THE ONLY THING THAT MAKES THAT HONEST.** 40% billed and that invoice settled is `Partly invoiced` `Paid`, both true. Read alone the payment cell says the order is finished with. #235 put the two chips adjacent for its own reasons and this is what that adjacency is now load-bearing for; `docs/briefs/pos.md` records it as a thing a redesign may not separate.
- **LATENESS IS A SECOND AXIS, NOT A FIFTH VALUE, AND ONE CASE DECIDES IT.** An order charged by one paid invoice and one late one is `Partly paid` AND overdue; a closed set has to pick one and either pick throws away something the reader came for. So `⚠ Overdue` is a badge composing with the two unpaid chips — #166's own rule, that a second fact composes with a chip instead of enlarging its set. It reads as a word rather than as a `!`: #232 retired `QualifierMarker` from the invoice list because a reader met the normal word first and had to hover for the one that mattered, and that objection is to a tooltip, not to composition. A paid set cannot be late, which is what keeps the badge composable with two chips and not three.
- **`dueDate < today`, SO THE DUE DAY ITSELF IS NOT YET LATE** — `lib/authTokenState.js`'s `expiresAt < now` one field across. `today` is a PARAMETER, which is `daysWaiting`'s shape and its reason: the offline tier pins every boundary without a clock. Both pages take the server's day once, so no two rows are judged against different days.
- **`Due Date` HAD NO JUDGMENT READING IT BEFORE THIS, AND THAT IS WORTH KNOWING BEFORE TOUCHING THE FIELD.** Four places read it and all four print it: the invoice list's column, the invoice detail's line, and the two forms that write it. Nothing compared it to anything — not to today, not as a sort key. **#311 is its first reader as a fact.** It is also OPTIONAL on both write paths — neither form marks the input `required` and neither action refuses a blank, unlike `Issue Date`, which both do — so an invoice this axis cannot judge for lateness is an ordinary record. All 23 rows carry one today because the seed filled them; that is not a guarantee. A blank is not overdue, which is #263's call on a null wait carried across.
- **ONE JUDGMENT, TWO SCREENS, AND THE MUTANT IS QUIET BECAUSE BOTH HALVES KEEP WORKING.** If the list computed its cell one way and the order page marked its invoices another, a reader could meet `Paid` on a row, open it, and find an unpaid invoice underneath — two answers, each right on its own screen, nothing failing. So `summarizePOPaymentStatus` is the only rule and `/pos/[poId]` renders the same chip beside its `Invoices` heading, over the invoices it already read. `offline/po-payment-column.mjs` holds four properties: both screens call the two shared functions, neither names `dueDate`, both render both named slots, and the invoice level is gathered from the gated rows. **Mutation, measured:** writing an own rule into the list fails three checks — the two shared-function assertions plus `PAID_READERS`, because an own rule has to name `.paid` and that forces registration; widening the gather to `pos` fails one.
- **AN AIRTABLE ROLLUP WOULD HAVE MADE IT FREE AND WAS REJECTED FOR A REASON THAT IS NOT COST.** A `Invoice Items."Invoice Paid"` lookup plus a `PO Items` rollup gets the answer at zero operations, and ALL/ANY over invoice items equals ALL/ANY over the distinct invoices behind them because the value is a per-document boolean. What it cannot do is be shared: the order's own page must read the invoice records anyway, since it lists them with a badge each, so a list judging from a rollup would be the two screens computing one fact two ways — the divergence above, built in by construction rather than left as a hazard.
- **COST: `/pos` GOES 8 TO 10, AND NEITHER IS PER ROW.** Measured on this base (36 orders, 38 ordered items, 25 invoice items, 22 invoices): `Invoice Items` list 1 and `Invoices` list 1, each one query per 50 ids, so the page steps with the number of INVOICE ITEMS and INVOICES rather than with the rows it draws — the same step `PO Items` already had. Both return early on an empty id list, so a set of orders nothing has charged pays for neither. **`/pos/[poId]` measured 14 before and 14 after**: it already read both levels in exactly this shape (#235), which is also what lets the two screens fold one judgment. `getPOItemsByRecordIds` gained the `Invoice Items` link array, free — `findByRecordIds` passes no `fields`.
- **THE EIGHTH COLUMN DOES NOT RE-CUT THE BUDGET EITHER, WHICH IS #235's CALL APPLIED A SECOND TIME.** The declared widths now sum to 65.25rem (1044px) against the 832px the page has; the table is inside an `overflow-x-auto`, so what widens is the scroll in that container and nothing is truncated. Same 6.625rem as its two siblings, and the badge STACKS under the chip so the cell needs the wider of the two rather than their sum — measured at 1280px, chip 57px and badge 68px inside 106px, the stacked pair 38px against a 30px single-line row.
- **TWO FALSE LINES IN `lib/airtable/poItems.js` CORRECTED IN THE SAME COMMIT (#181).** `recordToPOItem` justified keeping the `Invoice Items` array out by "the same standard that keeps `Invoiced Qty` out of this mapper" — a standard #235 retired when it put `Invoiced Qty` in, so the appeal had nothing left to appeal to. And `getPOItemsForReconciliation` said `Paid` is President-or-Admin, which #309 reversed.
...
- **IT DOES NOT MAKE THE PAGE SHORTER, and expecting that would be the wrong reason.** Three ordered items invoiced by two invoices is six pair-facts either way. What drops is the repetition of the header facts, six to two. The pair facts moved rather than disappeared — they are inside the invoice's entry now instead of under the ordered item's row — which is also the answer to "where did the per-ordered-item figures go": nowhere, they changed parent.
  - **The alternative that was rejected was dropping them.** `⚠ Variance` on an item is `Invoice Items."Variance Flag"`, which the backend sets against THIS ordered item's agreed price and quantity, so it is a fact about the order being invoiced wrongly and belongs on the order's page. Dropping it would have meant opening each invoice in turn to find which item was flagged.
...
- **ONE OF THE TWO VARIANCE WORDS CHANGED, AND THE OTHER WAS PUT BACK AFTER BEING CHANGED.** `Invoice Items."Variance Flag"` — one item against what the order agreed — read `⚠ Line Variance` and reads the bare `⚠ Variance` the invoice detail's items table already uses for that same field. That is not #179 arriving early: it is a word this repo bars being kept out of new screen text — for `Lines` at the time, and since #280 for #303's rule instead, and `offline/line-vocabulary.mjs` cannot see text written straight into JSX, which is how it got there. The copy is a `*_COPY` constant now, which is what brings it under that check.
...
- **AN ORDER FROM A REQUEST WITH NO ADDRESS FREEZES NOTHING AND THE ORDER'S PAGE
  ...
  - **THE ISSUE'S OWN COMMENT SAYS SIX REQUESTS AND THE NUMBER WAS TWO.** Six are
    ...
    spent `-04`**: `HYE-PO-260915-02` came from it, froze nothing, and is what
...
- **THE ORDER DOCUMENT IS A PARTIAL SNAPSHOT, AND THIS BRANCH CLOSED THE PATH THAT WOULD HAVE SHOWN IT RATHER THAN COMPLETING THE SNAPSHOT.** `generateAndAttachPOPdf` reads two kinds of thing. **Frozen:** the order and its `PO Items` — the item snapshot PO generation took, and `Total Amount` over it. **Live, re-read on every generation:** the vendor, the job, both addresses (the order's delivery address and the vendor's — this said three, with an alternate, until #384 removed the alternate), `Our PIC`, `Our Manager`, the President from `getPresidentUser()`, and every quotation appended from the request (one, the first, until #40). So a second generation does not reproduce the document — it produces a current one, and some of the differences **misstate history**: a changed President would put another name in the signature block of an order somebody else signed.
...
- **TWO THINGS DELIBERATELY NOT BUILT HERE, recorded so neither reads as an oversight.** A **strip above `/pos` for signed orders nobody has sent** was real work and was not here: that state now exists and is worth surfacing, in the shape #176 set for approved requests with no order. Counting it needs `Purchase Orders` filtered to `President Signed` with an empty `Sent At` — a `filterByFormula` on the same table the list already reads, so it costs one query rather than a walk, and the wait ordering is `sortLongestWaitingFirst`'s with `Sent At` absent and `President Signed At` as the key. **#295 built it, and it cost no query at all** — the list already reads every order, so the strip selects from rows the page holds rather than asking the base a second question; see its own section below. And a **manual control for marking an order sent outside the app** is not built and should not be: once the app can send, the reason to send outside it shrinks, and a manual mark would put rows in `Sent By` and `Sent To` that are somebody's claim rather than the app's record — two kinds of fact in one field, which is the defect #233 spent a whole issue separating on the invoice list.
...
- **WHICH OF THE FIVE SENDS PUT MONEY THROUGH `formatUSD`, measured after this change**, since #233's rule was written about screens and the mail path was never audited against it. `sendMagicLinkEmail` and `sendSignerTurnEmail` carry no figure. `sendPOToVendorEmail` is formatted, at its call site in `sendPOToVendorAction` (#281). `sendPOSignedEmail` is formatted as of this issue — it printed `po.totalAmount` raw until now, so `HYE-PO-260819-27`, whose `Total Amount` is `220.00000000000003` on this base, would have gone out carrying every digit. **`sendPOAwaitingSignatureEmail` was still raw** at the end of this issue, from `notifyPOAwaitingSignature`, and it is the President who reads it — left for its own issue rather than fixed here, since this one rewrites a single mail's body and that one's wording belongs to whoever changes it. #292 is that issue, and it moved the formatting one layer down for all three: see its own section below.
...
### Listing the signed orders nobody has sent (#295)

#281 put the send in the app and gave it to the requester as well as the office; #290 tells the requester once, by mail, at the moment of signature. After that the only trace was a control on a page nobody has a reason to open, so an order whose owner read that mail on leave never reached the vendor and no screen said so. #281 recorded this strip as the work it was not doing; this is it, and the second strip on `/pos`.

- **IT COUNTS `Sent At` AND NEVER THE STATUS BESIDE IT, which is #281's rule applied rather than a new one.** The two are written in one operation, so they agree everywhere on this base — 30 rows either way, which is exactly what makes reading the wrong one invisible. They come apart in two places: a hand edit, and a send whose record write failed. **The second is the dangerous one**: the vendor HAS that order, and a status test would list it and tell somebody to send it again — the one thing a strip about sending must never say. #281's own words settle which field wins: "the timestamp is the fact while the status is the stage".
- **`"Withdrawn"` IS SPELLED IN THE SELECTOR, AND THAT IS THE COST OF THE TIER BOUNDARY.** `PO_WITHDRAWN_STATUS` lives in `lib/airtable/purchaseOrders.js`, which throws at module load without credentials, so no offline-safe module can import it — `lib/poSend.js:getPOSendEligibility` spells the same string for the same reason, and this joins the sweep #281 recorded rather than starting one. **Reading the status rather than `Withdrawn At` is deliberate:** `isPOWithdrawn` is the app's one implementation of that question and it reads the status, so keying on the timestamp would be a second answer to it. **The exclusion has a witness on the base**: `HYE-PO-260819-26` is withdrawn AND signed with no `Sent At`, so an unconditional list would carry it forever — a withdrawn order's `Sent At` is empty and will never be filled.
- **NO THRESHOLD, AND #263 SUPPLIES THE AXIS RATHER THAN THIS ISSUE ARGUING ONE.** That issue put a threshold on the invoice strip because "the vendor emails the invoice at shipment, so an invoice with no delivery matched to it is what an ordinary Tuesday looks like", and it recorded the other half of its own rule in the same breath: "material standing uninvoiced reads as waiting from the first day, so `AWAITING_INVOICE_COPY` carries no threshold and must not grow one". **What is waited for here is one click by a colleague who was emailed at the signature, not a vendor's shipment**, so this axis is the second kind. The only threshold that would fit it is "not today's", which buys nothing measurable and hides exactly the case #295 names — the requester who read the mail and was pulled away — for as long as it takes to matter. **If the office ever reports same-day orders as noise, the shape to copy is #263's**: a constant in `lib/poListView.js`, a sentence in the copy that names it, and a check pinning the two so a tuned number cannot leave the strip claiming the old one.
  - **THE 30 ROWS THIS SHOWS TODAY ARE A SEED ARTIFACT AND NOT EVIDENCE FOR A THRESHOLD.** 29 of them were signed before #281 existed, when no send path could have emptied them; the thirtieth is `HYE-PO-260819-27`, signed during #290's verification, and it is the only honest instance on the base. **Measured both ways rather than argued**: with 26 of the pre-#281 rows temporarily marked sent the strip read 4 rows at 5d, 2d, 2d and 1d, which is what the steady state looks like. A reader who meets the 30-row screen and concludes the strip needs a threshold is reading the seed.
- **ONE VOICE AND NO CONTROL, AND BOTH FOLLOW FROM ONE FACT.** #281 put the vendor's address above the send button and made that the reason there is no confirmation dialog — "the reader has already seen where it goes" — so a button in a strip row, with no address beside it, would owe that dialog back. The row links to the order instead, where the address, the control and the document all are. With nothing offered there is nothing for a second voice to split over, which is #263's own reasoning for the invoice strip carrying one line where #176's carries two. **The sentence names no control either**, because who may send is per record rather than per role (#281) and a missing document changes which control the order's page even offers (#290) — so it says why the state matters instead: sending IS placing the order.
- **EVERY READER WHO CAN SEE AN ORDER SEES IT IN THE STRIP, which is what makes this fix reach the requester at all.** The rows come from the same `visible` array the table is built from, so the strip inherits `canViewPR` rather than applying a second rule; the office sees all of them and a requester back from leave sees their own. **`isMine` was available for free and is not used** — the table already computes it for its own filter — because a mark saying "yours" would imply the unmarked rows are not this reader's business, and for the office they are all its business.
- **NOT ONE NEW READ, AND `Sent By` IS WHY THE OBVIOUS NEXT COLUMN WOULD BREAK THAT.** `getAllPOs` already returns every order and `recordToPO` already carries `Sent At` (#281), `President Signed`, `President Signed At` and the links; Job, Line and Vendor come out of the three maps the table built. Measured on the render: **5 awaited calls and the same seven readers before and after**. The send writes `Status`, `Sent At`, `Sent By` and `Sent To` in one `updatePO`, so a row with no `Sent At` has no sender either — **0 of the 30 rows carry one** — and naming the person who has not acted would cost a Users read per row to name nobody.
- **THE ORDERING IS THE SHARED ONE, and this is its fourth caller.** `sortLongestWaitingFirst` takes `waitingSince`/`createdKey` rows, so the page maps first and sorts second — `app/invoices/page.js`'s shape. `waitingSince` holds the INSTANT of `President Signed At` while the row RENDERS its calendar day, which is #256's both-names rule and buys something real: two orders signed seven seconds apart on one day order exactly instead of falling to the id tie-break.
- **Not in this issue:** a send control anywhere but the order's own page, a threshold, a cap on the row count, `isMine` as a mark, and any change to #176's strip or to the table.

### Formatting every money figure the app emails (#292)

#290 found the order-signed mail printing `220.00000000000003` and routed that one figure through `formatUSD`. Two issues later the same shape was still on the mail asking the President to sign. #233 settled that money reaches a reader through `formatUSD`; the mail path had simply never been audited against it.

- **THE FIVE SENDERS, COUNTED, so nobody counts again.** `sendMagicLinkEmail` carries a link and a TTL in minutes. `sendSignerTurnEmail` carries a request id and a link — its `context` is free text and its one caller passes a human's correction note, so nothing the app computes reaches it. The other three carry the order's total: to the President who decides whether to sign, to the vendor who checks the right document arrived, and to the requester who has to recognize the order they must place. That count is now `offline/mail-money.mjs`'s table rather than a sentence here, with the reason per entry.
- **THE FIX IS ONE LAYER DOWN FROM #290's, AND THAT IS THE WHOLE POINT.** The money arrived at every builder already formatted, so the step that can be forgotten sat at the call site — which is exactly where #290 found it forgotten. The builders take the NUMBER and render it, so a caller has nothing to remember and passing `po.totalAmount` is the correct thing to pass. **This reverses a paragraph #290 wrote** (`SIGNED_NOTICE_COPY`'s "the total is formatted by the caller"), and reverses it for the reason that issue existed.
- **THE THIRD BODY MOVED OUT OF `lib/email.js`, WORD FOR WORD, AND THE CRITERION IS NOT TIDINESS.** That module throws at load without `RESEND_API_KEY`, so the offline tier can never import it: a body assembled there can be read as source and never CALLED, so no check can ask what figure it prints. A mail carrying a figure therefore has to be a pure builder; a mail carrying none has nothing for a check to compute, so the two that carry none keep their wording where their own issues put it. **The side effect is that `offline/line-vocabulary.mjs` can now read the President's mail** — it walks `*_COPY` constants — and the words were moved unchanged, so that check gained a body without gaining a decision.
- **IT WENT TO `lib/poUnsigned.js` RATHER THAN BESIDE THE OTHER TWO.** `lib/poSend.js` holds them and was the obvious home; #181's file-name test refused it — does the name make a claim the contents contradict — because that module's subject is sending the order to the vendor and this mail is not about that act. `poUnsigned.js` owns the signature axis since #281: the status a signature has not reached, the two it has, and now the message that asks for it.
- **THE ALTERNATIVE THAT WOULD HAVE CLOSED BOTH MUTANTS STRUCTURALLY WAS WEIGHED AND REJECTED.** A builder taking the mapped order record (`html({ po, … })`) and reading `po.totalAmount` itself leaves no money parameter to get wrong: neither a sibling figure nor a pre-formatted string could be passed, because nothing is passed. What it costs is that the copy would then depend on the mapper's field names, and a rename there would render `$0.00` in silence — trading a call-site mistake for a rename-time one, in a module whose whole value is being pure and shape-free. `getPOSendEligibility` does take a record, so the shape is not foreign; the check carries this weight instead, and that is why its call-site assertion is by SHAPE (`po.totalAmount` as a member expression) rather than by presence.
- **WHAT IS STRUCTURAL AND WHAT IS ENUMERATED.** Structural: money reaches a mail only as a number handed to a builder, so no sender takes a money parameter. Enumerated: which senders carry money at all — the check's table, which fails on a sender it has never heard of. The first makes the fix permanent for the three that exist; only the second notices a fourth. **The residual is named in the check's header:** the no-money test is a list of money-ish parameter names, so a mail carrying a figure in a parameter called `figure` passes that clause. It still fails the inventory until a human classifies it, and the classification is where the honesty has to live.
- **WHO PROVES WHAT, since neither half could prove the other.** The float is unreachable on this base — the two approved requests with no order both hold whole-number prices — so `220.00000000000003 → $220.00` is proved by calling each of the three builders offline. The real send proves the currency symbol and the cents: generating `HYE-PO-260826-01` from `HYE-PR-260819-04` mailed `Total Amount: $260.00` where the same code would have mailed `260` the day before.
- **A TOTAL NOBODY COULD READ STILL PRINTS AS `$0.00`, AND THIS ISSUE DELIBERATELY LEAVES IT.** `formatUSD` does `Number(value) || 0`, so `null`, `undefined` and `NaN` all render as a confident zero — and the same two call sites already print `—` for a vendor name they could not read. **So the mail's one uncheckable fact is its most confident one, in front of the reader with a decision attached**: the President is being asked to sign, and a signature on a `$0.00` order is either right or badly wrong with nothing on the screen to say which. What kept it out of this issue is that the two decisions come apart — this one is about five mails using one formatter, that one is about what absence looks like, and merged they could not be reverted separately. What it would take is a mail-side rendering of absence, not a change to `formatUSD`, whose zero-for-blank is a deliberate cell convention every screen depends on.
  - **NEITHER READING IS UNREACHABLE, WHICH IS WHY THE AMBIGUITY IS REAL.** A zero total is reachable in the app: `app/prs/new/actions.js` refuses a falsy `unitPrice`, which — as #254 established — means a typed `0` is refused rather than that no zero-priced item can exist, and `editAndContinueAction` writes `parseFloat(submitted.unitPrice)` with no such test, so a signer can set a price to `0` mid-chain. An unreadable total is reachable too: `notifyPOAwaitingSignature` re-fetches the order precisely because `Total Amount` is a formula over a rollup that is not populated until the ordered items are attached. **Measured on this base: 0 of 35 orders hold a zero or blank `Total Amount`, every one is a finite number, and the three smallest are 120** — and on the run above the figure was settled by the time the mail was built. So the hazard is not observed today, and nothing about it is prevented.
- **Not in this issue:** `formatUSD` itself, the two mails that carry no money, the wording of any mail (#288's inventory), and any screen's money rendering.
...
- **ROWS CARRY PR ID, Job, Vendor — and nothing the table below
  repeats,** because the table below holds no row for any of them. It uses the same
  values and separator as the list's own columns, so a reader locating work
  reads one shape in both places. **They carried a Discipline until #314**, and both
  strips lost it with the column for that reason: a strip row is a row a reader
  scans, so it follows the column's rule, and a strip disagreeing with the table
  above it would be #314's own finding reproduced on one screen.
...
- **INVOICED BEYOND THE ORDER COUNTS AS INVOICED**, mirroring an over-delivery counting as
  ...
  `Invoiced` and #179's `Order variance` on the item, which is #241's division
...
### What the order list filters by (#324)

Four axes, from a bar `/pos` shares with the other three document lists. The rule that decides which closed sets become filters is `lib/listFilters.js`, and its derivation is in `docs/notes/deliveries-and-invoices.md`; this is what it settled here.

- **THE VENDOR PICKER IS THE AXIS EVERY ONE OF THESE LISTS WAS MISSING.** All four head a `Vendor` column and none could narrow by it, so a reader chasing one vendor had nothing. It is the same control as the job picker — searchable, several at once — rather than a single select, because one control with one parameter shape is the point of the issue and the stated need (one vendor) is served by picking one. A vendor is never blank on a row here: `Purchase Orders."Vendor"` is a lookup through the request, and `createPRAction` refuses a submit without one.
- **THE JOB PICKER'S OPTIONS ARE UNCHANGED AND `/prs` MOVED TO THEM.** The note above under #168 recorded this list's rule as "a deliberate divergence from `/prs`" and said the fix was a UI decision made where the page was new. That is closed: both lists take their options from the visible rows now, and the reason is the one written there — an assignment with no order on it is an option that empties the table, and an order reachable through `canViewPR` clause 5 or 6 was a row no filter could reach.
- **THE TOGGLE'S WORD DID NOT MOVE AND THE BRIEF'S DID.** `Requested by me` is true of both request-side rows; `Raised by me` is false of an order, since an order is generated when a request is fully approved and #138 records that it carries no requester of its own. `docs/briefs/pos.md` had been quoting `Raised by me` since it was written, which made it false about this screen — corrected here rather than filed (#181). `docs/notes/purchase-requests.md` carries the tie-break.
- **THE THREE CHIPS GET NO CONTROLS.** `Awaiting X` -> `Partly X` -> `X` is a wait with degrees on all three axes, so each is a strip's subject rather than a filter's — and two of the three already have their strip, on the screen where the outcome is recorded. The `Status` column is a stage the order is IN rather than a wait, which is why it filters: `Withdrawn` is terminal and is not "behind" `Signed`. The one genuine wait inside that set already got carved out into a strip by #295, which is the pattern rather than a counterexample.
- **`EMPTY_COPY` AND `emptyStateKind` LEFT `lib/poListView.js` FOR `lib/listFilters.js`.** Neither the sentences nor the ordering changed; what changed is that three more lists became callers, and a judgment four screens make cannot live in one list's own view module. This was the only list obeying the three-way rule it stated — `/prs` folded two of the three together and `/deliveries` was saying `yet` to a reader whose scope was simply empty. `offline/po-list-view.mjs` gave its empty-state block to `offline/list-filters.mjs` for the same reason, and keeps everything about the Status text and both strips.
- **NEITHER STRIP IS NARROWED BY THE BAR.** `AwaitingPOStrip` selects approved requests with no order and `AwaitingSendStrip` selects signed orders with no send; each heading carries a count of what is waiting, so a filtered count under an unfiltered sentence would be the heading lying. The table's filters narrow the table.

### What the order list searches (#325)

A box above the bar, on `/pos` as on the other three. The rule that decides what a list searches is `lib/listFilters.js` and its derivation is in `docs/notes/deliveries-and-invoices.md`; this is what it settled here.

- **AN ORDER HAS NO SECOND NAME, AND THE LIVE SCHEMA IS WHERE THAT WAS CHECKED RATHER THAN ASSUMED.** `Purchase Orders` holds `PO ID` and no vendor-side code of any kind — `Sent To` is the email address the PDF went to (#281), which is an address and not a name. So the box takes three names where `/invoices` takes four, and the reason is the strongest one available: **what the vendor receives IS the document this app generates**, with our own id printed on it, so there is no second name for the order to have. A vendor ringing about an order quotes ours.
- **THE PARENT REQUEST'S `PR ID` IS NOT SEARCHED, AND IT IS THE ONE EXCLUSION SPECIFIC TO THIS LIST.** An order is strictly 1:1 with a request and this page already holds `prById`, so it would have cost nothing — which is exactly why it is worth writing down that cost is not the test. A `PR ID` names the REQUEST; searching it here would make one typed string mean two different things on two lists with neither screen saying so, and it is the first step of the chain the rule exists to stop. `/prs` is where a request is found, and the order is one click along.
- **THE THREE CHIPS AND THE `Status` COLUMN ARE UNTOUCHED.** The box matches names, not states; what a reader narrows a state by is the select #324 gave this list.
- **NEITHER STRIP IS NARROWED BY THE BOX**, for the reason neither is narrowed by the bar: each heading carries a count of what is waiting, and a filtered count under an unfiltered sentence is the heading lying.

### The freeze refuses a figure the order cannot carry (#308)

The request half of this issue is in `docs/notes/purchase-requests.md`. What belongs here is the last door: `createPOItem` and `createPO`, and what the retry says when one of them shuts.

- **THE FREEZE IS WHERE OUR OWN CODE MULTIPLIES THESE TWO, WHICH IS WHY IT GUARDS RATHER THAN TRUSTING THE REQUEST.** `createPOItem` writes `Amount: qty * unitPrice` as a STATIC currency value — unlike an invoice item's, which is a live Airtable formula — and that figure rolls into `Purchase Orders."Items Subtotal"` and `Total Amount`, which is the TOTAL line on the PDF the office emails the vendor (#281). A fraction reaching here is not a mark on a record; it is a wrong number on a document that has left the building.
- **IT IS UNREACHABLE THROUGH THIS APP, AND THAT IS THE ARGUMENT FOR IT RATHER THAN AGAINST.** With `createItem` and `updateItem` guarded, every path into `PR Items` is refused upstream. The one path that still arrives is a hand edit in the Airtable UI, which `docs/notes/deliveries-and-invoices.md` records under #254 as the case nothing in this repository catches. A guard whose only trigger is that case is the definition of a last line, not dead code.
- **TWO CALL SITES BECAUSE THERE ARE TWO TABLES AND TWO FIELDS.** `createPOItem` asks both halves of the rule; `createPO` asks only `assertWholeCentPrice`, on the `Shipping Fee` it freezes off the request. One judgment, two doors — and the fee goes through a door of its own because it is written by a different function on a different table, not because it is a different rule.
- **THE FAILURE IS LOUD, RECOVERABLE AND ALREADY HAS A SCREEN.** A throw rolls `generatePOForApprovedPR` back and leaves the request Approved with no order. `approveAction` already swallows a generation failure as non-fatal by design (#10), and `selectPRsAwaitingPO` already selects exactly that state — status Approved, no `Purchase Orders` link — so `/pos`'s #176 strip lists the row and offers the Admin retry. Nothing new was needed to make the state visible.
- **WHAT WAS NEEDED IS THE REASON, AND THE RETRY HAD NONE.** `generatePOHandler` answered every failure with `Something went wrong generating the PO. Please try again.`, which for this one is uninformative AND false: the retry fails identically until the row is repaired, so the sentence invites a loop. The guard's throw carries `PRECISION_BLOCKED` and the handler matches on it, returning `PRECISION_BLOCKED_COPY`. **A code rather than the message**, because the message names a field, a value and two issue numbers, and matching on prose is a rule that breaks when somebody improves a sentence.
  - **THE SENTENCE NAMES AIRTABLE BECAUSE THE APP HAS NO OTHER ANSWER.** `editAndContinueAction` requires `In Review`, so an Approved request's items are past editing here. `/deliveries` already names Airtable in its own out-of-scope sentence, so this is the app's existing grammar rather than a new admission.
  - **IT DOES NOT NAME THE ROW, and that is a size judgment.** The request's own page lists every item with its quantity and unit price, so the odd one is one click away; carrying a row id out through a service-layer throw would put the identifier in the error and the sentence in two pieces.
  - **THE AUTO PATH STAYS SILENT, which is #176's design and not an oversight.** The approving signer's approval succeeded; the strip is the surface for "approved, no order", and the retry is where somebody who can act meets the reason.

### Every quotation on the order document (#40)

The order document appended the request's FIRST quotation in link order, and only when it was a PDF: an image quotation was skipped, a second quotation never went out, and nothing anywhere said either had happened. `lib/poQuotations.js` appends every quotation behind the order's own pages, in the order its `Quotation ID` gives, or makes no document and names the ones it could not append. The send refuses, before the press, a document larger than an email to the vendor can carry — a failure this issue is what made reachable.

- **ALL OR NOTHING, AND #281 IS WHY THERE IS NO THIRD OPTION.** Appending the readable quotations and skipping the rest looks kinder and is the one choice that cannot be undone: `regeneratePDFAction` refuses an order that already has a document, so a document missing a quotation would stay missing and go to the vendor as it was. Refusing leaves the order signed with no document, which is recoverable — replace the file, press again — and which the app already has a whole screen for: `SEND_COPY.documentMissing`, the control beside it, and #295's strip listing the order as not sent.
- **THE STATE IS "SIGNED, NO DOCUMENT", NOT #176's "APPROVED, NO ORDER", AND THE STAGE DECIDES IT.** The document is made when the President signs, long after full approval generated the order, so no failure of the document can leave a request without an order. Moving the check forward into generation would need every quotation fetched and parsed at approval as well as at signature — a transient failure at signature still needs the later path — so one failure would have two places to happen. The retry is where the reason is met, and the automatic path stays silent, both exactly #308's shape one screen over: a code on the throw (`QUOTATION_UNREADABLE`), matched in `regeneratePDFAction`, returning `PO_QUOTATIONS_COPY.unreadable`.
  - **IT NAMES EVERY QUOTATION IT CANNOT APPEND, NOT THE FIRST.** Each is tried, so one repair and one press is enough rather than a round per file. Each is named as the request's page shows it — its file's name, which is the link there — with its `Quotation ID`, which is what Airtable shows; one with no file on record by the id alone. The four reasons are no file, not a PDF, JPEG or PNG, a file that cannot be read, and a protected PDF.
  - **THE REMEDY NAMES AIRTABLE AND NEVER SAYS TRY AGAIN.** The request is approved by then and `editAndContinueAction` takes only one in review, so the file is past changing in the app — `PRECISION_BLOCKED_COPY`'s position. A fetch that fails is the one case another press can clear, so it throws an ordinary error and keeps the generic `Please try again.`, which is true of it.
  - **THE PROTECTED CASE DOES NOT SAY PASSWORD.** The app's viewer opens a PDF locked only against editing without asking for anything, so "protected by a password" would be false to a reader who had just looked at the file. pdf-lib cannot decrypt, and copying from an encrypted file carries streams still encrypted into pages that draw as noise, so it is refused. **It is detected by opening with `ignoreEncryption` and asking `isEncrypted`, not by catching the error**: pdf-lib's error subclasses are compiled to ES5 and fail `instanceof`, so the refusal `load` throws is a plain `Error` whose only distinguishing mark is its message.
  - **THE BASE HOLDS ONE ORDER THIS WILL STOP, AND IT IS LEFT AS IT IS.** `HYE-PO-260915-03` is awaiting signature, and its request `HYE-PR-260915-02` carries one quotation, `quote-387.pdf` — 48 bytes, a PDF header and a comment, from #387's walkthrough. Signing that order makes no document and the retry names that quotation, which is the behavior this issue asks for rather than a fault in the record. Under the code this replaced, the same signature also made no document — reading the file's pages threw on its missing catalog — and the retry said `Please try again.` for every press.
- **THE FORMAT IS READ OFF THE FILE'S CONTENTS, AND THAT HOLDS WHEREVER THE FILE CAME FROM.** Two facts made the declared type worthless as evidence, both read in this issue. The upload routes' `allowedContentTypes` is checked against a type derived from the file's NAME: no form passes `contentType`, the client SDK sends `x-content-type` only when one is given, and Blob falls back to the pathname's extension, so a HEIC or WebP file called `.jpg` is accepted and stored as `image/jpeg` — read from the SDK and Blob's documented default, not measured with an upload. And `createPRAction`/`saveDraftAction`, `editAndContinueAction` and `createInvoiceAction` hand Airtable a caller-supplied URL without asking whose it is, so a file of any kind can become a quotation or an invoice that an overage request then copies into one. Restricting those three is its own issue; reading the bytes makes this module right before and after it lands. **PDF's header is searched in the first 1024 bytes**, which is how far PDF.js looks, so a file this reads as a PDF is one the app's viewer opens as one.
- **AN IMAGE'S PAGE IS THE PICTURE'S SHAPE AT THE ORDER'S OWN SCALE.** Its proportions are the picture's once turned, so the body's "sized to the image" holds with no margin drawn; its size is the largest that fits `ORDER_PAGE_SIZE` turned the same way — A4, which `lib/poPdf.js` now renders the order's own pages at from the same constant. A 4032 by 3024 photograph lands at 793.71 by 595.28 points, a 1170 by 2532 screenshot at 389.03 by 841.89.
  - **PIXELS AS POINTS WAS THE ALTERNATIVE, AND THE READER IS WHY NOT.** That photograph would be a page 56 inches wide. The app's own viewer fits every page to its column (#433), so the screen would never show it; a vendor's viewer set to actual size prints one corner of it. Held to the order's page, every page of the document prints on the paper the order does.
  - **THE PICTURE'S DENSITY FIELD WAS THE OTHER, AND IT DOES NOT CARRY INFORMATION WHERE IT MATTERS.** A phone writes 72 or nothing, which gives the same 56 inches. Where density is real — a scanner — the fitted page lands within a few percent of the sheet anyway: a Letter page at 300 dpi comes out 595.28 by 770.36 against 612 by 792, an A4 one at 595.18 by 841.89. One rule, and no field trusted.
  - **A FULL A4 PAGE WITH THE PICTURE LETTERBOXED ON IT** would make every page one size and was rejected on the body's own words: the page would be the order's, not the image's, and a landscape photograph would sit in a white band across a portrait sheet.
- **IT IS DRAWN THE WAY UP THE APP ALREADY SHOWED IT, AND THAT IS A MEASUREMENT OF CHROMIUM RATHER THAN A READING OF THE EXIF SPECIFICATION.** pdf-lib reads a JPEG's frame header and a PNG's pixels and nothing else, so a phone photograph stored on its side and tagged to stand up printed on its side, while the app's viewer — an `<img>` in Chromium, which applies the tag — showed it upright. Measured in Chromium 152 on a 60 by 40 picture whose four edges were four colors: a JPEG's Exif segment and a PNG's `eXIf` chunk are both applied, all eight values, in either byte order, and the three encodings agree on every row. At the edges: an Exif segment behind an XMP one, or behind the frame header, still counts; an `eXIf` chunk behind the image data does not; an Orientation entry typed LONG, carrying two values, or holding 0 or 9 is ignored. The parser follows exactly that, and each orientation is drawn through its own matrix.
  - **THE CHECK HOLDS THE DRAWING, NOT THE MATRICES.** `offline/po-quotations.mjs` renders a page per orientation with PDF.js and reads the color at each edge against that table, typed in. A matrix and an expectation derived from the same idea would agree whatever it was; a table measured in a browser cannot. Rendering in Node needs a canvas, which is why `@napi-rs/canvas` is a devDependency — see `verification.md`.
  - **pdf-lib READS A JPEG'S HEADER THROUGH `bytes.buffer` FROM OFFSET 0**, so a view into a larger buffer — which a Node Buffer often is — is read from the wrong place and refused as `SOI not found in JPEG`. The module copies the bytes first; the check shows the direct call failing and the module succeeding on the same view.
- **ORDERED BY THE NUMBER IN THE ID, NOT THE STRING AND NOT THE LINKS.** `{PR ID}-Q{seq}` is padded to two digits and `padStart` does not truncate, so the hundredth is `-Q100` and a string sort puts it ahead of `-Q11` — the order list can sort `PO ID` as a string because its date segment is fixed width (#168), and this sequence's number has no such guarantee. `lib/idSequence.js:sequenceOf` is the one reading of that number and `nextSequence` now calls it too, so what counts as a sibling cannot differ between minting and ordering. The link order is not used because it is whatever the base returns, which is creation order until somebody drags the links in Airtable, and it is the order the old appendix's "first" came from. An id that is not the request's own sequence goes after every one that is.
- **THE SEND'S CEILING IS 27 MiB, DERIVED AND NOT CHOSEN.** Resend refuses a mail over 40MB with its attachments base64-encoded. On the strictest reading, 40,000,000 bytes, and with a MIME part writing 78 bytes for every 57 of document, a document may be 29,230,769 bytes less the body; 27 MiB is the largest whole MiB under that. **Before this issue no document could come near it** — one quotation under the 20 MiB upload ceiling behind two order pages encodes to about 28 MB — and after it two large quotations, or a paper quotation photographed a page at a time, pass it. Past it the send answered `try again`, recorded nothing, and failed the same way on every press: #308's loop on the send button.
  - **JUDGED FROM THE ATTACHMENT'S OWN `size`**, which the order record already carries, so the refusal costs no read; a size Airtable has not filled in yet refuses nothing. It comes before the missing-address refusal, so nobody adds an address only to be told the order can never be mailed from here. The figure is not measured: Resend documents no error for an oversized mail, and measuring one takes a real send, which this issue never makes. What the SDK does with a refusal was read instead: it never throws, and a non-JSON body becomes `application_error`, so the old path ended at `SEND_COPY.sendFailed` whatever Resend said.
  - **WHAT IT CANNOT DO IS RECORD A SEND IT DID NOT MAKE.** The refusal says to download the document and send it another way, and that the order will still show as not sent — true, since #281 declined a manual mark and #295's strip counts `Sent At`. Nothing here closes that; it is written down so the strip listing such an order reads as a known state.
  - **THE FILE ROUTE'S COST COMMENT SAID 20 MB OUT AND STOPPED BEING TRUE HERE.** The order document is the one file that route serves which nobody uploaded, and it can now reach the upload ceiling once per quotation; corrected in the same commit (#181).
- **BOTH PATHS MAKE THE SAME DOCUMENT.** `signPOAction` and `regeneratePDFAction` each call `generateAndAttachPOPdf(po.id)` and nothing else to make it, so the credentialed run wrote the three keys a signature writes and drove the retry, which sends no mail; a signature would additionally have mailed the requester through `notifyPOSigned`. What differs is around the call — the retry's gates, the order of the request's status sync — and none of it is read by the document.
- **THE BASE HAD NO ORDER TO REPAIR, MEASURED 2026-09-24.** Of 37 orders, 2 carried a document: `HYE-PO-260915-01` with its request's one PDF quotation as its third page, and `HYE-PO-260915-02`, whose request has none. `Quotations` held 2 rows, both PDFs, each alone on its request, so no stored document was missing an image or a second quotation; none was regenerated, which #281 refuses in any case.
- **ONE READ FEWER PER DOCUMENT.** The request's link array is already in hand, so the quotations are one query and the request is not found a second time (#193's `rowIds`); a request with no quotation costs nothing where it cost two.
- **Not in this issue:** restricting the three actions to our Blob store (its own issue); the request page's quotation list, which still shows link order; a JPEG whose header parses and whose picture data is broken, which pdf-lib appends as whatever survives; rasterizing a protected PDF, which would need a renderer on the server.
