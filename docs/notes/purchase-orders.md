...
- **THE REVERSE LINK ALREADY EXISTED AND SETTLED THE SHAPE.** `/prs/[prId]` renders a `Purchase Order` section with the order's ID as an underlined anchor and its status beside it, gated by nothing but the page's own `canViewPR` — that page's #143 comment even cites this one's gate reading the parent PR. So the visible shape here is the same: an underlined identifier, no badge, no button. **The mechanism differs and was left alone:** the reverse side is a plain `<a>`, and this uses `Link` with `encodeURIComponent` because that is what this page's other two record links (deliveries, invoices) do. Matching the reverse side instead would have put two spellings inside one file for a difference no reader can see; changing that side is behavior and belongs to its own issue.
...
- **Not in this issue:** the `<a>` on `/prs/[prId]`, the `PR:` label itself (#288's inventory), and any change to what `canViewPR` decides.
