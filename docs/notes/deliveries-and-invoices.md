...
Every field on `/invoices/new` transcribes something printed on the vendor's document, and the document was not on the screen: it was attached, read for an order number, and then stood as a filename while the reader worked from a second window. The pane draws it beside the form. The rendering is `app/components/FileFrame.js`, which is #331's frame moved out of `FileViewer.js` unchanged; the classes and the breakpoint are `app/invoices/new/filePaneLayout.js`, whose header carries the arithmetic.
...
- **THE PAGE WIDENS THROUGH `:has()` RATHER THAN THROUGH A PROP, AND THE ALTERNATIVE WAS RESTRUCTURING THE SCREEN.** The container is in `page.js`, a Server Component that also renders the heading and the direct-purchase notice; whether a file is attached is client state inside `InvoiceForm`. One class on the container reading the pane's own data attribute keeps the server page a server page. The alternative was moving the heading into the client so that one component could hold both, which is a restructuring of a screen to carry one class. The attribute and the selector are one string and cannot be bound as one, since a Tailwind class is matched in the source as a literal — they are adjacent lines in the layout module with a comment saying so.
...
- **THE TWO COLUMNS ARE CONDITIONAL ON THE PANE, AND THE COST IS A SHIFT.** `_shared.md`: reserving space for a conditional is the likeliest way to get a screen wrong. So with no file the page is the single centered column it has always been, at every width — measured at 1440: container 672px, form centered at x=409. Attaching a file moves the form to x=785 and puts the document where the form had been. **That jump is the thing to watch in the design pass**: on `PDF Upload` the file is the first block and the first act, so it happens before a reader has anchored on any field; on `Manual Entry` the file is last, so the whole form moves under a reader who has just finished filling it. **The left-hand placement makes the jump land where the reader was looking** rather than in the empty margin, which is the one thing the right-hand version had going for it. If it proves to be a problem, the two repairs are widening the container at `xl` unconditionally — which reserves an empty column — or centering the form in a three-column grid so it never moves, which needs a wider breakpoint than 1280 to leave the pane a usable column. Neither was taken now, because which is right depends on how the shift reads rather than on an argument.
