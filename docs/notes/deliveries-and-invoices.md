...
- **THE DENOMINATOR WAS CONSIDERED FOR THE KEY AND REFUSED, AND THE DECISIVE TEST WAS A ONE-ORDER INVOICE.** Signing `5 of 5` apart from `5 of 50` is a real argument — those lines are not one answer. It fails because it signs them apart ON ONE ORDER too: two items of 5 against ordered quantities of 5 and 50, charging a single order, would draw a list that repeats the items table line for line, and #409's own sentence is that an invoice charging one order stays silent. It is also the wrong KIND of fact — an ordered quantity belongs to the order and is identical for every invoice charging it, so it says nothing about how THIS invoice divided anything. **If it is proposed again, run it against a one-order invoice**; the proposal is built and run as the third mutant in `offline/invoice-order-breakdown.mjs` so the refusal is executable rather than a paragraph.
  - **ONE REACHABLE CASE ESCAPES THE ONE-ORDER SENTENCE AND IT IS NOT WORTH A CLAUSE.** A zero-quantity invoice item signs `A:0` where a real charge signs `A:1`, so a one-order invoice mixing the two draws a list. The qty control is `required` with no `min`, so a typed `0` reaches the base; there are none on it (counted 2026-09-18). Both repairs were worked out and both NARROW the judgment somewhere else — dropping zero shares from the key silences an item that touched an order its neighbour did not, and dropping zero-total items silences an item that named an order alone — and this issue only widens. The issue body's phrasing takes a degenerate row as not charging.
