...
- **THE URL MIRROR IS DEBOUNCED NOW, AND THE REASON IS A CLAIM NOBODY HAD MEASURED.** `useListFilters` wrote the URL on every state change, which was one `router.replace` per click until this issue put a text box in the bar and made it one per KEYSTROKE — seventeen for a printed id. Three comments in this repository call that write free ("no navigation, no history entry, no server round trip") and none of them was ever measured; a soft navigation on a page that reads `searchParams` refetches that page's payload, so being wrong costs every read the screen makes, seventeen times over. A 250ms trailing delay makes the question moot rather than settling it: one write per query, and nothing at all if the mirror really is free. **The rows do not wait** — narrowing is synchronous and only the address bar catches up.
