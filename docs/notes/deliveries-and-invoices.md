...
- **THE BOX AND THE DOCUMENT ARE ONE RECT, MEASURED.** The empty box carries `FileFrame`'s own `mt-4 min-h-0 flex-1`, so at 1440 the box sits at top 80, 721 wide and 792 tall, and the frame that replaces it sits at top 80, 721 wide and **768** tall. The 24px is the standing hint line under a document, which an image does not have. Closing it means either editing a frame six other surfaces share or reserving a line of text that is not there, so it is left and written down.
- **THE BOX STOPS TAKING DROPS ONCE IT HOLDS A DOCUMENT, AND THE FILE CONTROL UNDER IT IS THE ANSWER.** A frame is another document: a file dropped on it goes to the browser's own viewer and nothing in the page can intercept it. So the control sits under the box in every state rather than appearing with the file — which is also what keeps the box one size, since a control that appeared would take its line out of the box the moment a document arrived. It is the browser's own file input, which means no second way to replace a file and no word coined for one.
