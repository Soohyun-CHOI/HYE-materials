...
- **The photo has TWO writers, and that is the one deliberate departure from #142's one-writer rule** — the photo is editable in place, unlike a Quotation's file. `createDelivery` writes it at creation; `replaceDeliveryPhoto` is the narrow second writer, in `setPOItemMaterial`'s shape, and **refuses any url that is not a fresh Vercel Blob upload**. That precondition is what makes #142's failure mode — re-submitting an url Airtable issued, which succeeds and silently empties the field — unreachable by construction rather than by discipline. `updateDelivery` has no `file` parameter and must not grow one. Enforced by `offline/source-shape.mjs`: exactly two writes, one in each function, none in `updateDelivery`. **Since #438 both writers hold the precondition**, `createDelivery` having never asked, and "a Blob upload" means one on this app's own store rather than any Vercel customer's — `assertOurBlobFiles`, which `offline/file-source.mjs` requires of every attachment writer, with `createDeliveryAction` refusing such a url first in the sentence a missing photo gets. `uploads-and-drafts.md` carries the rest.
...
A delivery linked a job and recorded no address, so where material arrived was read through the job — editing a job's address moved where every past delivery appeared to have gone, and a delivery that turned up somewhere other than the ordered address had no way to say so. `Deliveries` gains a `Delivery Address` link, chosen at entry and defaulted from the orders the delivery attaches to. Last of the four address issues; `docs/notes/addresses.md` owns the chain.

- **#386's PROBLEM DOES NOT ARISE HERE, AND CHECKING THAT WAS THE FIRST THING.** That issue refused to retype `Purchase Orders."Delivery Address Used"` because the Metadata API rejects a type change and its own code had to WRITE the field, so no ordering left the committed half correct. **The premise there was an EXISTING field of the wrong type.** `Deliveries` had no address field of any kind, so this is a plain additive create and the window is zero for the ordinary reason: code that does not know a field ignores it, and code that does writes it. One hand step (the single-record toggle), no deletion, no type change.
- **THE DEFAULT IS WHAT THE ORDERS AGREE ON, AND A DELIVERY REACHES SEVERAL BY CONSTRUCTION.** `planDelivery` fills oldest-order-first across every candidate ordered item for a (job, vendor, material), and two MATERIALS on one packing list are planned independently and land wherever their own orders are. So `lib/deliveryAddress.js` reports one address, several, or none, and never picks between rivals — which is `lib/deliveryInvoiceMatch.js`'s posture on a tie, already rendered in amber on this same form.
  - **MEASURED: 0 of the 12 deliveries on this base touch two orders**, and that is a fact about the seed rather than about the design. Eleven are one order (either two of its ordered items, or one ordered item split into a within row and an over row) and the twelfth carries a row with no ordered item at all — the `UNATTRIB` fixture. **So the disagreement branch could not be seen on the base as it stood**, and the browser walk BUILT the case through the app's own path rather than asserting it from the offline tier alone: a second request with a different address, approved into a second order, then one delivery carrying a material from each.
  - **AN ORDER WITH NO ADDRESS NEITHER AGREES NOR DISAGREES.** 34 of the 35 orders record none, because #386 deliberately backfilled nothing, so counting silence as a rival would make every ordinary mixed delivery read as a conflict. It is excluded from the rival test and reported as `partly` instead, which is the one shape where the app says what its answer is standing on.
- **IT DOES NOT FALL BACK TO THE JOB'S DEFAULT ADDRESS, and the fallback is one token away.** The form holds the job — `addressOptions` needs it for the picker's first group — so `|| selectedJob?.deliveryAddress?.[0]` costs no read and reads as a kindness. Three things against it. A delivery address is a claim about where material PHYSICALLY ARRIVED and a job's default is a habit rather than evidence. **Inventory is counted per address from the next issue on**, so a prefilled guess a busy recorder accepts puts stock in the wrong place with nothing on screen to say so. And reading a job's address live is exactly what #384, #385 and #386 spent three issues removing. What replaces the convenience is the grouping: the job's own addresses are first in the list, one click away, without the app asserting one. Held on the AST, because a comment cannot stop a one-token edit.
- **REQUIRED AT SUBMIT, AND THERE IS NO SAVE TO BE OPTIONAL AT.** #385's rule is "required at submit, not at save" and its second half has no subject here: `/deliveries/new` has one write path and no draft. The argument for requiring it is #385's one table across — an optional link would hand INVENTORY a nullable location, which is the state this chain exists to end. The 12 existing deliveries keep none and are not backfilled: 11 attach to orders that record no address either, and the twelfth would be a guess about where a pallet went. "No delivery carries an address" is a property of this base rather than of the design.
- **THE DELETE PATH NEITHER RESTORES NOR COPIES IT, AND THE REASON IS STRUCTURAL.** The address is on the HEADER, which `deleteDeliveryAsUser` destroys — there is nothing to restore. What that path does CREATE is `Delivery Items` rows, when `recomputeFlagsForOrderedItems` splits a straddling row, and those carry no address. **Adding it to that copy list would be the defect rather than the fix:** a split row is created on its source row's own delivery (`deliveryRecordId: from.delivery?.[0]`), so it INHERITS the header's address, and copying it would put one value in two places. The check asserts the inheritance rather than the absence, since the absence is what a future edit would remove.
- **ONE WRITER, AND NOT FOR ALLOCATION'S REASON.** `updateDelivery` does not take the field and must not grow it. The three editable things — `Received Date`, `Notes`, `Packing List File` — are editable because nothing re-plans when they move, and the address passes that test: nothing is re-allocated by changing it. What fails is a different test. From the next issue on the address is what a quantity on hand is counted by, so **moving a recorded delivery to another address is a stock movement rather than a correction**, and a stock movement with no record of itself is a behavior nobody has designed. Leaving the field write-once keeps that question open instead of settling it by omission; the issue body was edited to say so rather than being read narrowly.
- **THE WORDS ARE `/prs/new`'s WHEREVER THE QUESTION IS THE SAME.** The label, the placeholder, the two group headings and both refusals come from `ADDRESS_CHOICE_COPY`; `DELIVERY_ADDRESS_COPY` holds only the four sentences about where the DEFAULT came from, which is a question that does not arise on a form whose default is the job's. The check asserts the borrowed six are ABSENT from the new constant, so re-coining one shows up as a new key rather than as a silent second wording.
  - **`arrived` IS BARRED IN THE COPY AND KEPT IN THE PROSE, which is not a gap.** `offline/line-vocabulary.mjs` says so in its own header — the word is legitimate for something outside this base, so "prose keeps it and only a screen may not" — and #166 chose `delivered` for this act. The first draft of the copy said `pick where it arrived` and failed that check; it says `pick where the material was delivered`. This file's own sentences keep the word, deliberately.
- **`lib/addressChoice.js` GAINS ITS SECOND SCREEN AND ITS MOVE CONDITION DOES NOT FIRE.** That condition is on `addressesOnJob` and asks for a THIRD direct caller, or one wanting the job's addresses without the create screen's framing; this screen calls `addressOptions`, which is that function's second caller and stays its second. What did change is the module's header, which said it was #385's and now serves two forms — corrected in this commit rather than left to read as one screen's.
...
- **The filter follows the PR list's pattern (#119)**: the server sends rows it has already computed, a Client Component narrows them instantly with no Apply button, and the active filter is mirrored into the URL with `router.replace` — no history entry and no remount — so refresh, a shared link and the back button all restore the view. **That sentence said "no navigation, no server round trip" until #325 and both halves were false**: the mirror is a soft navigation, and on a page that reads `searchParams` that fetches the page’s payload again — measured in a browser, one request per write. It is debounced now; the figures are in this file’s #325 section. It is `Over-delivered` (`?over=1`); the name does not say "only", because a filter is a toggle and the word is implied. **There were two until #216**, which moved `Not fully invoiced · oldest first` (`?unbilled=1`) to a strip above `/invoices` — see that issue's section below.
...
- **BOTH LIST TABLES ARE `table-fixed` WITH A DECLARED `colgroup` SUMMING TO EXACTLY 52rem**, which is what a `max-w-4xl` page minus `p-8` has (832px). A column is never appended; the budget is re-cut. Measured against each base's widest real cells, with every row one line, no wrap and no horizontal scrollbar.
  ...
  - **AND AN EIGHTH COLUMN IN #314, WHICH IS THE ONE CORRECTION THAT DID NOT COME BACK HERE (#183).** Every re-cut above was appended to the sub-bullet it corrected — #210 to `/deliveries`, #211 inside `/invoices`, #309 as the bullet before this one — and #314 wrote its own in a section two thousand lines down instead. So the sub-bullet above still reads `(7 columns)`, `seven columns need 832px against 832px`, `Six of the seven are bounded by construction` and `Status by ... (176px)`, and all four are about a table that has carried **eight columns** since `Job` was added, with Status re-cut to 106px to help pay for it. **The parent bullet is untouched and is why this is a correction rather than a finding:** this table re-cuts rather than appending, so the eight still declare exactly 52rem and the budget never moved. The per-column figures are in that issue's own section; what is fixed here is the count a reader of THIS bullet would act on. The chain matters more than any one number — a living bullet that stops being appended to is indistinguishable from a frozen record, and nothing says which one a reader is holding.
...
### What the delivery and invoice lists filter by (#324)

The four document lists filtered three different ways and no list filtered by vendor, though every one of them heads a `Vendor` column. `/deliveries` carried one checkbox and `/invoices` carried nothing at all. The shared answer, the rule behind it and the words are `lib/listFilters.js`; this section is the half of it that belongs to these two screens.

- **THE RULE IS THIS ISSUE'S AND #216 IS THE PRECEDENT IT GENERALIZES, WHICH IS WORTH SAYING PRECISELY BECAUSE A READER WILL GO LOOKING.** #216 moved `?unbilled=1` off `/deliveries` and made it a strip above `/invoices`, and it wrote down why: "a log reads newest first and its empty state means nothing delivered, a chasing list reads oldest first and its empty state means there is nothing left to do". That is one move with its grounds, and it is all that is there. The general rule — **a closed set a list renders is a filter when the document or its own item rows hold the value, and a strip's subject when the values lie along a wait whose end is the good one** — is #324's, derived from that reason rather than quoted from it.
  - **AN EARLIER DRAFT OF THE RULE SAID "a relation to another document is a strip's subject" AND IT IS FALSE.** `AwaitingDeliveryStrip` (#256) counts INVOICES, and every one of them has a row in the table directly beneath it. A strip is not defined by what its rows are about; it is defined by being a worklist — ordered longest-wait-first, rendering nothing when empty. So a fact may take both shapes, and often should: `/pos` carries a status filter alongside two strips about statuses.
  - **A WAIT IS RECOGNIZED BY DEGREES.** `Awaiting X` -> `Partly X` -> `X` is a wait; a binary fact with no middle is a state. That is what splits payment across two lists instead of leaving it to taste — see below.

- **`Over-delivered` SURVIVES ON `/deliveries` BECAUSE THE RULE PRODUCES IT, NOT AS AN EXCEPTION.** #181's body records `Delivery Items."Over Delivery"` as a checkbox rendered `Over-delivered`, so it is a stored value on the delivery's own item rows. It has no degrees, it does not end, and its absence is not "done". The wait it creates — an excess with no request raised for it — is #217's strip on `/prs`, which is the same fact in the other shape. **Recording it as an exception would have been the defect**: an axis whose only ground is one precedent gets reopened by the next person, and the brief's own warning ("a redesign adding filters here should know it is adding, not restoring") was already carrying that weight alone.
- **THE `Invoiced` CHIP ON `/deliveries` AND THE `Delivery` CHIP ON `/invoices` ARE NOT FILTERS**, by the same rule and with no case-by-case argument: both are waits with degrees, and both already have their strip on `/invoices`.

- **PAYMENT IS A FILTER ON `/invoices` AND NOT ON `/pos`, AND THE APP ALREADY DREW THAT LINE BEFORE THE RULE NAMED IT.** An order's payment aggregates the invoices charging it, so it has a `Partly paid` and reads as a chip in the shared palette; an invoice holds `Paid Date` itself, so it is binary and reads as a word in the `Status` column. Degrees is what makes one a wait and the other a state. The filter is named `Status:` after the column it narrows rather than after the field behind it, which is the same relation `job` already has to the `Job` column.
  - **`Overdue` IS NOT A THIRD OPTION.** #316 made it a badge that composes with the payment word rather than replacing it, so an option would make the select's values overlap and leave a reader wondering what picking `Not paid` had excluded.
  - **THE TWO WORDS COME OUT OF THE CHIP SET RATHER THAN BEING WRITTEN AGAIN.** `INVOICE_PAYMENT_WORDS` reads `STATUS_COPY.column.poPayment`'s two builders, so #311's convergence on `Not paid` — which followed the app having spent `Unpaid` and `Not paid` on one fact — is now held by construction. The invoice list had been spelling the pair inline, in JSX, where no check could see it.

- **`/invoices` GAINED A CLIENT COMPONENT, AND ITS ABSENCE WAS THE REASON THAT LIST HAD NO FILTER.** The table was rendered inside `app/invoices/page.js`, so there was nowhere for a narrowing to live and nothing to hand a row to. `InvoicesListClient.js` is the shape the other three already had: every judgment stays on the server — the chip, the payment word, the overdue badge, and the job the gate's own walk resolved — and the browser reads keys. #316's rule is kept rather than re-argued, since nothing in the new component compares `Due Date` to anything. **Four checks moved with the table** and none of them changed what it asserts: `offline/job-column.mjs`'s header map, `offline/invoice-overdue.mjs`'s due-date render, and `offline/invoice-visibility.mjs`'s `.paid` inventory, which now carries the page for the judgment and the client for the render.

- **`/deliveries` WAS TELLING A READER SOMETHING FALSE, AND THIS IS THE FINDING RATHER THAN A CARRIED-OVER RULE.** The three-way empty state is the rule `lib/poListView.js` stated when `/pos` was written and `/pos` was the only list obeying it: "nothing here yet" and "nothing here FOR YOU" are two facts, and the word `yet` is what makes the first false for the second case. This list had `No deliveries recorded yet.` for a reader who has jobs and no delivery on them — on a base with plenty. `/prs` had the milder version of the same thing, two sentences where its own brief recorded that the second "covers both an empty base and a reader whose scope is empty, and it does not distinguish them".
  - **THE BASE COUNT COSTS THIS PAGE NOTHING, which is why the fix is here rather than filed.** `getAllJobs()` already returns every Job's `Deliveries` reverse-link and the page was narrowing that array and discarding the original. Counting the union of the unnarrowed one is a `flatMap` over records already in hand: **zero new queries**, and `Deliveries."Job"` is single-link so no delivery is counted twice. `allJobs` and the narrowed `jobs` stay two locals for the reason #314 refused to fold the same pair on `/invoices`.
  - **THE ACCESS SENTENCE SURVIVES AS THE `hidden` SENTENCE.** "You are not assigned to any job yet" and "your jobs hold no delivery" are one state — nothing in your scope — so one sentence covers both, and it keeps the Airtable line that makes this the app's only screen telling a reader how to get access. The packing-list teaching moved to `none`, which is the state it is true of.

- **THE VARIANCE AXIS HOLDS AND IS NOT BUILT HERE, AND THIS PARAGRAPH IS WHY RATHER THAN AN OMISSION.** `Invoices."Variance Flag"` is a state this invoice holds — backend-set, closed, and already rendered on the row — so the rule produces a filter for it, and it can never be a strip's subject: it is arithmetic inside one document with no wait to order by, so a fact the rule admits nowhere else would fall through the partition. What it lacks is a NAME on this list. The design pass has settled that the marks of that family merge into one textless `⚠` here with the kind distinction moving to the detail, and a separate issue groups them as tabs, where the tab names and the subject marks get decided. A filter label would then be the only word this screen has for the fact, which is code naming a symbol before design has said what it asserts — and it is not yet settled whether the merged mark covers the header flag alone or order variance too, so a filter keyed on the one flag could disagree with the symbol beside it. Recorded in `docs/notes/backlog.md` so it is picked up with that work rather than rediscovered.
  - **AN UNPAID-INVOICE STRIP IS A DIFFERENT QUESTION AND IS ALSO NOT HERE.** The rule says which shape a fact takes when a screen surfaces it, not that a fact may have only one; payment is a filter on this list and whether the office also wants a chasing strip for it is untouched. #263's shape is what one would copy.

### Recording who entered an invoice (#382)

`Deliveries` and `Direct Purchases` each store the person who entered them and `Invoices` stored nobody. Two things followed from that one gap, and the second is why this issue stood when it did: an invoice was the one document whose reader had no way to ask about a figure that looks wrong, and `/invoices` was the one document list `#324` could give no reader's-own toggle. That issue wrote the absence down in five places as a fact about the schema rather than a judgment about the screen, so the field and the toggle had to ship together or the repository would be holding a reason that had stopped being true.

- **ONE ENTRY POINT WRITES IT, AND THAT WAS CHECKED RATHER THAN ASSUMED.** `createInvoiceAction` is the only thing in the app that creates an `Invoices` row. **#272's direct-purchase path does not**, which is the reading a glance at `app/invoices/new/actions.js` invites, since both actions live in that file and both take an invoice PDF: `createDirectPurchaseAction` calls `createDirectPurchase` and writes a `Direct Purchases` row, and the invoice it came from cannot be entered at all until the request it raises is approved and its order signed.
- **THE EDIT PATH IS CLOSED BY HAVING NO PARAMETER, NOT BY A GUARD.** `updateInvoice` builds its `fields` object from named parameters, so a field absent from the signature has no route to the record — and who typed an invoice in is not who last touched it. Stating that as a refusal inside the writer would have been a rule to keep in step; leaving the parameter out is the rule.
  - **SO THE RULE HAS NO CHECK OF ITS OWN AND DOES NOT NEED ONE.** `offline/invoice-money-terms.mjs` pins the parameters `createInvoice` and `updateInvoice` DO take, one term at a time, because a money term silently dropped is a wrong total. This is the opposite shape: the mutant would be somebody adding a parameter, which is an edit to a signature nobody makes by accident, and a check asserting the absence of a name is a check that any rename passes.

- **NO BACKFILL, AND UNLIKE `#386` THERE IS NOTHING TO BACK FILL FROM.** Every invoice on this base predates the field. The base records no fact that would say who entered one — an invoice carries no `Created At`, and its `Invoice ID` dates the record without naming a person — so a backfill would have to guess from who signed the order it charges, which is a different claim wearing this field's name. This is `#387`'s decision one table over, taken for a sharper reason: there the twelfth delivery *could* have been guessed at and was not, here there is no candidate at all.

- **THE INVOICE'S LINE CARRIES NO INSTANT AND THE DELIVERY'S DOES.** `/deliveries/[deliveryId]` reads `Recorded by: {name} on {Instant}` off its own `Created At`; `Invoices` has no such field and this issue did not add one. The day is not missing from the screen — an `Invoice ID` is `HYE-INV-YYMMDD-##`, which is exactly why `getAllInvoices` sorts by it — so the heading already dates the record, and a second rendering of the same day beside a name would be the app saying one thing twice. **The word is identical and only what is said beside it differs**, which is the distinction `docs/notes/naming.md`'s new row records.

- **THE LINE IS ALWAYS DRAWN, WITH AN EM DASH WHEN EMPTY, AND TODAY THAT IS EVERY INVOICE.** The three lines above it in that block already render an em dash for a missing fact, so the grammar is the screen's own. Hiding it when empty was considered and rejected on the ground the field exists for: a reader who cannot tell who to ask needs to be told that nobody is recorded, and a screen that says nothing leaves them unaware the app holds this at all.

- **THE SESSION IS READ ONCE AND THE ACTION SPENDS NO MORE THAN BEFORE.** `createInvoiceHandler` already called `requireUser()` — below the create, inside `#231`'s pairing block, with a note that `withAdminAction` loads a user to decide the gate and discards it. The write needs the same person, so the call moved above the first create and the pairing reads the binding: two reads of one person, which is what this action already spent, rather than the three a second call would have made. Placed below the refusals, so a submission this action turns away still costs nothing and no record is created before the person writing it is known.
  - **AND IT TOOK THE CALL OUT OF A `catch` THAT SHOULD NEVER HAVE HELD IT.** `requireUser()` can `redirect()`, which in a Server Action is a throw; inside the pairing's best-effort `try` that throw was caught and logged as a pairing that could not be computed. Nothing had met it — the wrapper has already established an Admin by then — but the two states it could reach are both worse than a refusal: a signed-out reader, and since `#381` a signed-in one with no name. Outside the `try`, an Airtable failure on that read now means no invoice was created rather than one created without a recorder.

- **THE AXIS IS `#324`'s DECLARATION WITH ONE MORE ROW, AND THE BAR NEEDED NO CHANGE AT ALL.** `LIST_AXES["/invoices"]` gains `mine("Recorded by me")` in the subject-axis position, `ROW_KEY.mine` and `FILTER_PARAMS` already held the names, and `app/components/ListFilterBar.js` draws whatever is declared. What that leaves is one line on the page — `isMine: inv.recordedBy?.[0] === user.id`, resolved on the server so nobody's identity reaches the browser, which is the arrangement all three neighbors already use.
  - **AND IT EXPOSED A MUTANT THE TIER COULD NOT SEE, WHICH IS THE REAL FIND OF THIS ISSUE.** Declaring an axis is half the work and shaping its row key is the other half, in another file, with nothing pairing them. `matchesFilters` reads `row[ROW_KEY[param]]`, and a missing key is `undefined` — which passes every control while it is idle and fails every one of them the moment a reader picks something. So a list could declare `mine` and never set `isMine`, drawing a live toggle that narrows to nothing, and every assertion `#324` wrote would still pass. `offline/list-filters.mjs` now reads each of the four pages' own object literals against `ROW_KEY`, which is why that constant is exported. It is `offline/job-column.mjs`'s shape — "remove the render, leave the read, and the screen is right, the budget is unchanged and nothing anywhere fails" — one axis over, and it was reachable here because adding an axis to a list is the one edit that produces it.

- **THE LABEL IS WRITTEN INTO JSX, AND THE MEASUREMENT BEHIND THAT IS WORTH KEEPING.** `Recorded by:` is a bare text node on both detail screens rather than a copy constant. `scripts/screen-strings.mjs` reports one as `[JSXText]`, measured on the delivery's own line, so a vocabulary sweep finds it; what a JSX literal cannot have is a PIN, since `offline/screen-briefs.mjs` falls back to asking whether the literal appears anywhere under `app/`. A constant was weighed and dropped: it would be invoice-only, which is one word in two shapes across two screens saying it, or shared, which is a new module for one label against `CLAUDE.md`'s remaining headroom. **The word is not a sentence a design may reword** — it follows the field name, which `naming.md`'s participle convention fixes — so what a pin would protect is the thing least able to drift.
  - **`lib/userName.js` WAS CARRYING THE WRONG REASON FOR THE SAME CHOICE AND IS CORRECTED ON SIGHT (`#181`).** Its `USER_NAME_COPY` docstring said the extractor cannot see a string written into an element, which has been false since `#288` added `JSXText` to that tool. The real reason a constant buys something is the pin, and `offline/screen-briefs.mjs`'s own header says so.

- **THE WHOLE PATH WAS EXECUTED, NOT ARGUED — `verify-invoice-recorder-382.mjs`.** `createInvoiceAction` is out of reach of both cheaper tiers (the offline tier never opens a page, and no browser tooling can fill the file input `/invoices/new` requires), so the half of this feature that WRITES had no execution without a script. `verify-invoice-pairing-231.mjs` is the precedent and PR #234 named this pattern as the way to measure an action behind a required file input without adding one. Measured, against the field in its final single-record shape: the create wrote `["recobE1w6Hrlk4xwA"]` and nothing else, the detail page rendered `Recorded by: Soo` and an invoice entered before the field rendered the em dash, the list's bar carried `Recorded by me`, and `?mine=1` through the production filter admitted exactly the one row. 13 Airtable operations.
  - **THE EDIT IS RUN RATHER THAN READ OFF THE SIGNATURE, AND BOTH HALVES ARE ASSERTED.** "There is no parameter for it" is a claim about a PATH, so the script posts a real edit and checks that the field it was given changed AND that the recorder did not. Either half alone is the vacuous version: a field that did not move proves nothing if the edit never ran.
...
### What each document list searches (#325)

The four lists offered no way to reach a record by its name. A vendor rings the office quoting THEIR invoice number and an office reader holds a printed document and reads the one on it, and neither reached the row except by scanning the list. The box, the words and the composition are `lib/listFilters.js` and the tokenizer is `lib/searchTokens.js`; this section is the half of the decision that belongs to these two screens, plus the rule itself, which is settled across the four exactly as the filters are.

- **THE RULE: A LIST SEARCHES THE NAMES ON THE ROW — the document's own id, any second name the document's OWN row carries, and the two subjects every one of these lists heads a column for, the vendor and the job. It never reaches into another document to borrow a name.** The premise is that a reader types what they are holding and does not know which field they are naming, so the app works that out; what the rule bounds is how far "what they are holding" reaches.
  - **AN EARLIER DRAFT SAID "the names the document itself is called" AND IT COLLAPSES THE MOMENT THE VENDOR AND THE JOB ARE IN.** Neither of those is the row's own name. What survives is a different distinction: the vendor and the job are the row's SUBJECTS — a row is "this document, for this job, from this vendor", which is how a person says it out loud and what #314 gave all four lists a column for — where the next document along is a link.
  - **AND THE REASON THE LINE HAS TO BE DRAWN SOMEWHERE IS THAT THE CHAIN HAS NO END.** Allow a delivery to be found by the order on its packing list and the same argument reaches an invoice by the order it charges, an order by its request, a request by its quotation. The chain is the app.

- **THREE NAMES FALL ON THE FAR SIDE, AND EACH WAS WEIGHED RATHER THAN OVERLOOKED.** `Deliveries."Packing List PO"` is a link, so what a delivery holds is a reference and the name is the ORDER's; the way from an order to the deliveries filling it already exists and is better, since `lib/poDocuments.js` lists them under the order. A request's `Quotations."Vendor Quotation Code"` and its `Direct Purchases."Vendor Invoice Code"` are both real vendor-side names one link away, on another table's row. **All three also cost a read of a table the page does not fetch**, where the vendor and the job cost nothing — which is not why they are out, but is the shape of the same fact.
  - **`offline/list-filters.mjs` HOLDS THE THREE BY NAME**, because an exclusion nobody wrote down is one the next reader re-adds.

- **WHAT THE LIVE SCHEMA SAYS, READ RATHER THAN ASSUMED.** `Invoices` is the ONE of the four document tables holding a human-entered second name (`Vendor Invoice Code`, singleLineText). `Purchase Requests` holds none. `Deliveries` holds none — it has a packing list FILE and a link to an order, and no packing list number anywhere on the base. `Purchase Orders` holds none either, and that one is worth stating: what the vendor receives is the PDF this app emails them (#281) with our own `PO ID` on it, so our name is the only name an order is ever called.

- **THE TOKENS AND THE MATCH ARE `/materials`' RULE, CONVERGED RATHER THAN COPIED.** `lib/searchTokens.js` now owns the fold, the AND, the substring and the empty case, and `buildSearchTokens` is a call to it with the two arguments that belong to that screen — the separator a pasted category path carries, and the formula-length ceiling. The alternative that was rejected outright is `lib/airtableFormula.js:prefixMatch`: measured case-SENSITIVE in #164, anchored at the front, and an Airtable formula builder, where this box matches in the browser. Each of those three alone rules it out, and the middle one matters most — the fragment a person picks out of `HYE-INV-260803-02` is as often the date in the middle as the prefix.
  - **THE MATCH SIDE IS NECESSARILY TWO IMPLEMENTATIONS AND THE QUERY SIDE IS NOT.** `/materials` asks Airtable because it queries a table its screen does not fetch; the lists ask JavaScript because every row they could match is already in the browser. One semantic, two languages, and `offline/search-tokens.mjs` holds them to the same answers.

- **THE AND CROSSES FIELDS, WHICH IS THE ONE DECISION THIS ISSUE ADDED OVER ITS FIRST DRAFT.** The row's names are joined into one haystack, so `acme 2608` finds an invoice from Acme whose id carries 2608 — two tokens, two fields, one row. Per-field AND, where one name must hold every token, is predictable and was rejected: it requires the reader to know the fields, which is the burden this box exists to remove, and `acme 2608` would find nothing. The asymmetry decides it the way it decided #357's substring question — joining can only ADD matches and an unwanted one is narrowed by another word, where per-field can only lose them.
  - **AND `/materials` ALREADY CROSSES FIELDS, WHICH IS WHAT MAKES THIS CONVERGENCE RATHER THAN TASTE.** Its haystack is `Material Label`, itself a concatenation of category path, size and unit, and `buildSearchTokens` records the consequence in as many words. Choosing per-field here would have made one rule behave two ways on identical input.
  - **JOINING ON A SPACE NEEDS NO SEPARATOR CHARACTER.** A token never contains a space, because the tokenizer split on one, so nothing can match across the boundary between two joined names.

- **A TYPED NAME AND A CHOSEN ONE COMPOSE WITH AND, AND NEITHER CONTROL TOUCHES THE OTHER.** Typing `Acme` with the vendor picker set to another vendor returns nothing, which is right — the reader said two different things, and the count beside `Clear all filters` is how they see it. The box does not preselect a picker and a picker does not fill the box: one narrowing silently rewriting another would put a state in a shared link that the screen never showed. `Clear all filters` empties the box too, which is what "all" has to mean for a control sitting in this bar.

- **THE EMPTY RESULT IS `filtered` AND THERE IS NO FOURTH STATE.** The box is a control in this bar and the value it matches is held by the document, so #324's own rule makes it a filter — `emptyStateKind` is untouched and still answers its three questions. What is new is that the sentence NAMES the narrowing, because a typed term is the one whose content the reader can get wrong: `No invoices match “ABC-1029”.`, and `… “ABC-1029” and these filters.` when both are on, since neither half alone would be true. The four `filtered` sentences are built from a noun per list now and are byte-identical to #324's; `offline/list-filters.mjs` pins all four as literals, which is the only way to catch a builder that punctuates differently.
  - **NO SECOND SENTENCE EXPLAINING THE MISS, WHICH `/materials` HAS.** That box can ask the catalog whether the words exist at all. Here the two candidates are a typo and a document nobody has entered, and nothing on the base distinguishes them — the pair this list CAN tell apart is already `none` and `hidden`.

- **IT RUNS IN THE BROWSER AND COSTS NOT ONE AIRTABLE OPERATION, ON ANY OF THE FOUR.** Each list already fetches every row its reader may see, so there is nothing for a server search to find that is not in hand; going to the server would also need a second implementation of each list's gate. Every value the box matches is already resolved for a column or a picker label: `vendorName` is a rendered cell on all four, `jobCode` likewise, `jobName` rides on the same `getAllJobs()` records the job picker's label is built from, and `vendorInvoiceCode` is on the record `getAllInvoices` returns. The rows carry one new key each (`jobName`) and `/invoices` two.
  - **`/api/pos/search` IS NOT THIS AND STAYS AS IT IS.** That route is Admin-only and backs the invoice form's escape hatch to orders the FORM did not fetch, which is the one condition that makes a server search the right shape.

- **THE URL MIRROR IS DEBOUNCED NOW, AND THE REASON IS A CLAIM NOBODY HAD MEASURED.** `useListFilters` wrote the URL on every state change, which was one `router.replace` per click until this issue put a text box in the bar and made it one per KEYSTROKE — seventeen for a printed id. FOUR places in this repository called that write free ("no navigation, no history entry, no server round trip") and none of them had ever been measured — `ListFilterBar.js`, `POListClient.js`, `DeliveriesListClient.js`, and the #166 note near the top of this file. **All four are corrected in the same commit**, which is #181’s rule rather than a courtesy: a comment false about the current code is fixed on sight, and this one had been repeated three times by copying. **Measured in a browser:** a soft navigation on a page that reads `searchParams` fetches that page's payload again, one `GET …?_rsc=…` per write, so being wrong costs every read the screen makes, seventeen times over. A 250ms trailing delay brings it back to one write per query — seventeen keystrokes, one request, measured after the change. **The rows do not wait** — narrowing is synchronous and only the address bar catches up.

### What a page of a document list holds (#326)

The four lists rendered every row a reader could see, and not one of them said how many that was — so a reader could not tell a whole list from the beginning of one, and a long list was a long scroll. `#324` left the name `page` reserved for this and `#325` spent the one beside it; this is the last of the three and it is the same module's declaration with a slice on the end, not a second one beside it.

- **THE CUT IS IN THE BROWSER, AFTER THE BAR, AND THE READ IS UNTOUCHED.** The chain is: the server reads, gates, shapes rows and hands them over; `useListFilters` narrows them with the bar; and `pageOfRows` divides what survived. It cannot be earlier than the gate, which is the issue's own sentence — most of these lists admit rows through `canViewPR`, whose ordered rules are not expressible as a query, so a page of RECORDS read from the base is not a page of ROWS a reader sees. It cannot be on the server at all, for a second reason the first one hides: **the bar narrows in the browser**, so nothing upstream knows how many rows survive to divide. That is also why the page parameter is the only one on these four routes that no page validates before handing it down — what it has to be checked against is a count only the browser holds.
- **WHEN THE READ WOULD HAVE TO DIVIDE, WITH THE FIGURES THAT SAY WHEN.** Measured while this issue was open — rows on the base, and operations per render aggregated from 18,643 scopes in `.airtable-ops.jsonl`:

| list | rows | ops per render (min / median / max) | pages at today's size |
|---|---|---|---|
| `/prs` | 41 | 1 / 15 / 19 | 2 |
| `/pos` | 37 | 7 / 8 / 10 | 2 |
| `/invoices` | 22 | 3 / 14 / 15 | 1 |
| `/deliveries` | 15 | 7 / 7 / 7 | 1 |

  The read grows in steps rather than smoothly: children come back one query per fifty identifiers, so a list crosses a step every fiftieth row and sits flat between them. **What must change before the read can divide is one of two things, and neither is cheap.** Either `canViewPR`'s ordered rules become expressible as a query — which means the gate's answer living on the base as a stored field, written by this app and kept in step, with everything that implies for a rule that reads a request's signers; or the lists accept that a page of records is not a page of rows, which means a reader seeing fewer than a page's worth on a page that is not the last, and a total that is a count of records rather than of rows they may see. **The figures above are the baseline for deciding that.** Nothing here is near it: the largest median is 15 operations and the largest list is 41 rows.
- **THE SIZE IS IN ONE PLACE AND NO SCREEN READS IT.** `LIST_PAGE_SIZE` lives in `lib/listFilters.js` and `pageOfRows` is the only thing that touches it, so the four lists agree by construction rather than by four constants that happen to match — and moving the figure is one edit with nothing to sweep. `offline/list-filters.mjs` derives every length it asserts from that constant rather than pinning the number, which was proved by moving the size to 40 and watching the check still pass while six real defects — an off-by-one slice, a clamp to the wrong end, a parameter written on the first page, a client slicing by hand, a list that stopped rendering the foot, and a noun that lost its singular — each failed it.
- **THERE ARE THREE NUMBERS IN THIS APP THAT LOOK LIKE ONE AND ARE NOT, AND THE COINCIDENCE IS WORTH RECORDING BEFORE SOMEBODY FOLDS THEM TOGETHER.** `TOOL_PAGE_SIZE` was ten when this was written, and its own note left this issue the choice: that screen is read at a phone width (#336) and its row is three short facts, where these four are read at a monitor and their rows are six columns. **#442 made it 25, the design's figure for that list, so all three carry one number now** — the coincidence at its most tempting, which is why that issue put the warning above both declarations and why `offline/tool-list-view.mjs` fails a fold of the two page sizes into one constant. `/materials` has a 25 of its own and it is a THIRD kind of thing — a bound on what Airtable is ASKED for, passed into `searchMaterials`, which is why that screen has no second page and says `Add another word to narrow the search` instead. Sharing it with this one would make a decision about how tall a table is change how much of the base a search reads.
  - **AND THAT IS WHY THE TOOLS BRIEF WRITES ITS NUMBER AND THESE FOUR DO NOT.** `docs/briefs/tools-toolRecordId.md` says how many printed ids a page holds in a sentence — ten when this was written, twenty-five since #442 — and it is right to: that screen prints a label sheet for the tool items on the page, so how many are on it is part of what the screen DOES and a reader of the brief needs it to understand the action. Twenty-five is how many rows a table shows — a value the design pass owns and will move — so writing it into a brief would put it in a second place to edit and make the brief wrong the day it changes. The difference is whether the figure is a property of the feature or a property of the layout; it is not that one of the two briefs is doing it wrong.
- **A PAGE PAST THE END RESOLVES TO THE LAST ONE, AND THE ADDRESS IS REWRITTEN TO THE PAGE ACTUALLY SHOWN.** A URL is typed, edited and copied, so the parameter is a request rather than a promise. `pageOfRows` clamps, which is `pageOfToolItems`' rule; what is new here is the second half, because clamping alone leaves `?page=99` in the address bar over a screen showing page 2, and a reader who copies that link hands on a number that means nothing. `useListFilters` mirrors the RESOLVED page rather than the asked-for one, so the two agree within the mirror's own delay. **The tools screen does not do this** — its clamp is a Server Component's and rewriting there means a `redirect()` rather than a `router.replace`, which is a different mechanism on a different axis; `backlog.md` carries it.
- **EVERY NARROWING RETURNS TO THE FIRST PAGE, AND THE FIRST PAGE HAS NO PARAMETER.** Page 3 of a job's orders is a row offset into a set of rows the reader has just replaced, so every setter in the hook puts the page back to 1. The address follows for free: `listQuery` appends the page only above 1, which is #324's own convention that absence is the unapplied state — so a bare URL, a cleared one and `?page=1` are one string rather than three.
- **`Previous` AND `Next` NOW EXIST TWICE, AND THE CONDITION FOR MERGING THEM IS WRITTEN HERE RATHER THAN LEFT TO TASTE.** `lib/toolListView.js` has held both words since #339 and `lib/listFilters.js` has them now, which is two literals for one control on two axes. They are not merged because there is no module either axis owns that could hold them: `toolListView` is the tools axis's copy and `listFilters` is the document lists', and importing one from the other would make a tools screen depend on the document lists' vocabulary for the sake of eight characters. **The condition that would change that is a THIRD screen wanting them**, at which point the shared home is a paging module of its own and both axes become callers — the move `countsAsOrdered` made at its third consumer (#169). Until then the duplication is two words with no judgment behind them, and the cost of the wrong shared module is higher than the cost of the copy. What IS shared is the rule, and that is `pageOfRows` against `pageOfToolItems`: two functions, because one divides ids before a read and the other divides rows after one, which the tools module's own note settled before this issue existed. The sweep in `offline/list-filters.mjs` is scoped to the four clients and the two shared components for exactly this reason — repository-wide it would report `toolListView.js` as a stray.
- **THE FOOT SAYS TWO THINGS AND NEITHER IS THE COUNT IN THE BAR.** `N of M` beside `Clear all filters` is the FILTER's arithmetic and renders only while something is active. The foot is about the LIST: how many rows it holds and which slice is on screen, stated unconditionally, because the reader who has filtered nothing is exactly the one with nothing else on screen telling them whether they are seeing everything. **Its total counts the rows after the bar** — what is actually being paged — so the two figures differ while a filter is on and agree when it is off. Seen in a browser: `39 of 41` in the bar over `39 purchase requests · Page 1 of 2` at the foot.

### The rest of the half-cent premise (#405)

`HEADER_TOLERANCE` is half a cent because both sides of the comparison are whole numbers of cents, and from #254 to this issue that sentence was enforced for one of the five figures it is about. `Calculated Total` is `SUM(Items Subtotal, Shipping Fee, Tariff, Sales Tax)`; #254 held `Items Subtotal` through the item writers and the item loop in both actions, and nothing asked the three terms summed beside it, or `Amount Due` on the other side. Both invoice actions refuse all four now, in a sentence naming the figure, and both writers in `lib/airtable/invoices.js` throw.

- **THE HOLE WAS NOT A ROUNDING NUISANCE, it was the tolerance ceasing to mean what its own docstring says.** Half a cent is chosen to sit one unit below the smallest difference the currency can express, so `> 0.005` separates a real cent from binary slop. Put `1.005` in a shipping fee and `|100.004 - 100.00|` is 0.004 — silent, on a genuine disagreement — while `0.006` fires on a difference no invoice can actually state. The mark stops being either a true positive or a true negative; it becomes a report about the third decimal place.
- **`Amount Due` IS IN THOUGH IT IS NOT A TERM OF THE SUM, and that is what the derivation asks for rather than a widening.** #254's own words are "both sides of the comparison are whole numbers of cents". The terms are one side. Holding only them moves the hole one step rather than closing it, and closing the terms while leaving the figure they are compared against would have made this file's claim false in a new way. It costs nothing real: a vendor's printed total is in cents, so the refusal catches a typo and never a document.
- **FOUR SENTENCES IN ONE CONSTANT, WHICH IS `ITEM_PRECISION_COPY`'s SHAPE AND #330's TEST.** That test asks whether two refusals are really about one fact. Here the fact is one — `isWholeCentPrice` decides all four — and only the subject differs, which is the case for one constant keyed by subject. `HEADER_PRECISION_COPY.shippingFee` IS `SHIPPING_FEE_PRECISION_COPY`, the same binding rather than a copy: #308 named that constant for the figure instead of for the request's screen precisely so this issue would import it, and said so in its docstring. The field is `Shipping Fee` on a request and on an invoice alike.
- **THE STATED TOTAL'S SENTENCE SAYS `Amount Due` THOUGH `/invoices/new` LABELS THAT CONTROL `Vendor's Stated Total`.** Both actions already answer `Amount Due is required.` on that screen, so a refusal naming the label would disagree with the refusal directly above it; and the edit screen's own label is `Amount Due (vendor's stated total)`, which is both names in one place. The brief for each screen records it, because a design that renames one has to rename the other.
- **THE ORDER OF THE REFUSALS IS ONE DECISION AND NEITHER SCREEN'S LAYOUT.** `headerPrecisionRefusal` reports the sum's terms in the order `Calculated Total` sums them and then the other side, which is the same answer whichever action asked. Taking a screen's control order would have made the other screen's arbitrary: `/invoices/new` puts the stated total last and the edit screen puts it first.
- **THE READER THIS INVENTS IS NOT #308's, AND THE EDIT SCREEN IS WHERE THEY APPEAR.** That issue's edit turn refused over a row the person had just typed. Here every save from `/invoices/[invoiceId]/edit` sends all four header figures, so an invoice that reached the base carrying an off-cent `Tariff` — which only a hand edit in the Airtable UI can now produce — is refused on an edit that changed only the vendor. **Verified rather than reasoned about** (`verify-invoice-header-precision-405.mjs`, section D): the `Tariff` control renders on that screen carrying `value="1.005"` under a label reading `Tariff`, the refusal names `Tariff` and neither of the boxes the reader was changing, the refusal writes nothing at all, and correcting `Tariff` lets the same vendor change land. So the sentence is an instruction rather than a dead end — which is the property that makes checking every submitted figure the right trade here, as it was for the edit turn.
  - **THE ALTERNATIVE WAS CHECKING ONLY WHAT CHANGED, and it needs a before-and-after the action does not have.** It would also let a save write the header while leaving a term of the total off the cent, which is the state this issue exists to remove.
  - **THE BROWSER IS NOT A FIRST LINE OF DEFENSE ON EITHER SCREEN, and working out why took finding the mechanism rather than observing the two screens disagree.** All eight controls carry `step="0.01"` and no `min`, and neither form sets `noValidate`, `formNoValidate`, `step="any"` or an `onSubmit` — grepped. What decides it is the STEP BASE: for `input type=number` HTML takes it from `min` when present and **from the `value` CONTENT ATTRIBUTE otherwise**, so with no `min` the constraint asks whether a figure is a whole number of cents *away from whatever that attribute holds* rather than away from zero.
    - **`/invoices/new` binds these four to React state, which keeps that attribute equal to the current value** — measured, it goes from `""` to `"1.005"` the moment the value does. The base follows the figure, so the figure always sits on it and `checkValidity()` is true for anything typed. The constraint is vacuous there.
    - **The edit screen renders them with `defaultValue`, so the attribute is whatever the record loaded with and never moves** — measured, it stays `"0"` however the property changes. **Proved rather than inferred:** giving that same control a `min="0.005"` with the value and the step untouched makes the mismatch disappear, and removing it brings the mismatch back.
  - **WHICH MEANS THE EDIT SCREEN'S CONSTRAINT INVERTS ON EXACTLY THE RECORD THIS ISSUE IS ABOUT.** An invoice storing `Tariff` of `1.005` renders that as the content attribute, so the base becomes `1.005`, and the browser then **accepts `1.005` and `2.005` and refuses `1.01`, `1.00` and `12.34`** — measured, all five. It admits the off-cent figure and blocks every repair of it. The earlier reading of this, that the browser catches the bad value and focuses the right box, is true only while the stored figure is itself on the cent, which is the case that needs no catching. `docs/notes/backlog.md` carries what to do about the controls, because whether these four may hold a negative is a product question this issue did not ask.
  - **SO THE APP'S REFUSAL IS THE ONLY CORRECT CHECK ON BOTH SCREENS**, rather than the backstop behind a working control. On the create path nothing else fires at all; on the edit path what else fires is wrong in both directions on the one record that matters. And a Server Action is directly callable whatever the page rendered — CLAUDE.md's re-authorization rule is the same fact one axis over — so the action is where this has to be regardless of what any control does.
- **TWO LAYERS, NOT #308's THREE, AND THE MISSING ONE IS THE FREEZE.** That issue's third layer was `createPOItem`/`createPO` writing a request's figures into a document about to be emailed to a vendor. An invoice is snapshotted into nothing, `Calculated Total` is an Airtable formula, and the other two writers on `Invoices` (`setInvoiceDelivery`, `linkInvoiceToPO`) carry no figures. So the layers are the actions with words and the two service writers with a throw.
  - **THE ISSUE BODY SAYS `lib/airtable/invoices.js` THROWS AND THERE ARE TWO FUNCTIONS IN IT THAT DO.** `createInvoice` writes all four; `updateInvoice` writes any subset, and `isWholeCentPrice`'s abstention on `undefined` is what lets one guard serve a partial update — the callers that write only `paidDate` or only `varianceFlag` are asking about nothing and are refused nothing.
- **A SIDE EFFECT WORTH NAMING: `Tariff` and `Sales Tax` could be written as `NaN`.** Both actions coerce with `tariff ? parseFloat(tariff) : null`, so a non-numeric value posted directly became `NaN` on the record. `isWholeCentPrice(NaN)` is false — `Number.isFinite` is the first half of it — so that now meets a sentence instead. Not the issue's subject, and not a state the `type="number"` control can reach, but the guard closes it on the way past.
- **THE BASE NEEDED NO REPAIR, COUNTED RATHER THAN ASSUMED.** Of 22 invoices: `Amount Due` set on 22, `Shipping Fee` on 22, `Tariff` on 2, `Sales Tax` on 2, and **none of the 48 values is off the cent**. #308's census said the same of the three terms and this issue re-ran it over all four. No seed or verification fixture writes a sub-cent figure either — `61.05`, `118.8`, `866.05` and `1810.8` are the only non-integers among them and all four are whole cents — so nothing that was green before this guard is refused by it.
- **THE CHECK GOES IN `offline/invoice-header-tolerance.mjs` RATHER THAN A NEW FILE**, on that file's own stated rule: a threshold and the thing that makes it correct should fail together. It already asserted the item half of this premise at both levels, so the header half sits beside it. **The figures are asserted BY NAME at both levels**, because the mutant is dropping one of the four rather than all of them — `Tariff` carries a value on 2 of 22 invoices, so a count reading three is what nobody would notice.

### Giving each charge in the order breakdown its ordered quantity (#408)

Each line under an order on `/invoices/[invoiceId]` carried the quantity this invoice charged against that order and nothing beside it, so `5 EA` read the same whether the order asked for 5 or for 500. The line now carries that quantity over the ordered quantity of the `PO Items` rows it charges on that order. The rule, the sum and the words are `lib/invoiceOrderBreakdown.js`.

- **THE DENOMINATOR WAS ALREADY IN THE PAGE'S HAND, MEASURED RATHER THAN ESTIMATED.** `getInvoiceReconciliation` calls `getPOItemsForReconciliation` over every invoice item that names an ordered item — the same set `ordersNamedByFoldedItem` walks — and that reader already maps `Qty`, which #232 used and this module now needs. `findByRecordIds` passes no `fields`, so the whole record was already coming back: **zero extra Airtable operations and zero extra bytes.** What was missing was only that the level was read and thrown away, which is `orderedItemsByInvoice`'s shape from #256 one function along, and the reason that shape exists — a function that does not hand back what it read makes the next caller read it again.
- **IT TRAVELS AS A MAP AND NOT AS A FIELD ON THE RECONCILIATION ROW, AND THAT IS THE DECISION.** #232 took `ordered` OFF those rows deliberately: nothing on the delivery side of this screen states what was ordered, and `/pos/[poId]` answers it. `lib/invoiceDeliveryEntries.js` reads the same rows, so putting the figure back on one would place it in front of the section that was cleared of it — a later editor would find it there with nothing saying why not. `orderedQtyByOrderedItem` goes to `chargesByOrder` and to nothing else.
- **THE SUM IS OVER DISTINCT ORDERED ITEMS, AND THE SHAPE THAT NEEDS IT IS REACHABLE THROUGH THE APP.** `lib/prItemMerge.js:mergeKey` holds `Remark` and the quotation; `lib/invoiceItemFold.js:foldKey` holds neither. So two request items differing only in their remark survive the merge as two `PR Items`, `lib/poGeneration.js` snapshots them one-for-one into two `PO Items` on one order, #18's cache gives both the same `Materials` row, and the price is equal because it is part of the merge key — one folded invoice item then covers both. The fold is strictly the coarser unit. **Run rather than argued** (`verify-order-breakdown-408.mjs`, section A): `mergeIdenticalItems` is asked, its output is what gets written, and the two ordered items are read back off the base sharing one material and one price. The control runs in the same section — two rows agreeing on everything merge to one — because a merge that never merges would make the observation vacuous.
- **BOTH FIGURES ARE ACCUMULATED IN ONE LOOP UNDER ONE TEST, WHICH IS AN INVARIANT AND NOT AN ARRANGEMENT.** A row with no `PO Item` leaves the numerator and the denominator in the same pass. Written as two walks, one could admit a row the other refused and the line would read `7 of 15` with the two figures counted over different sets. That row still says nothing of its own, which is the silence `lib/deliveryReconciliation.js` and `lib/poDocuments.js` both keep for the same row: #278 removed the one sentence this app had about a charge with no ordered item behind it, and the numerator has omitted such a row since — this issue puts a whole beside a part that was already partial rather than creating the gap.
- **THE UNIT IS WRITTEN ONCE, AFTER BOTH FIGURES.** `Invoice Items."Unit"` is a frozen copy of the linked `PO Items."Unit"`, so the two figures are in one measure by construction and a disagreement means the wrong ordered item was picked — a different fact with its own home. `5 EA of 10 EA` would assert two measurements that happen to agree; `5 of 10 EA` states one measure with a part and a whole on it, which is `FILTER_BAR_COPY.count`'s shape for the same relation. Pinned by COUNTING the unit in `offline/invoice-order-breakdown.mjs` rather than by matching the sentence, because the two-unit form contains the one-unit form's terms.
- **`of` CARRIES THE SCOPE, WHICH IS WHAT `docs/briefs/_shared.md` ASKS OF A LINE HOLDING TWO OF THEM.** The numerator is this invoice's and the denominator is the order's. `N of M` puts the part inside the whole, so which figure belongs to which document is read rather than counted, and no label is needed to say it. **What the line deliberately does not say** is what is left to invoice: a sibling invoice may already have charged the rest, `PO Items."Invoiced Qty"` is the figure that would answer it, and that is the order's cross-document total and `/pos/[poId]`'s.
- **AN UNRESOLVED WHOLE IS ABSENT RATHER THAN ZERO.** A charge whose ordered items yield no numeric `Qty` renders exactly as the line rendered before this issue. A whole of `0` is a figure and is printed; `null` is not. On this base neither is reachable — 0 invoice items have an empty `PO Item` and 0 ordered items have a blank `Qty`, counted 2026-09-18 — so this is a crash guard on a hand edit, in the shape #278 left the rest of the module.
- **THE BASE COULD NOT SHOW THE FEATURE, WHICH IS WHY THIS ISSUE HAS A CREDENTIALED SCRIPT AT ALL.** Counted 2026-09-18 over 22 invoices, 24 invoice items and 39 ordered items: exactly ONE invoice turns the list on (`HYE-INV-260911-12`), and its three lines read `60 of 60`, `25 of 25` and `120 of 120` — every denominator equal to its numerator, so a screen with the feature and a screen without it are indistinguishable there. No folded item anywhere covers two ordered items on one order. `verify-order-breakdown-408.mjs` creates a partial charge and a summed whole, reads them back through the production path, **fetches the rendered page and asserts both lines out of the HTML**, then deletes everything inside the same run — it reuses one order and one of its ordered items and writes on neither, and no path through it leaves rows on the base while a person decides something.

### Showing the order breakdown when the charges are split differently (#409)

`chargesByOrder` decided whether the breakdown appears by comparing the SET of orders each folded item named, so an invoice whose items all named the same two orders stayed silent however unevenly they were divided between them. The comparison now takes each item's SPLIT — the orders it names and the proportions its quantity divided among them. The key is `lib/invoiceOrderBreakdown.js:splitSignature`.

- **THIS IS NOT #232 AND #233 BEING UNDONE, AND THE DISTINCTION IS WHERE THE ISSUE STANDS.** What those two removed is A SINGLE ANSWER RESTATED ONCE PER ITEM — an invoice fact printed once per row it touched, a delivery named once per item of the invoice that matched it. Items that divided themselves differently across the same orders are not a single answer: one went 10 and 3, the other 2 and 11, and nothing else on the screen holds either figure. The repetition rule is untouched; what changed is which invoices fall under it.
- **#408 HAD TO LAND FIRST, AND NOT FOR A MECHANICAL REASON.** While a line carried a bare quantity, two lines under one order carrying different quantities really were one answer restated — the quantity is in the items table directly above. The denominator is what makes them say different things, so the ground for silencing them went with it. An issue ordering that looks like a dependency is a dependency of the ARGUMENT here rather than of the code.
- **THE REDUCTION IS BY THE GREATEST COMMON DIVISOR, AND WHAT IT BUYS IS THE ABSENCE OF A CLAUSE.** A one-order item's vector has one component and reduces to `1` whatever the quantity, so a one-order invoice is silent by construction. The raw per-order quantity does not have that property — `A:10` beside `A:20` differs — and an invoice whose every item sits on one order would then draw a list repeating the items table exactly. The five cases (one order; one item split; all split alike; different orders; same orders split differently) fall out of one comparison with no case for a corrective order, which is the same property the set-only rule had and the reason to keep the shape.
- **NO FLOAT REACHES THE KEY, WHICH IS A FACT ABOUT THE BASE.** `isWholeQty` is `Number.isInteger` and `assertWholeQty` guards every writer of a quantity into an item table — `createItem`/`updateItem`, `createPOItem`, `createInvoiceItem`/`updateInvoiceItem` — so quantities are integers, their sums are integers, and the reduction is integer arithmetic end to end. Counted 2026-09-18: 0 non-integer quantities across 24 invoice items and 39 ordered items. A hand-edited fraction is compared UNREDUCED rather than rounded, which can only tell two items apart and never merge them — and telling them apart turns the list on, which states a fact rather than hiding one. No tolerance and no chosen precision anywhere.
- **TWO INVARIANTS, MEASURED RATHER THAN ARGUED, AND BOTH HELD IN `offline/invoice-order-breakdown.mjs`.** Over every pair of integer vectors up to (12,12) — 28,561 pairs — reduced-equality and proportionality agree exactly, so the reduction merges the proportional class and nothing else; there is no accidental collision to find. And raw-equal never becomes reduced-different, so **the reduction only ever merges**. Separately, the order ids stay in the key, so two items whose sets differ cannot sign alike: **nothing listed before this issue goes silent after it.**
- **WHERE THE PREMISE BREAKS, NAMED SO THE NEXT PERSON DOES NOT HAVE TO FIND IT.** `same ratio, one answer` is not the same claim as `same line`, and the gap is the denominator. Two items each split 2:1 across A and B, one ordered 10 and 100 and the other 1000 and 10, read `10 of 10` and `5 of 100` against `20 of 1000` and `10 of 10` — one exhausted A, the other exhausted B, and this rule is silent. That is ordinary data rather than a corner, since two materials on two orders carry their own ordered quantities. **The condition for revisiting is a reader shown to need `which order is used up` on THIS screen**, and the answer would then be a figure rather than a trigger: the honest one is `PO Items."Invoiced Qty"` across every invoice, which is `/pos/[poId]`'s.
- **THE DENOMINATOR WAS CONSIDERED FOR THE KEY AND REFUSED, AND THE DECISIVE TEST WAS A ONE-ORDER INVOICE.** Signing `5 of 5` apart from `5 of 50` is a real argument — those lines are not one answer. It fails because it signs them apart ON ONE ORDER too: two items of 5 against ordered quantities of 5 and 50, charging a single order, would draw a list that repeats the items table line for line, and #409's own sentence is that an invoice charging one order stays silent. It is also the wrong KIND of fact — an ordered quantity belongs to the order and is identical for every invoice charging it, so it says nothing about how THIS invoice divided anything. **If it is proposed again, run it against a one-order invoice**; the proposal is built and run as the third mutant in `offline/invoice-order-breakdown.mjs` so the refusal is executable rather than a paragraph.
  - **ONE REACHABLE CASE ESCAPES THE ONE-ORDER SENTENCE, AND THE REPAIR BELONGS TO THE VALUE RATHER THAN TO THIS JUDGMENT.** A zero-quantity invoice item signs `A:0` where a real charge signs `A:1`, so a one-order invoice mixing the two draws a list. The qty control is `required` with no `min`, so a typed `0` reaches the base; there are none on it (counted 2026-09-18). Both repairs INSIDE the key were worked out and both NARROW the judgment somewhere else — dropping zero shares from the key silences an item that touched an order its neighbour did not, and dropping zero-total items silences an item that named an order alone — and this issue only widens, so neither was taken. **What closes it with no change to the judgment is refusing the quantity**, which is `docs/notes/backlog.md`'s entry beside #405's `min` finding: `isWholeQty` admits `0` and it is the only test any quantity meets. Whoever fixes the value should know this key follows it — a `0` that cannot be written is a signature that cannot read `A:0` — and should not reach for the two narrowing repairs above.
- **THE BASE COULD NOT SHOW ANY OF IT, WHICH IS WHY THERE IS A SECOND CREDENTIALED SCRIPT.** Counted 2026-09-18: one invoice charges more than one order and it is listed already because its order sets differ, so this issue turns nothing on that was not already on; and no invoice has two folded items on one order with different quantities, so even the raw-versus-reduced distinction is unobservable. `verify-order-breakdown-409.mjs` builds three invoices through the app's own paths — one listed, two silent — reads the judgment back through the production path, fetches all three pages and asserts the exact lines present on one and ABSENT on the other two, then deletes everything inside the same run. The two silent pages are asserted to still name their orders, because a page that failed to render would satisfy every absence on its own.

### Drawing the invoice file beside the form (#422)

Every field on `/invoices/new` transcribes something printed on the vendor's document, and the document was not on the screen: it was attached, read for an order number, and then stood as a filename while the reader worked from a second window. The file has its own column now, to the left of the form — a box that takes the file and then holds it. The rendering is `app/components/FileFrame.js`, which is #331's frame moved out of `FileViewer.js` unchanged; the classes, the breakpoint and the box's one sentence are `app/invoices/new/filePane.js`, whose header carries the arithmetic.

- **THE FILE IS IN TWO PLACES BEFORE SUBMIT AND THE PANE USES THE CHEAPER ONE.** `handleInvoiceFileChange` has the browser's own `File` and uploads it to Vercel Blob in the same handler, so by the time anything renders there is a public Blob URL as well. The pane draws `URL.createObjectURL(file)`: it costs no request, it is ready before the upload finishes rather than a second later, and it dies with the document instead of being an address that exists. The Blob URL would also have brought #140's measured naming defect along — Blob's own `Content-Disposition` carries the random suffix — which is why `uploads-and-drafts.md` already keeps these three pre-save previews off the file route.
- **`/api/files` IS NOT AN OPTION AND THAT IS STRUCTURAL.** The route takes `{axis, documentId}` and re-reads the record per request, gating the invoice axis on `getVisibleInvoiceIds`. There is no `Invoices` row until this form is submitted, so there is no address to build and nothing for a gate to answer about. **No new route was added either**, which is what keeps this issue out of the authorization surface entirely: an object URL is reachable only from the document that minted it.
- **THE FRAME MOVED RATHER THAN BEING COPIED.** How a file is drawn — an image for a photograph, a frame for a document, neither for a type that would open a download dialog, and a sentence under the frame because nothing can detect that a document failed to render — is one rule, and it now has two callers. The move was diffed character for character and the body is byte-identical; the export sits below the declaration so it could stay that way. `offline/file-route.mjs` follows it: the clause that pinned `documentHint` inside the viewer now reads the frame's own module AND requires the viewer to render `FileFrame`, so neither half can pass while a reader sees nothing and is told nothing about it. **#433 drew a PDF in the app rather than in a frame, so the standing sentence is gone** and the check pins each failure sentence where its failure is seen instead — `uploads-and-drafts.md`, "Drawing a file".
- **THE PANE HAS NO CONTROLS, AND THE DOWNLOAD CHECK IS WHAT SAYS SO OUT LOUD.** `offline/file-route.mjs` holds `download` to exactly one attribute in `app/`, in the viewer. That is the right answer here for a reason of its own rather than by deference: the reader picked this file off their own disk seconds ago, so there is nothing to save. The same goes for a title — the file section names the file a few centimetres to the left. **Since #433 the pane has a toolbar** — the page in view, the zoom, `Fit width` and `Rotate` — and still no title and no download, for the same reasons.
- **A TYPE THE FRAME WILL NOT DRAW GETS NO PANE, WHERE THE VIEWER GETS A SENTENCE.** `notViewable` exists because a viewer somebody opened may not open empty. Nobody opens the pane, so it owes nothing, and half a screen of empty column carrying `download it to open it` would answer a question the reader did not ask. The file control offers PDF, JPEG and PNG, so this is a state a reader arrives at rather than one the screen offers.
- **THE BREAKPOINT IS `xl` AND THE COLUMN IS `38rem` BECAUSE THE FIRST ARITHMETIC WAS WRONG.** 1280 leaves 1216 inside the page's `p-8`, and 42rem to the form looked right because the container is `max-w-2xl`. Measured in a browser, the form is **608px** at 1279 — the 42rem cap less that same padding — so a 42rem column put the padding outside the form and handed it 672: crossing the breakpoint widened every field by 64px. At `38rem` the form measures 608 on both sides of the boundary and nothing inside it reflows; the pane gets 576, measured at 561 with the scrollbar this form always has — at 1280 the pane sits at x=32 and the form at x=625, and at 1440 the pane widens to 721 and the form moves to x=785. At `lg` the same subtraction leaves 320, which is not a document.
- **THE PAGE WIDENED THROUGH `:has()` WHILE THE COLUMN WAS CONDITIONAL, AND THAT WENT WITH THE CONDITION.** The container is in `page.js`, a Server Component that also renders the heading and the direct-purchase notice, while whether a file is attached is client state inside `InvoiceForm` — so the container carried `xl:has-[[data-invoice-file-pane]]:max-w-[96rem]`, one class reading the pane's own data attribute, rather than the heading being moved into the client to let one component hold both. Worth keeping in this record because the trick is the answer whenever a server-rendered container has to follow client state: the attribute and the selector are one string that cannot be bound as one, since a Tailwind class is matched in the source as a literal, so they belong on adjacent lines with a comment saying so. Here the box made the column unconditional and the container is simply wider at that width.
- **THE DOCUMENT IS THE LEFT COLUMN AND THE FORM THE RIGHT, AND THE KEYBOARD ORDER IS THE OTHER WAY ROUND.** The sides decide nothing about the widths — the form keeps its 38rem and the pane takes what is left, either way — so what they decide is reading order: the source on the left, the transcription of it on the right. The FORM still comes first in the document and the pane is placed into the first column by hand, which is a deliberate mismatch between visual position and focus position. Reordering the elements instead would need no placement classes at all, and it would put a PDF frame between the reader and every field of the longest form in the app: a browser's own viewer is several focusable controls rather than one stop, and the pane has nothing to fill in. So the pane is first in the order things are read and last in the order they are reached, and the one element whose two positions disagree is the one nobody acts on.
- **THE COLUMN STARTED OUT CONDITIONAL, AND WHAT CLOSED THAT IS MAKING IT THE CONTROL.** It was drawn when a file was attached, which is what `_shared.md`'s rule about not reserving space for a conditional asks for — and the cost was that the page widened under the reader at the moment of attaching, moving the form sideways: measured at 1440, from x=409 to x=785. Two repairs were written down and neither was taken, because the third one was better than both: **the box is the file control now**, standing empty at the document's exact size and taking a file dropped on it or opening the dialog when clicked. An empty control is not a reserved space — it is the normal state of a control — so the rule is satisfied rather than traded against, the columns are drawn from the first paint, and the form never moves at all. The conditional thing is what the box HOLDS.
- **THE BOX AND THE DOCUMENT ARE ONE RECT, AND THE FIRST CUT WAS 24PX OFF.** The empty box carries `FileFrame`'s own `mt-4 min-h-0 flex-1`, and that was taken to be enough: measured at 1440 it was not — the box 792 tall and the PDF frame that replaced it 768, the difference being the standing sentence `FileFrame` puts under a document inside the same column. An image has no such line and was drawn at 792. It was left and written down as the cost of not editing a shared frame, and #422's body then fixed the size as part of what the issue is: an empty box at the document's size, and the document in the same place at the same size. **The repair is a slot in the pane rather than an edit to the frame.** `FILE_SLOT` holds whatever is drawn, and in the two states that have no sentence — the empty box and an image — it keeps the sentence's line empty with `pb-6`, which is `mt-2` plus a `text-xs` line, 24px. Measured afterward at 1440 the box, the PDF frame and the image's container are each top 80, left 32, 721 × 768, and at 1280 — the narrowest pane the breakpoint allows — each is 561 × 768 with the sentence at 16px, one line. **What would reopen it is the sentence wrapping** at the narrowest pane, which a type change could do; the module header says the fix then belongs in the slot and not in the frame six other surfaces share. **#433 removed the sentence and put the pane's toolbar under the file**, so the reserved row is the toolbar's now: `FILE_SLOT_TOOLBAR_ROOM`, `pb-11`.
- **THE BOX STOPS TAKING DROPS ONCE IT HOLDS A DOCUMENT, AND THE FILE CONTROL UNDER IT IS THE ANSWER.** A frame is another document: a file dropped on it goes to the browser's own viewer and nothing in the page can intercept it. So the control sits under the box in every state rather than appearing with the file — which is also part of what keeps the box one size, since a control that appeared would take its line out of the box the moment a document arrived. It is the browser's own file input, which means no second way to replace a file and no word coined for one. The drop handler is on the column rather than on the box, so a drop outside the frame still reaches the form — observed with a drop dispatched on the column while a PDF was showing, which replaced it with the dropped image. **The first half is untrue since #433**: the file is drawn in this document now, so a drop on it reaches the column's handler and replaces it. The control stays under the column for the other reasons given.
- **ONE CONTROL, TWO PLACES IT IS DRAWN, AND ONE FUNCTION EVERY FILE ARRIVES AT.** Below the breakpoint the input is in the `Invoice File` section where it has always been; above it, under the box. Only one of the two is ever visible, each inside something the other width hides. All three ways in — either input, or a drop — call `acceptFile`, so the size guard, the preview, the upload and the detection are one sequence with three doors rather than three sequences.
- **A DROP ASSIGNS ITS `FileList` ONTO THAT INPUT.** Without it the control beside a dropped document reads `No file chosen` while the section opposite says the file is uploaded — two controls on one screen disagreeing about what is attached. The input carries no `name`, so nothing is submitted from it either way; the form's own hidden fields carry the url. **That rule had two gaps, closed on #433's branch, and one function now covers all three cases** — `showHeldFile`, which puts the file the form holds on both inputs:
  - **A canceled dialog.** Reopening the control over an attached file and dismissing the dialog left the pane and `Uploaded …` describing the file while the control read `No file chosen`: Chromium empties the input and fires `change` with no file, and the handler returned. The form's state was right and the control was wrong, so the control is what is corrected. **The control below the breakpoint did the same, and did it before #422** — the handler returned on an empty list then too, with `Uploaded …` already beside it.
  - **The control the other width hides.** A pick goes through one input only, so the hidden one kept naming an older file or none — measured: after a pick below the breakpoint and a drop above it, the hidden control still named the first file, which is what a reader would see the moment the window crossed 1280px. Every accepted file now goes on both, which is what makes `renderFileInput`'s `nothing here for the two to disagree about` true rather than false.
  - **Why handing a file back is safe inside the handler, measured rather than assumed.** Assigning `files` from script dispatched neither `change` nor `input`, so nothing is uploaded again and detection does not run again: the upload and detection counts stayed where they were across every restore, and the only events were the ones the cancel itself fired.
  - **The form holds the `File`, never a `FileList`.** In Chromium a control keeps the very list it is given — after `input.files = list`, `input.files === list` — and emptying the control empties that list in place, the `DataTransfer` it came from included. A list held and handed out once would be empty at the second cancel, so `fileListOf` builds a new one for every hand-off.
  - **What was seen, and where.** Chromium only, with the cancel reproduced by emptying the input and dispatching the `input` and `change` a cancel fires, since a native dialog cannot be dismissed from a script; a real dismissal was not driven. Whether Firefox or Safari empties the input on a cancel was not looked at; in a browser that keeps the file, no empty `change` arrives and nothing here runs.
- **WHAT THE SECTION KEEPS AT A WIDE VIEWPORT IS WHAT IT SAYS, NOT WHAT IT DOES.** The heading, the line saying why a file is required, the upload's own state and the detection message stay in the text column. That is not only placement: the detection message runs to three lines and changes length, so under the box it would resize the box every time a file was attached, which is the one thing the reserved box exists to prevent.
- **UNDER BOTH TABS, AND THAT FOLLOWS FROM WHAT A TAB IS HERE.** The tab reorders four blocks and does nothing else, which `docs/briefs/invoices-new.md` calls the outermost structure of the screen. A pane that came and went with the tab would make the tab mean more than order, and would take the document away mid-transcription from anyone who switched. A file attached last on `Manual Entry` gets a pane it has less use for; that is the cost of there being one rule, and it was weighed rather than overlooked. **#327 removed the tabs**, so the question no longer arises: the form has one order, the file section first, and the pane never followed the tab.
- **THE PREVIEW RIDES ON `invoiceFile` SO THE PANE AND THE FILE SECTION CANNOT DISAGREE.** One state object, so every path that replaces it carries both the address and the content type forward or drops both. The split that falls out: a file refused for its size leaves no pane, because `refuseOversizeUpload` throws before the address is minted and the form is then holding nothing it could attach; an upload that fails over the network keeps it, because the file is fine and the retry is to pick it again. The address is minted as the second statement inside the try, after the guard `offline/upload-limit.mjs` pins as the first.
- **ONE OBJECT URL AT A TIME.** A replacement revokes the previous address in the handler before anything else, and an unmount effect revokes the last one. Verified in the browser: after a swap, `fetch` of the old address throws.
- **WHAT THE BROWSER SHOWED, INCLUDING A CONTROL THAT WAS READ WRONG.** A three-page PDF pages inside the pane — the built-in viewer's toolbar reads `1 / 3`, its sidebar holds three thumbnails, and the page text renders. **The first two PDF attempts drew an empty frame, and so did the app's existing file viewer on an invoice's own page**, which was taken for a browser with no PDF support and was in fact a cold plugin: both painted on a later attempt, in the same session and the same browser. The conclusion to carry is about the evidence rather than the frame — a blank frame is exactly the state `documentHint` existed for, and it looks identical whether the browser cannot draw a document or has not drawn it yet. #433 removed both the frame and the sentence.
- **WHAT IT COST THE BLOB STORE.** Five objects, 100,934 bytes, all orphans — a file picked on this form is uploaded whether or not an invoice follows, which is the one orphan source `uploads-and-drafts.md` describes as surviving by design. No Airtable record was created or deleted for this issue.

### One order of blocks on the invoice form (#327)

`/invoices/new` opened on two tabs, `PDF Upload` and `Manual Entry`, that changed nothing but the order of four blocks — the file section, the header fields, `Items` and the totals. The form has one order now, the file first, which is the order `PDF Upload` drew; that was the default tab, so on the screen most readers saw, nothing moved but the row of tabs. What the screen carries is in `docs/briefs/invoices-new.md`; this is what was checked before the tabs went and what their going showed.

- **THE TAB HELD NOTHING BUT THE ORDER, AND THAT WAS COUNTED RATHER THAN TAKEN FROM #46's COMMIT.** `activeTab` was one `useState("pdf")` with two readers: the buttons' styling and the branch that chose the order. No URL parameter carried it — `page.js` reads `recorded` and `job` and nothing else — nothing stored it, and neither `detectAndApplyPOs` nor its `pristine` test read it. So detection applies and backs off exactly as it did, and what a reader can no longer do is ask for the second order.
- **SWITCHING DID LOSE WHAT WAS TYPED, IN ONE FIELD — MEASURED BEFORE THE TABS WENT.** The two branches rendered the same four blocks at different positions and with no keys, so React matched them by position and remounted what was inside them on every switch. A field bound to state came back holding its value; `Due Date`, the one field that keeps its value in the element rather than in state, came back empty — a date typed on either tab was gone after a switch in either direction, while `Vendor Invoice #` kept `V-12345` across the same switch. So the brief's `switching never loses what is typed` was false for that field. Nothing carries forward: with one order there is no switch, and `Due Date` stays uncontrolled.
- **TWO SENTENCES WERE TRUE IN ONLY ONE OF THE TWO ORDERS, AND NEVER IN THE DEFAULT ONE.** Detection's back-off message ended `Select manually above if needed.` (#57), and the direct-purchase modal's vendor refusal read `Pick the vendor at the top of the form first.` (#272). Both put the header above the file section, which is `Manual Entry`'s order — and the default tab had been `PDF Upload` since #46, merged the day before #57. So removing the tabs is not what made them false: each was false on the screen most readers saw, from the day it was written. The tabs let two orders share sentences that were true in only one, and nothing on either tab could say which order a sentence had been written for — which is evidence for what this issue removes, not a side effect of removing it. The first says `below` now; the second is `Pick the vendor on the form first.`, which says where the field is without placing it.
- **THE MODAL'S REFUSALS START WITH THE FILE.** `directPurchaseBlocked` follows the order a reader fixes things in, and the form starts with the file, so its refusals do too: the file, then the vendor, then the Job. It named the vendor first, on a comment that placed the vendor at the top of the form — the same premise as the second sentence above. Observed at 1440: with neither a file nor a vendor the modal says `Attach the vendor's invoice first — the record is that document.`, with a file and no vendor `Pick the vendor on the form first.`, and with both `Pick the job it was bought for.`, `Record it` disabled at each. `createDirectPurchaseAction` asks the same function, so the action's first refusal moved with the modal's.
- **THE CHECK HELD A TAB LABEL AS ITS PROOF OF REACH.** `offline/no-free-text-item.mjs` proved its literal walk read the form by finding `Manual Entry`, so removing the label failed it for a reason unrelated to free-text items. It finds `/api/invoices/detect-po` now — a route, which a design pass does not reword. `Manual Entry` matches nothing among the form's literals on this branch.
- **WHAT EACH WIDTH LOST, MEASURED AT 1440 AND 1024 BEFORE AND AFTER.** The row of tabs went at both, and the `Invoice File` section starts where it stood: its heading at y=88 instead of 151, the vendor field 63px higher. From 1280px up the section carries the upload's state and detection's message and no control, and the column beside the form never followed the tab — the same rect before and after, x=32 and 721 × 836 at 1440. Below 1280px the section holds the control, which is the form's first keyboard stop now where `PDF Upload` was; the arrangement gone at that width is `Manual Entry`'s, which put the control just above the submit button.
- **THE WAY `Manual Entry` WAS USED STILL WORKS, AT BOTH WIDTHS.** Vendor, order, quantity, both dates and the stated total typed first, then the file — dropped on the column at 1440, picked through the section's control at 1024. Every typed value stayed, `Due Date` on the same element; detection backed off as it always has and said `Select manually below if needed.` from above the order picker (y=172 against 268 at 1440, 200 against 296 at 1024); and the submit button read `Create Invoice`. The same file attached first auto-filled the vendor, the order and its first ordered item at both widths, with the message, the fields and the one-operation `detect-po` call the unchanged form gave before any edit.
- **TWO READMES USED THE TABS AS THEIR EXAMPLE.** `docs/briefs/README.md` and `docs/briefs/strings/README.md` each named `/invoices/new` being two tabs as one of the 28 facts #288 moved into the briefs. The example went and the figures stayed, since they are still what #288 measured.
- **WHAT IT COST.** Six Blob objects, 5,981 bytes, all orphans for the reason the #422 section gives above. Airtable was read and not written: nothing was submitted.