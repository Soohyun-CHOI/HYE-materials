...
- **Not in this issue:** the fields inside the control, which are still a checkbox and a date and are a separate issue's to merge into one; the write path, unchanged in every part — `updatePaidAction`, its `withAdminAction` gate, its refusals and its redirect; the heading's own `Edit` link, whose label is a `design-sweep-list.md` question rather than this one's; and the invoice list, which states the same axis in a cell and did not move.
...
- **CLEARING THE DATE IS HOW A PAYMENT IS UN-RECORDED, AND THERE IS NO SECOND CONTROL.** One field, one act — a control for the negative would be a second way to say one thing on a section this issue exists to reduce to one, and `Cancel` beside it already means abandon. What emptying a date input is not is discoverable, so a sentence buys that instead and appears only while there is something in the box to clear: `Clear the date to record that this invoice is not paid.` It asks the FIELD rather than the record, and a check is why — `{paidDate && …}` reads the payment fact inside the `editing` branch, which `offline/invoice-visibility.mjs`'s 3b reports; nothing was hidden by it, but the branch is the same shape as the defect. Asking `date` is also the truer question, the sentence being about the box in front of the reader.
