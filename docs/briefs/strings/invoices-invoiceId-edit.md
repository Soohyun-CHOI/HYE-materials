...
**Counted by the extractor, then read for its conditions.** The two hand-counted
screens are `login.md` and `invoices-new.md`, and their measurements are what say
how much this one can be trusted: on 2,500 lines the extractor missed nothing a
reader sees and found one string the hand pass had missed.
...
5. **a figure inside a counted sentence** — the two previews and every item's
   amount.
6. **a state this pass could not create** — seven, marked `[unreachable]`.

## Strings

### The tab and the heading

- **`{Edit Invoice} · {HYE USA Portal}`** — read · auto
  ...
  - **`Edit`** — read · auto
    - when: always, followed by the invoice's own id, as the page heading
      `[reachable]`
    - from: `app/invoices/[invoiceId]/edit/page.js:52`, JSXText beside a
      container
    - names: `Invoices`
    - held: `_shared.md` lists `Edit {document ID}` among the tier-3 screen
      headings

- **`← Back to invoice`** — read · auto
  - when: always `[reachable]`
...
- **`Not authorized. Editing an invoice is Admin-only.`** — read · auto
  - when: a signed-in reader who is not Admin opens the URL `[reachable]` with
    `authz-fixture@`
...
- **`Invoice not found.`** — read · auto
  - when: the id in the URL matches no invoice `[reachable]`
...
- **`Vendor`** and **`Select a Vendor`** — read · auto
  - when: the label always; the option as the picker's first entry
    `[reachable]`
  - from: `EditInvoiceForm.js:53` and `:63`, JSXText
  - names: `Vendors`
  - held: not quoted

- **`Vendor Invoice # (optional)`** — read · auto
  - when: always `[reachable]`
...
- **`Issue Date`** and **`Due Date (optional)`** — read · auto
  - when: always `[reachable]`
...
- **`The figure printed on the vendor's invoice. Editing it re-checks it against
  our calculated total.`** — read · auto
  - when: always, under the field above `[reachable]`
  - from: `EditInvoiceForm.js:127`, JSXText
  - names: `Invoices`. **`our calculated total`, lowercase**, where the field is
    `Calculated Total`
  - held: not quoted

- **`Shipping Fee`**, **`Tariff (optional)`**, **`Sales Tax (optional)`** — read ·
  auto
  - when: all three always. **This screen shows every term unconditionally**,
    where `/invoices/new` hides two behind reveal controls — so a reader who adds
    a tariff here does it by typing in a field that was already there
    `[reachable]`
  - from: `EditInvoiceForm.js:144`, `:157`, `:171`, JSXText
  - names: `Invoices`
  - held: not quoted

- **`Leave blank if none`** — read · auto
  - when: on both optional currency fields, until one is typed into. **Two
    controls, one string** `[reachable]`
...
- **`Item Name`**, **`Size`**, **`Unit`**, **`Qty`**, **`Unit Price`**,
  **`Remark`** — read · auto
  - when: on every row. `Size` and `Unit` sit on disabled inputs and are
    reference copies; the other four are editable `[reachable]`
...
- **`PO:`** — read · auto
  - when: on every row, followed by that charge's order id, or an em dash when
    the row has none `[reachable]`
...
- **`⚠ Order variance`** — read · auto
  - when: on a row whose stored variance flag is set `[reachable]`
...
- **`Amount (preview):`** and **`Items Subtotal (preview):`** — read · auto
  - when: always, per row and once at the foot `[reachable]`
...
- **`Saving...`** and **`Save changes`** — read · auto
  - when: the first while the action runs, the second otherwise `[reachable]`
...
- **`Cancel`** — read · auto
  - when: always. **A link back to the invoice rather than a button** — nothing
    is discarded, because nothing was staged `[reachable]`
...
- **`Not authorized.`** — read · auto
  - when: `[unreachable]`. The page refuses a non-Admin before the form exists
...
- **`Select a Vendor.`**, **`Issue Date is required.`**, **`Amount Due is
  required.`** — read · auto
  - when: `[unreachable]`. All three controls are `required`, so the browser's
    own words are what a reader meets
...
- **`Every item needs a name, quantity, and unit price.`** — read · auto
  - when: `[unreachable]`. All three are `required` on every row
...
- **`Every charge's quantity has to be a whole number.`** and **`Every charge's
  ...
  - when: a typed quantity is fractional, or a price is not a whole number of
    cents. `[reachable]`, and the two refusals on this screen that are: #254
    measured that the controls' `step` validation does not fire
...
- **`Invoice not found.`** — read · auto
  - when: `[unreachable]` through this screen — the invoice would have to be
    deleted between the page load and the submit
...
- **`Something went wrong updating the invoice. Please try again.`** — read ·
  auto
  - when: `[unreachable]`. Any throw inside the update, which needs Airtable to
    fail
...
- **`on`** — switch · auto — `app/invoices/[invoiceId]/actions.js:49`, the value a
