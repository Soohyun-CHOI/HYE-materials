 ...
 * every unsigned and withdrawn row, and the table's declared widths already
 * spend all 832px the page has, so a further column would have to take its
 * width from Vendor, the one column with nothing to spare. **This said a
 * SEVENTH column and the table has carried eight since #311** — the widths now
 * declare 932px against the page's 832 and scroll inside their own container,
 * so a ninth would widen that scroll rather than wrap; Vendor is still the only
 * place width could come back from.
 *
...
// THE THREE EMPTY STATES AND THEIR JUDGMENT MOVED OUT IN #324, to
// `lib/listFilters.js`, where all four document lists read them. They were written
// here with this page and this was the only list that obeyed the rule they state —
// `/prs` folded "nothing yet" into "nothing for you", and `/deliveries` told a reader
// whose jobs simply hold no delivery that none had been recorded. Neither the
// sentences nor the ordering changed; what changed is that three more lists became
// callers, which is why the judgment could not stay in one list's own view module.
