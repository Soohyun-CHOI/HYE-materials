...
//
// AN INVOICE WITH NO RESOLVABLE ORDER IS REFUSED, not admitted. An invoice item with no
// `PO` link (the hidden free-text option, #96) names no order, so it carries no
// visibility of its own; an invoice whose every invoice item is like that is visible to
...
export async function getVisibleInvoiceIds(user, invoices, invoiceItems) {
    ...
    // (lib/airtable/invoices.js on the join table), so an invoice can bill one
