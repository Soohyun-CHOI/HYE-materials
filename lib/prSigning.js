...
// Airtable calls here — callers fetch PR/PR Signers/PR Edit Requests
// first, pass the plain data in, and act on the plain data returned.
//
// THE ROWS THIS MODULE READS ARE `PR Edit Requests` SINCE #333, AND THE SCREEN
// STILL SAYS `correction`. So the split in this file is deliberate: a name for
// the ROW says edit request, and a name for the ACT — `Return for correction`,
// the arcs drawn for it, `returnForCorrectionAction` — says what the button
// says, because the wording is Design's. `docs/briefs/design-copy-findings.md`
// lists every survivor so that a chosen word is one sweep rather than a hunt.
//
...
// whenever it's 0 there is always exactly one Pending edit request
// with Sent To = the Requester (see computeAdvance below, which relies on
// that invariant).

// #440 — the requester every turn and target below names is `lib/prRequester.js`'s,
// the same one the actions and the page compare a reader against. With its extension:
// this module is offline-safe.
import { requesterOf } from "./prRequester.js";

const REQUESTER_STEP = 0;
...
export function getReturnTargets(pr, signers, currentSequenceOrder) {
    const targets = [{ type: "requester", value: "requester", userId: requesterOf(pr) }];
...
/**
 * Finds the Pending edit request (if any) that the given actor is
 * currently expected to resolve. There can be more than one Pending
 * edit request on a PR at once (a return can itself be returned
 ...
 * one" — Current Signer Step always points at whoever's edit request
 * is the one to resolve right now, so matching by Sent To is exact.
 */
function findPendingEditRequestForActor(editRequests, actorUserId) {
    const pending = editRequests.filter(
 ...
 *     Pending edit request addressed to them), resolve it and jump
 ...
 * module (pr/signers/editRequests only) doesn't have — see
 ...
 *
 * Also returns `withdrawn` (issue #122): true once the PR is Withdrawn.
 * Dropping the correction arcs and having no "current" step already stops
 * a withdrawn bar from reading as mid-flight, but the remaining steps sit
 * in categories ("paused"/"not-reached") that share a neutral color with a
 * not-yet-started chain (per CLAUDE.md), so "ended" isn't legible on its
 * own. SignerProgressBar.js uses this flag to render an explicit terminal
 * "Withdrawn" caption instead of leaving the bar ambiguously neutral.
 */
export function getSignerChainProgress(pr, signers, editRequests) {
    const orderedSigners = [...signers].sort((a, b) => a.sequenceOrder - b.sequenceOrder);

    const requesterStep = {
        type: "requester",
        sequenceOrder: REQUESTER_STEP,
        userId: requesterOf(pr),
        category:
    ...
    // Signed, or Withdrawn per issue #122), any still-Pending edit
    ...
    //
    // Only Pending edit requests get an arc — Resolved ones are
    ...
    //
    // Known ambiguity: PR Edit Requests."Sent To" stores only a user
    ...
    // edit request using pr.currentSignerStep's already-known role, not by
    // re-deriving it from Sent To.
    // #440 — a request naming no requester is the target of no arc. This compared
    // `c.sentTo?.[0] === pr.requester?.[0]`, which is true when BOTH are missing and
    // drew an arc to the requester's step for an edit request sent to nobody. `createPR`
    // always links a requester, so no request the app wrote reaches that case and no
    // screen moves.
    const requesterId = requesterOf(pr);
    const arcs = (pr.status === "In Review" ? editRequests : [])
        ...
        .map((c) => {
            ...
            const isRequesterTarget = !targetSigner && requesterId !== null && c.sentTo?.[0] === requesterId;
            const to = targetSigner ? targetSigner.sequenceOrder : isRequesterTarget ? REQUESTER_STEP : undefined;
            if (to === undefined) return null;

            return { editRequestId: c.id, from: initiatorSigner.sequenceOrder, to };
        })
        .filter(Boolean);

    return { steps, arcs, withdrawn: pr.status === "Withdrawn" };
}

export function computeAdvance({ turn, signers, editRequests }) {
    const pendingEditRequest = findPendingEditRequestForActor(editRequests, turn.userId);

    if (pendingEditRequest) {
        const initiatorId = pendingEditRequest.initiatedBy?.[0];
        ...
        return {
            resolveEditRequestId: pendingEditRequest.id,
    ...
    if (turn.type === "requester") {
        ...
        throw new Error("Requester's turn with no pending edit request to resolve.");
    }
    ...
    if (next) {
        return { resolveEditRequestId: null, nextStep: next.sequenceOrder, prApproved: false };
    }

    return { resolveEditRequestId: null, nextStep: null, prApproved: true };
}
