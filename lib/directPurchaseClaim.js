...
export async function getDirectPurchasesAwaitingRequest(jobs, vendors) {
    ...
    const holderIds = [
        ...new Set(
            waiting
                .map((row) => requesterOf(prById.get(row.purchaseRequest?.[0])))
                .filter(Boolean)
        ),
    ];
    ...
    return sortLongestWaitingFirst(
        waiting.map((row) => {
            ...
            return {
                ...
                notes: row.notes || "",
                // Issue #331 — the file's NAME and TYPE, never its url. The strip
                // builds an href onto this app's own route from the id above, which
                // re-reads the record per request, so nothing here carries a link
                // that can go stale between the render and the click.
                file: row.file?.[0]
                    ? { filename: row.file[0].filename ?? null, type: row.file[0].type ?? null }
                    : null,
                ...
                heldBy:
                    stage === WAIT_STAGE.draft
                        ? WAIT_COPY.draftChip(userName(holderById.get(requesterOf(pr))) || null)
                        : null,
            };
        })
    );
...
export async function describeClaimRefusal(directPurchase) {
    ...
    const holderId = requesterOf(pr);
    const holder = holderId ? (await getUsersByRecordIds([holderId]))[0] : null;
    return DIRECT_PURCHASE_COPY.refused.taken({ holderName: userName(holder) || null });
}
