...
// ordered items means walking Job -> Disciplines -> PRs -> POs -> PO Items, and both
...
//
//   1  Disciplines       (by record id, from the jobs' own Disciplines links)
 ...
 *
 * `jobs` are already-loaded Job objects carrying their `disciplines` link array — which
 ...
 * Item of those Jobs that points at a material. That key was `lines` until #227,
...
export async function getDeliveryCandidates(jobs) {
    ...
    const disciplineIds = (jobs || []).flatMap((j) => j.disciplines || []);
    if (disciplineIds.length === 0) return { orderedItems: [], vendorNameById: new Map() };

    const disciplineRecords = await findByRecordIds(TABLES.DISCIPLINES, disciplineIds);
    // Discipline -> Job, so every ordered item can be attributed back to a Job. Taken
    // from the Discipline's own `Job` link rather than from Purchase Requests' `Job`
    // lookup:
    // the link is core data, the lookup is computed from it.
    const jobByDisciplineId = new Map(
        disciplineRecords.map((r) => [r.id, (r.get("Job") || [])[0] ?? null])
    );

    const prs = await getPRsByRecordIds(
        disciplineRecords.flatMap((r) => r.get("Purchase Requests") || [])
    );
    const jobByPrId = new Map(
        prs.map((pr) => [pr.id, jobByDisciplineId.get(pr.discipline?.[0]) ?? null])
    );
    ...
    const orderedItems = poItems
        .map((item) => {
            ...
            return {
                ...
                // #387 — the address this order was placed to, which the entry form
                // offers as the delivery's own default. FREE: `getPOsByRecordIds`
                // already carries it (#386 put it on `recordToPO`), so the order is
                // in hand here and this adds no query and no round trip. It reaches
                // the browser like everything else on this object and discloses
                // nothing new — the form already renders this order's id and its
                // quantities to the same viewer.
                //
                // NULL IS THE ORDINARY VALUE, NOT A GAP: 34 of the 35 orders on this
                // base record no address, because #386 deliberately backfilled none.
                // `lib/deliveryAddress.js` reads a null as "this order says nothing"
                // rather than as a disagreement.
                deliveryAddressRecordId: po.deliveryAddress?.[0] ?? null,
