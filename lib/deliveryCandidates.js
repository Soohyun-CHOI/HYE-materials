// The read side of #162: every PO line a delivery could be allocated against,
...
// page (to build its dropdowns) and the Server Action (to re-allocate
...
// lines, and that is what makes a single-page form possible at all. Every step
...
//
// THE ITEM LABELS COME FROM THE PO LINES, NOT FROM `Materials`. Every line already
// carries its own Item Name/Size/Unit — a frozen copy of what was ordered — so
// grouping lines by their `Material` link gives both the identity and the label
// without another query. It is also the more faithful label: it is the text on
// the order document the delivery is being matched to.
//
...
/**
 * Every candidate PO line across the given Jobs, in the flat shape planDelivery
 ...
 * query of its own.
 *
 * Returns { lines, vendorNameById }. `lines` is one entry per PO Item of those
 * Jobs that points at a material.
 *
 ...
 * at PO-generation time and PO lines created before it were not backfilled, so an
 * older order is invisible here — as is a line the cache skipped for having no
 * Unit. Matching such a line would mean matching on `Item Name` text, which is
...
export async function getDeliveryCandidates(jobs) {
    ...
    if (lineIds.length === 0) return { lines: [], vendorNameById: new Map() };

    const lineRecords = await findByRecordIds(TABLES.LINES, lineIds);
    // Line -> Job, so every PO line can be attributed back to a Job. Taken from
    ...
    const lines = poItems
        .map((item) => {
            ...
            // A line whose Job could not be resolved is dropped rather than shown
            // under the wrong one: it would otherwise become a candidate for a Job
            // it does not belong to, which is worse than being invisible.
...
// its vendor list from the lines it already holds instead.
