...
// invoicing a purchase order in this job's candidate set", which is free because
...
// also admits a requester, a signer and the recipient of an edit request,
 ...
 * is Job-scoped and an invoice can charge two jobs, so the holder is not always in
 ...
 *
 *   1  PO Items      (the invoiced ordered items — their agreed price, and which
 ...
 *   5+ getInvoiceLinkCandidates (the vendor's invoices, ≤3)
 *
...
export async function getDeliveriesForInvoice(user, { vendorRecordId, orderedItems } = {}) {
    ...
    const charged = [...new Set((orderedItems || []).map((o) => o.poItemRecordId).filter(Boolean))];
    if (!vendorRecordId || charged.length === 0) return empty;

    const poItems = await getPOItemsByRecordIds(charged);
 ...
 */
async function checkInvoicePairing({
    user,
    invoiceRecordId,
 ...
 * transaction rather than an oversight. createDeliveryAction writes the link as the
 * last step INSIDE its own rollback, so a failure there takes the whole delivery
 * with it — and nothing it writes is picked: `planPairings` computes every link from
 * a fresh read (#231), so there is no chosen invoice for `checkInvoicePairing` to
 * refuse and the entry path calls `setInvoiceDelivery` alone. This said it called
 * both, which stopped being true when #231 took the entry form's invoice field away
 * (corrected per #181 by #438).
 *
