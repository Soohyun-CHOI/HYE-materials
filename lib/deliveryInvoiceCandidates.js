...
import { billFromInvoiceOption } from "./deliveryInvoiceMatch";
...
export async function getInvoiceLinkCandidates(user, { vendorRecordIds } = {}) {
    ...
    // passes without it; the computed pairing needs the ordered items every bill
    ...
    // What each bill charges against, keyed by its parent. `Item Name` text is
    ...
    for (const item of items) {
        ...
        chargedByInvoice.get(parent).push({ poItemRecordId, unitPrice: item.unitPrice });
    }
        ...
        return {
            ...
            linkedDeliveryId:
                holder && canAccessJobDeliveries(user, (holder.job || [])[0])
                    ? holder.deliveryId
                    : null,
            // #231 — what this bill charges against, which is what places it on a
            // shipment. The matcher reads it through
            // lib/deliveryInvoiceMatch.js:billFromInvoiceOption rather than
...
/**
 * #231 — THE SAME PAIRING READ FROM THE OTHER END: the shipments a bill being
 ...
 * guarded write for `Invoices."Delivery"`, and "which arrivals may this bill name"
 ...
 *
 * THE BILL POOL COMES FROM `getInvoiceLinkCandidates` ABOVE, WHICH IS THE WHOLE
 ...
 *   2  Delivery Items (those slices, for the shipments they belong to)
 *   3  Deliveries     (those shipments — vendor, label, Job, full item arrays)
 *   4  Delivery Items (each candidate shipment's FULL set, so the arrival handed to
 *                      the matcher is the same shape the delivery form hands it)
 *   5+ getInvoiceLinkCandidates (the vendor's bills, ≤3)
 *
 * A BILL WHOSE ORDERED ITEMS HAVE NOT ARRIVED COSTS ONE OPERATION, and that is the
 * ordinary case rather than the optimized one: the vendor emails the invoice when
 * the material ships, so the bill is normally entered before the packing list
 * exists. Measured on this base 2026-08-13, 6 of 13 unpaired invoices have no
 * candidate arrival at all. Step 2's id list is empty for them, so 3, 4 and the
 * bill pool are never reached.
 *
 ...
 */
export async function getArrivalsForBill(user, { vendorRecordId, orderedItems } = {}) {
    const empty = { arrivals: [], bills: [], agreedPrices: new Map() };
    ...
    for (const slice of fullSlices) {
        ...
        if (!parent || !poItemRecordId) continue;
        if (!broughtByDelivery.has(parent)) broughtByDelivery.set(parent, new Set());
        broughtByDelivery.get(parent).add(poItemRecordId);
    }

    const options = await getInvoiceLinkCandidates(user, { vendorRecordIds: [vendorRecordId] });

    return {
        arrivals: candidates.map((d) => ({
            deliveryRecordId: d.id,
            deliveryId: d.deliveryId,
            orderedItemRecordIds: [...(broughtByDelivery.get(d.id) || [])],
        })),
        bills: options.map(billFromInvoiceOption),
        agreedPrices,
    };
}
