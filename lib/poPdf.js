 ...
 * Public Vercel Blob store, writes the result onto the PO record, and then
 * (issue #140) deletes the Blob object once Airtable confirms it took the
 * file — Airtable's copy is the copy of record.
 ...
 * never rolled back by a PDF failure. Cleanup itself never throws.
 */
export async function generateAndAttachPOPdf(poRecordId) {
    ...
    // because this function is already decoupled from the signing
    // transaction — PDF generation is non-fatal to a signature — so "the end
    // of the action that ingested it" is the end of this function. It also
    // removes regenerate's second orphan: each run now cleans up the object
    // whose attachment it just replaced.
