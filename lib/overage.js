...
// one bill for the ordered item, or nothing names this shipment at all. AND ONE THING
// IS REFUSED RATHER THAN INFERRED — several bills with no pairing between them, where
...
export const OVERAGE_BLOCKED = {
    ...
    // #219 — more than one bill and not one of them names this shipment. REFUSED
    ...
    // between two bills both recorded as describing this arrival, here it would be
    ...
    severalUnpairedBills: "several-unpaired-bills",
 ...
 *
 * CALLED FROM ONE OF THE TWO TIERS, NOT BOTH — see candidateBills. Ordering the bills
 * that name this shipment is a tie-break; ordering the ones that name nothing would be
 ...
 * changes which bill an inference favors. That is tolerable HERE and nowhere else in
 * this module: the consequence is a coin-flip landing the other way between two
 * documents both recorded as describing this arrival, on an answer already marked as
...
/**
 * The bills that may carry THIS shipment's excess, and why the answer is an
 ...
 *
 *   - PAIRED, two candidates: both bills are RECORDED as describing this arrival, so
 ...
 *     arrival. An ordering there would not be a tie-break, it would be a CHOICE WITH
 ...
 *     the honest answer is that we cannot say. REFUSED (`severalUnpairedBills`).
 *
 ...
 * nothing says that bill belongs to this arrival either.
 *
...
function candidateBills(bills, deliveryRecordId) {
    ...
    if (namesNoDelivery.length > 1) {
        return { candidates: [], inferred: null, blocked: OVERAGE_BLOCKED.severalUnpairedBills };
    }
    if (namesNoDelivery.length === 1) {
        // INFERRED THOUGH THERE IS ONLY ONE, unlike the tier above: with nothing
        // named, the single bill is still only the bill that HAPPENS to be the one
        // nobody paired.
        return {
...
export function selectOverageBill({ bills, excess, deliveryRecordId } = {}) {
    const all = bills || [];
    const { candidates, inferred, blocked } = candidateBills(all, deliveryRecordId);

    // A tier that refuses does so before anything about quantities: there is no
    // chosen bill for the excess to fit inside, and `inferred` is null because a
    // refusal has no answer to qualify.
    if (blocked) return { bill: null, inferred: null, blocked };
...
export const OVERAGE_COPY = {
    preview: {
        ...
        blocked: {
            ...
            // #219 — several bills and no pairing to choose between them. THE FACT
            ...
            [OVERAGE_BLOCKED.severalUnpairedBills]: () => ({
                key: OVERAGE_BLOCKED.severalUnpairedBills,
                text:
                    "More than one invoice bills this ordered item and none of them names this " +
                    "delivery, so nothing records which one bills what was delivered here. " +
    ...
    strip: {
        ...
        reason: {
            ...
            [OVERAGE_BLOCKED.otherDeliveryOnly]: "bills name another delivery",
            [OVERAGE_BLOCKED.severalUnpairedBills]: "no invoice names this delivery",
            [OVERAGE_BLOCKED.spansInvoices]: "spans two invoices",
            [OVERAGE_BLOCKED.excessExceedsBill]: "more than the invoice bills",
    ...
    banner: {
         ...
         * did not, so wherever the facts reach it without a bill the sentence
