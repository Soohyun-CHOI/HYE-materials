 ...
 *
 * Before the apply step the row's own `PO Item` IS the original; afterwards that
 * names the overage order and the provenance link holds the original. One
 * expression rather than a branch at each of the four readers.
 */
export function originalPOItemRecordId(row) {
    return row?.overageOfPOItemRecordId ?? attachedPOItemRecordId(row);
}
 ...
 * distinction from originalPOItemRecordId above matters exactly once: after the
