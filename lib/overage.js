...
// shows it. This is the step that squares the RECORD with it: an overage request for
...
//
// WHAT EARNS AN OVERAGE REQUEST IS THE TWO DOCUMENTS AGREEING ABOVE THE ORDER (#265), and
// the flag alone does not. An overage request exists because a site ordered more without
...
// ten charged — is the opposite shape, and an overage request raised on it sends the vendor
...
// (an overage request is owed), they do not (the vendor's own discrepancy, either
...
// made. Under the agreement rule an overage request is owed only where the excess IS
...
//
// Pure, and its one import is another pure module — `lib/prWait.js`, the
// listed/offered split this strip shares with the direct-purchase one (#272) — so
// scripts/tests/offline/overage.mjs can still pin every clause.

// #272 — the listed/offered split both strips share. The extension is required
// rather than stylistic: the offline tier imports this module under plain `node`,
// which does not resolve an extensionless specifier — the same reason
// `deliveryAllocation.js` writes `./poItemQty.js`.
import { stillWaiting } from "./prWait.js";

/**
 * Why an overage request cannot be raised for a row. Keys, so a reworded message fails nothing.
 *
...
export const OVERAGE_BLOCKED = {
    ...
    // unit price, so which one is quoted changes the figure on the overage order.
 ...
 *
 * `agreed` is the only one that earns an overage request. The other two are refusals with
...
/**
 * WHETHER AN OVERAGE REQUEST IS PENDING IS READ, NEVER STORED — the linked PR's own
 ...
 * option added to the field later must not silently make a live overage request
 ...
 * `generated` and never be offerable again, locked out by an overage request that no
 * longer exists.
 *
...
export function overagePRState(overagePR, overagePO) {
    ...
    if (overagePR.status === "Approved" || overagePR.status === "PO Signed") {
        // A withdrawn overage order is no overage request at all. Reaching here with the
...
/**
 * Does this row still need an overage request raised (#217)?
 *
 ...
 */
export function awaitsOverageRequest({ row, overagePR, overagePO } = {}) {
...
/**
 * Is this row still ON the strip — which is a different question from whether the
 * button is offered for it (#272).
 *
 * THEY WERE ONE QUESTION AND THAT COST THE ROW. `awaitsOverageRequest` above is
 * false the moment any request covers the excess, Draft included, and the strip
 * used to select on it alone: the first person to press the button took the row
 * off everyone's screen, and if they abandoned the draft the excess was visible
 * nowhere at all, since a Draft is visible to its requester and nobody else. The
 * split is `lib/prWait.js`'s rule and the direct-purchase strip asks the same one.
 *
 * THE TWO CLAUSES ARE ORDERED, NOT COMBINED. `overagePRState` answers `none` for
 * a withdrawn overage ORDER even though its request is `PO Signed`, which is
 * #167's reopening rule and has nothing to do with the request's own stage — so
 * it is asked first, and only a row nothing offers falls through to the stage.
 */
export function overageStillWaiting({ row, overagePR, overagePO } = {}) {
    if (!row?.overDelivered) return false;
    if (!attachedPOItemRecordId(row)) return false;
    if (overagePRState(overagePR, overagePO) === "none") return true;
    return stillWaiting(overagePR);
}

/** Which stage a live overage request has reached, for the copy that names it (#217). */
...
/**
 * The stage of an overage request that already covers an excess (#217).
 *
 ...
 * to go and nudge a draft nobody submitted. Returns null when no overage request is
...
/**
 * Is a live overage request now covering an excess that no longer matches its row
 * (#206)?
 *
 ...
 * the `Overage PR` link is never cleared, because delete-then-reenter IS how a
 * delivery is put right and a link destroyed mid-edit cannot be restored when the
 * excess reappears seconds later. So the contradiction is reported rather than
 * resolved, as a qualifier composed with whichever banner the overage request already
 ...
 * overage request covers — cannot be produced by a delete, because the only row a
 * delete resizes is the one that stops being flagged; every other flagged row
 * keeps its quantity untouched. It IS reachable by editing a draft overage request's
 * quantity, which is outside this issue and would cost a read of the overage
 * order's own items on every banner render.
 *
 * NOT REACHABLE FOR AN APPLIED OVERAGE REQUEST, which is why the copy has two voices
 ...
 * discrepancy rather than the order's, and both are refused for the same reason: an
 * overage request raised on either sends an order for material the two documents do not
...
export function overageAgreement({ orderedQty, deliveredQty, invoicedQty, hasInvoice } = {}) {
    ...
    // overage request — fail-OPEN, on the one path this issue exists to close. A flagged row
 ...
 *   3. If they disagree on the unit price, REFUSE. The overage request quotes a price, so
...
/**
 * The signing chain to copy onto an overage request, minus anyone inactive (#217 moved
...
/**
 * May an overage request be raised for this row, and with what.
 *
 * Order matters. `already-raised` is tested before anything about the invoice,
 * because a row someone is already raising one for must not be reported as blocked for
 ...
 * rule. Where the two documents do not agree there is no overage request to raise at all,
 ...
 * same ordered item; the overage request covers the row in hand, which is the row the
...
export function overageEligibility({ row, invoices, overagePR, overagePO, orderedItem } = {}) {
    ...
    // `PO Item` was emptied by hand, because an overage request needs an order to point at,
 ...
 *
 *   null          — nothing to say (no live overage request).
 *   pending       — an overage request is being raised; the excess has not moved.
...
// overage request (second person, future), the way ALLOCATION_COPY.preview does.
// `banner` addresses whoever later opens one of THREE documents that all describe
// the same overage request from different sides — the overage PR, the overage PO, and
...
// `line`, since #303 gives each item table's row its own table's name — this said
// `since a `Line` on this base is a child of a Job` until #321, which is the
// premise #280 retired with the table. And facts, never
 ...
 * (#217). Each says where the overage request has got to and therefore whether there is
...
export const OVERAGE_COPY = {
    preview: {
         ...
         * excess; there is no such guess now, because an overage request is offered only
        ...
        tieBreak: (f) => ({
            ...
            text:
                ...
                "least this much of the ordered item at the same price, so which one " +
                `supplies the quotation changes no figure — ${f?.chosen ?? "the first"} is ` +
                "the one quoted.",
        }),
        /**
        ...
        blocked: {
            ...
            [OVERAGE_BLOCKED.noInvoice]: () => ({
                ...
                text:
                    ...
                    "delivery against. Whether this earns an overage request is answered as soon as " +
             ...
             * an overage request would order material they do not both account for; what
            ...
            [OVERAGE_BLOCKED.documentsDisagree]: (f) => {
                ...
                const tail =
                    f?.disagreement === DISAGREEMENT.invoicedShort
                        ? "The vendor has not invoiced everything it sent, so an overage request would " +
                        ...
                        : f?.disagreement === DISAGREEMENT.invoicedOver
                          ? "More is invoiced than was delivered, which is a discrepancy on the " +
                            "invoice rather than an excess to raise a request for. Take it up with the vendor."
                          : "The two do not meet, so there is no agreed excess to raise a request for. Take " +
            ...
            [OVERAGE_BLOCKED.spansInvoices]: () => ({
                ...
                text:
                    "No single invoice charges as much of this ordered item as was delivered beyond the " +
                    "order, so the excess spans more than one. There is no one quotation to " +
                    "attach — raise the request by hand.",
            }),
            /**
             * #265 — several invoices could supply the quotation and they disagree on
             * the unit price, so the choice would change the figure on the overage
            ...
            [OVERAGE_BLOCKED.severalPricesDiffer]: () => ({
                ...
                text:
                    "More than one invoice could supply the quotation and they charge this ordered item at " +
                    "different prices, so which one is quoted would change what the overage " +
                    "order asks for. Raise the request by hand at the right price.",
            }),
     ...
     *
     * NO KEYS FOR `notOverDelivered` OR `alreadyRaised`, deliberately: `awaitsOverageRequest`
    ...
    strip: {
        heading: (n) =>
            n === 1
                ? "1 over-delivery is waiting for a request"
                : `${n} over-deliveries are waiting for a request`,
        ...
        // sentence.
        explain:
            "Longest wait first. A row with everything it needs raises the request here; " +
            "the rest say what has to come first.",
        // #265 — THE DISAGREEMENT TAKES ONE CHIP FOR BOTH OF ITS DIRECTIONS, and
    ...
    banner: {
        /** Reading the overage request itself. */
        overagePR: (f) => ({
        ...
        /** Appended while the overage request has no order yet. */
        pending: (f) => ({
            key: "banner-pending",
            text:
         ...
         * design: the invoice an overage request spans is invoice-derived, so it is
        ...
        /**
         * #206's qualifier: the excess this overage request covers is no longer there.
         *
