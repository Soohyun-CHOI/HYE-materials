...
// as unbilled when it is billed twice over. An excess exists only relative to the
...
// direction), or nothing bills the ordered item yet (nothing to judge). The third is
...
// document when the app could not tell whether the excess had been billed at all: the
...
// billed, so the document exists by construction and there is nothing to infer.
...
export const OVERAGE_BLOCKED = {
    ...
    // #265 — the third state. Nothing bills the ordered item, so there is nothing to
    // agree or disagree with yet, and this resolves itself when the invoice is entered.
    noInvoice: "no-invoice",
    // #265 — the second state: what was delivered and what was billed do not meet, so
    // the discrepancy is the vendor's rather than the order's. ONE KEY, TWO VOICES —
    // billed short of the delivery and billed beyond it are opposite errors and the
...
/**
 * Which way a disagreement runs — `billed-short` or `billed-over` (#265).
 *
 ...
 */
export const DISAGREEMENT = { billedShort: "billed-short", billedOver: "billed-over" };
...
export function disagreementDirection({ deliveredQty, invoicedQty } = {}) {
    ...
    return invoiced < delivered ? DISAGREEMENT.billedShort : DISAGREEMENT.billedOver;
}
 ...
 * item filled by two deliveries and billed by two invoices, each invoice large enough
 ...
 * `orderedItem` reads as 0 delivered against whatever is billed and refuses as a
...
export function overageEligibility({ row, invoices, overagePR, overagePO, orderedItem } = {}) {
    ...
    if (!attachedPOItemRecordId(row)) {
        return { eligible: false, blocked: OVERAGE_BLOCKED.noOrderedItem };
    }
...
export const OVERAGE_COPY = {
    preview: {
         ...
         * 19 delivered and 4 billed, and nothing on the screen says where 19 came
        ...
        compared: (f) => ({
            ...
            text:
                ...
                `${qtyUnit(f.invoicedQty, f.unit)} billed.`,
        }),
        /**
         ...
         * where the excess is billed and a candidate has to cover it on its own. What
        ...
        blocked: {
            ...
            [OVERAGE_BLOCKED.noInvoice]: () => ({
                ...
                text:
                    "No invoice bills this ordered item yet, so there is nothing to compare the " +
             ...
             * on this axis: at any one moment "the vendor under-billed" and "a second
            ...
            [OVERAGE_BLOCKED.documentsDisagree]: (f) => {
                const totals =
                    ...
                    `${qtyUnit(f?.invoicedQty ?? 0, f?.unit)} billed`;
                const tail =
                    f?.disagreement === DISAGREEMENT.billedShort
                        ? "The vendor has not billed everything it sent, so a correction would " +
                          "order material nobody charged for. Take the difference up with the " +
                          "vendor, or wait for the invoice that covers it."
                        : f?.disagreement === DISAGREEMENT.billedOver
                          ? "More is billed than was delivered, which is a discrepancy on the " +
            ...
            [OVERAGE_BLOCKED.spansInvoices]: () => ({
                ...
                text:
                    "No single invoice bills as much of this item as was delivered beyond the " +
            ...
            [OVERAGE_BLOCKED.severalPricesDiffer]: () => ({
                ...
                text:
                    "More than one invoice could supply the quotation and they bill this item at " +
 ...
 *
 * `notOverDelivered` AND `noOrderedItem` GET NO COMPARISON LINE, and they are the two
 * that would make it false: neither reaches the totals at all, so naming figures
 * would claim a measurement that was never taken.
 */
const NO_COMPARISON = [OVERAGE_BLOCKED.notOverDelivered, OVERAGE_BLOCKED.noOrderedItem];
