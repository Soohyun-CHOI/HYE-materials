...
// Here the question is which bill's invoice item the excess quantity sits in, and a
 ...
 * `invoiceId`). `bills` is one entry per invoice item on the ordered item:
 * `{ invoiceItemRecordId, invoiceRecordId, invoiceId, issueDate, qty, unitPrice,
 * hasFile }`.
 *
 * THE EXCESS MUST FIT INSIDE THE CHOSEN BILL'S INVOICE ITEM. When it does not,
 * the excess spans two invoices, which is out of scope: there is no single
 * quotation to attach, so the button is hidden and says so rather than picking a
 * second invoice's file. Under oldest-first that condition is exactly "the oldest
 * bill's invoice item is smaller than the excess", so it falls out of the
 * ordering rather than needing a rule of its own.
 */
