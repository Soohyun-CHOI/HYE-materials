...
/**
 * An `&` inside an `href` is an entity start, and this file writes its own HTML.
 *
 ...
 * called. Escaping the separator is one line and removes that dependency; it
 * lives here rather than in the builder because the builder makes a URL and this
 * is the only place one is put into markup.
 */
 ...
 *
 * SINCE #373 IT MAY ALSO CARRY A DESTINATION, which is how a reader who scanned a
 * tool label while signed out gets back to that tool item. `lib/auth.js` builds
 * the whole URL through `confirmPath`, so nothing here knows the parameter's name
 * and nothing here judges it.
 *
