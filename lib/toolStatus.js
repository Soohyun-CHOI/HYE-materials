...
// where a tool item can be, what can happen to one, which of the first each of
// the second leaves behind, and which of the second each of the first offers
// next (#362).
//
// THE TWO MAPS ARE BOTH HERE BECAUSE THEY ARE ONE VOCABULARY SEEN FROM TWO
// SIDES. Neither is derivable from the other — two events land on `In Stock`
// and `Registered` is offered by nothing — so a fourth status or a fifth event
// has to visit both, and putting one of them beside the screen that reads it
// would leave the other unvisited. `offline/tool-status.mjs` pins them
