...
// anyone who may see the order behind a line, so a site recorder can already read
// their own job's invoices; before that a picker would have had to show invoice
// IDs to someone barred from the invoice list, and typing was the only shape that
// disclosed nothing. With that barrier gone the dropdown is strictly better: it
// cannot be mistyped and it cannot name an invoice that does not exist. What it
// still cannot do is invent one — see LINK_COPY.emptyList.
//
...
export function invoiceLinkRefusal({ invoice, deliveryRecordId, vendorRecordId, visible } = {}) {
    ...
    if (visible === false) return LINK_REFUSED.outOfScope;
    ...
    refused: {
        ...
        [LINK_REFUSED.takenByAnother]: (f) => ({
            ...
            text:
                ...
                `${f?.deliveryId ? `delivery ${f.deliveryId}` : "another delivery"}. One invoice ` +
                "belongs to one delivery — detach it there first if this is the right shipment.",
        }),
    },
};
