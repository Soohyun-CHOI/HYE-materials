// The invoice-to-delivery pairing (#210) — which invoices a delivery may name, what
// the dropdown calls them, and why one is refused.
//
// THE OFFICE RULE IS NOW A FIELD. `Invoices."Delivery"` is n:1 — one invoice names
// one delivery, one delivery can carry several invoices — which is the containment
...
// vendor emails the invoice when the material ships, so the invoice is normally on
...
// never on the invoice, where the office would have to know which delivery a
// number belongs to.
//
...
export function invoiceLinkRefusal({ invoice, deliveryRecordId, vendorRecordId, visible } = {}) {
    ...
    // An invoice from another supplier cannot describe this delivery, and the vendor is
 ...
 *
 * VENDOR IS THE WHOLE NARROWING, AND NOT THE JOB. An invoice can charge orders on
 * more than one job (lib/airtable/invoices.js on the join table), so narrowing by
 * job could hide the right invoice; narrowing by vendor cannot, because a delivery
 * has exactly one vendor and an invoice from another supplier is never the answer. The
 ...
 * list use. An undated invoice sorts LAST rather than first, the call sortCandidates
...
export function invoiceOptionLabel(option) {
    ...
    // Job-scoped, and an invoice can charge two jobs, so the one holding it is not
...
// an invoice to a delivery that already exists — so one wording for both would be worse
...
/**
 * The fact both `taken-by-another` voices open with: this invoice is on some other
 * delivery, and one invoice belongs to one delivery. No trailing punctuation, because
 * one voice continues into an action and the other stops here.
 *
 * NAMES THE DELIVERY ONLY WHEN THE CALLER COULD NAME IT. `deliveryId` is null when
...
export const LINK_COPY = {
    field: {
        /**
         * THE CHECKBOX LABEL, and it is phrased as a fact about the DOCUMENT since
         * #231. It asks what is printed on the packing list, not which of our
         * invoices this is — the second question is the app's now, and asking a
         * recorder to answer it beside a sentence saying the app already has would
         * be asking twice. Worded to match `The packing list shows a PO number` on
         * the same form, because the two are the same kind of question.
         */
        // `label`, `optional` and `transcribed` LIVED HERE AND ARE GONE (#231).
        // All three belonged to the entry form's invoice control, and that control
        // is gone: the pairing is computed, and #210 built the picker on the premise
        // that a packing list carries an invoice number — which this app's plan
        // never claimed. Only the PO number was ever said to be on that document.
        // Deleted rather than left for #182, since a copy entry with no caller is
        // exactly what that issue exists to find, and this branch is what took the
        // caller away. What remains is read by the delivery's EDIT page, which is
        // where a computed pairing is corrected by hand.
    ...
    refused: {
        ...
        [LINK_REFUSED.wrongVendor]: () => ({
            ...
            text:
                "That invoice is from a different vendor, so it cannot be the invoice for " +
                "this delivery.",
        }),
        /**
         * TWO VOICES, AND THE DIFFERENCE IS WHETHER THE READER CAN REACH THE
         * DELIVERY HOLDING IT — #206's rule applied to a refusal rather than to a
         ...
         * three.
         *
         * A delivery is Job-scoped and an invoice can charge two jobs, so the delivery
         * holding an invoice is not always in view. `invoiceOptionLabel` already makes
         ...
         * `noLongerOverSentence` uses in lib/overage.js. Both voices say the invoice is
        ...
        [LINK_REFUSED.takenByAnother]: (f) =>
            f?.deliveryId
                ? {
                      ...
                      text: `${takenByAnotherFact(f)} — detach it there first if this is the right delivery.`,
                  }
