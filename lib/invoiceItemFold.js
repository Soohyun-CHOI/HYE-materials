// Folding an invoice's split lines back into one (#167).
//
// The overage flow SPLITS an invoice line: the quantity that exceeded the order
// moves onto the overage PO's own ordered item, so one line the vendor printed
...
//     invoice line's own — so the two products of a split always share it, while
//     two lines billed at different prices are two facts and must stay two rows.
//
...
// be half of a split, and folding it on anything else could merge two lines that
// were never one. If the cache failed, the two halves simply render as two rows —
 ...
 * lines were entered, so a fold never moves a line the reader was looking for.
 *
