...
// the builders' own escaping is pinned by that check's behavioral half.
//
 ...
 * for the whole table.
 *
 * `fields` MAY BE SEVERAL, AND EACH TOKEN THEN MATCHES IN ANY OF THEM (#416).
 * The AND stays over the TOKENS and the OR is inside each one — `every word
 * appears somewhere` rather than `every word appears in the same field`, which is
 * what a reader typing a category word and a size means: `/materials` matches a
 * material's own label and its category's path, and no row carries both words in
 * one of them. ANDing per field instead would find nothing at all.
 *
 * The fields are joined rather than OR-ed per clause for one reason worth stating:
 * a token could otherwise straddle two fields — `SEARCH` over `A & B` matches a
 * needle spanning the seam. They are joined WITH A SPACE, so a needle can only
 * straddle by containing one, and a token never does: `searchTokens` splits on
 * whitespace.
 *
 * **`LOWER()` GOES AROUND THE WHOLE JOIN AND NOT AROUND EACH FIELD, WHICH IS A
 * MEASURED AIRTABLE BEHAVIOR RATHER THAN A STYLE.** `LOWER({a lookup field})`
 * returns BLANK — measured against this base: `SEARCH("stainless",
 * LOWER({Category Path}))` matched 0 of 37 materials while
 * `SEARCH("stainless", LOWER({Material Label} & " " & {Category Path}))` matched
 * 35. A lookup behaves as an array, and concatenation flattens it where `LOWER`
 * does not. Lowering each field separately therefore matches NOTHING and reports
 * no error — the search simply goes quiet, which is what the first version of
 * this did. **The offline tier cannot catch it**: it simulates the rule in
 * JavaScript and never executes a formula, which is why `docs/notes/
 * verification.md` puts Airtable formulas outside CI. A live query is the only
 * witness.
 */
export function andSearchAll(fields, needles) {
    const names = Array.isArray(fields) ? fields : [fields];
    const haystack = `LOWER(${names.map((n) => fieldRef(n)).join(' & " " & ')})`;
    const clauses = (needles || [])
        .filter((n) => n !== null && n !== undefined && n !== "")
        .map((n) => `SEARCH("${formulaString(n)}", ${haystack})`);
