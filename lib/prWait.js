...
export const WAIT_COPY = {
     ...
     *
     * THE NAME COMES FROM `lib/userName.js`, which is what every other screen that
     * prints a person reads — the PR list's Requester column, the history
     * timeline, the signer chain. It is the FIRST name (#381), because this chip
     * reports somebody rather than asking the reader to choose them; the pickers
     * and the two vendor-facing surfaces are the places that print both names.
     * **This paragraph said the name was `Users."User Name"` and that it was the
     * email's local part because nothing else set it** — the field is `First Name`
     * now, typed by its owner at their first sign-in, and the local part survives
     * only as that module's fallback while a row has not been named. Inventing a
     * second way to name people here would still be the thing to avoid.
     */
