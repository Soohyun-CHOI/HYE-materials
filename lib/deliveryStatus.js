...
//
// The office cannot yet tell whether what a vendor billed for was delivered, nor what
...
// a delivery carrying material nobody had billed yet spilled onto the next invoice
...
// premise". In one line: the material an invoice bills is delivered by the delivery it
 ...
 * false`, #96) — so a conforming invoice has none. The ones on this base when this
 * was written were hand-entered dummy data, one of which CLAUDE.md records:
 * `HYE-INV-260727-04` carried a variance flag with an empty `PO Item`, a
 ...
 */
export function countsTowardStatus(invoiceItem) {
    return Boolean(invoiceItem?.poItemRecordId);
}
...
export function orderedItemStatus({ orderedQty, invoicedQty, deliveredWithinQty, deliveredOverQty } = {}) {
    ...
    return {
        ...
        billedNotDelivered: Math.max(0, invoiced - delivered),
        deliveredNotBilled: Math.max(0, delivered - invoiced),
 ...
 *
 * A share's delivered quantity is CLAMPED at what that invoice billed
...
export function invoiceVerdictKey(status) {
    ...
    if ((status.billedNotDelivered || 0) > 0) return "billed-more";
 ...
 *
 * CLAMPED AT WHAT THIS INVOICE BILLED, and the clamp is load-bearing rather than
 * tidy: a delivery may carry material that will be billed later, so the linked
 ...
 * `deliveredNotBilled` is 0 by construction here, where on `orderedItemStatus` it is a real
 * measurement.
 *
 ...
 */
export function invoiceShareStatus({ billed, delivered } = {}) {
    ...
    return {
        invoiced,
        delivered: clamped,
        billedNotDelivered: Math.max(0, invoiced - clamped),
        ...
        deliveredNotBilled: Math.max(0, clamped - invoiced),
 ...
 * the delivery this invoice describes, so "has the material this invoice bills for
 ...
 */
export function summarizeInvoiceStatus({ itemStatuses, hasDelivery = false, excludedCount = 0 } = {}) {
    const entries = itemStatuses || [];
    const judged = entries.length;
    const covered = entries.filter((s) => s.billedNotDelivered === 0).length;
    const short = entries.some((s) => s.billedNotDelivered > 0);
...
/**
 * ONE ORDERED ITEM AGAINST WHAT HAS BEEN BILLED FOR IT (#235) — the invoicing
 ...
 * the question at this scope is whether the ordered quantity has been billed, for
 ...
 *
 * BILLED BEYOND THE ORDER COUNTS AS BILLED, which mirrors the delivery axis
 ...
 * reading here keeps one answer to "is there anything left to bill".
 *
 ...
 * be withdrawn once anything bills it — `getPOWithdrawEligibility` refuses on
 * `invoicePoLinks` or `invoiceItems` — so "withdrawn and billed" is unreachable
 ...
 * is separate from the completed count, so a one-item order of 13 with 10 billed
 * reads as partly billed rather than as nothing billed, which is #166's lesson paid
 ...
 * a stage. An ORDER has no such premise: it is billed by as many invoices as the
 * vendor sends, so a half-billed order is an ordinary middle. The delivery axis
 ...
 *
 * "INVOICED" NOW MEANS THIS DELIVERY WAS BILLED, WHICH IS AN ATTRIBUTION #166 DID
 ...
 * delivery can carry material nobody has billed yet — two materials are delivered, one
 ...
 * beyond, for `orderedItemStatus`'s reason: the vendor bills what it shipped, so the
 * excess is part of what an invoice answers. `billed` is what the linked invoices bill
 ...
 * against: `{ poItemRecordId, delivered, billed }`.
 */
export function summarizeDeliveryInvoicing(orderedItemsTouched) {
    ...
    const invoiced = entries.filter((l) => (l.billed || 0) >= (l.delivered || 0)).length;
...
export const STATUS_COPY = {
    ...
    column: {
         ...
         * been billed" and this answers it of an order. One question at two scopes,
    ...
    detail: {
         ...
         * the delivery it matches or by none, so "everything billed was delivered" is a
         ...
         * bills either delivered against that delivery or was never shipped — so it is an
         ...
         *
         * EACH CARRIES A `tone` SINCE #241, AND IT IS ITS OWN TWO-VALUE VOCABULARY
         * RATHER THAN THE CHIPS'. `exception` is a discrepancy a person has to act on;
         * `unjudged` is an invoice item nothing was measured against, which is not a
         * problem but the absence of one — so `not-compared` cannot wear the same tone
         * as a shortfall, or a free-text charge would be dressed as an error while its
         * own sentence says nothing was compared. The chip tones (`complete`,
         * `partial`, `mismatch`, `none`) are a closed set of STATES with a background
         * each, and reusing them for a line of text would make one word mean a chip on
         * one screen and a text color on another. WHY IT IS HERE rather than at the
         * call site: the page needs one answer for the entry's NAME and its sentence
         * together — see lib/invoiceDeliveryEntries.js — and "this is a discrepancy"
         * is the same grade of decision as the sentence it colors, which is authored
         * here. Which amber stays a rendering decision and stays in the page.
         */
        verdict: {
            "billed-more": (s, unit) => ({
                key: "billed-more",
                tone: "exception",
                text: `${qtyUnit(s.billedNotDelivered, unit)} more billed than the matched delivery delivered`,
            }),
            /**
...
export function describeInvoiceItem(
    ...
    const beyond = status && (status.billedBeyondOrder > 0 || status.deliveredBeyondOrder > 0);
...
export const AWAITING_DELIVERY_COPY = {
    ...
    explain: "Longest wait first. Nothing has confirmed the material these invoices charge for.",
    kind: {
...
/**
 * The invoices waiting on a delivery, longest first (#256).
 *
 ...
 * and this follows.
 *
 * AN INVOICE THAT CHARGES NO ORDERED ITEM IS EXCLUDED, and that is the one place
 * this strip is narrower than the chip it selects on. Such an invoice can never be
 * paired — it is `fitRefusal`'s `noOrderedItem` — and the delivery question cannot be
 * asked of it at all, so both row words would be false. It is `countsTowardStatus`'s
 * own decision one level up: comparing a free-text charge against nothing would make
 * it read as a state it is not in.
 *
 * SO THE ROW COUNT AND THE NUMBER OF `Awaiting delivery` CHIPS IN THE TABLE BELOW
 * CAN DIFFER, AND THAT IS NOT A DEFECT. Two of this base's invoices name no ordered
 * item at all and sit in the table wearing the chip with no row up here. Anyone
 * comparing the two figures will find them apart; this paragraph is why, and
 * `docs/briefs/invoices.md` says the same thing for the design side.
 *
...
export function selectInvoicesAwaitingDelivery({
    ...
    for (const invoice of invoices || []) {
        ...
        rows.push({
            ...
            daysWaiting: daysWaiting(invoice.issueDate, today),
