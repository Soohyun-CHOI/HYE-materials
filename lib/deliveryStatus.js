 ...
 * keyboard.
 *
 * ONE DENSITY, TWO SCREENS (#232). It reads the same on the invoice list and on the
 * invoice detail's section heading, because it is one fact about one invoice and
 * both surfaces show it beside the same chip. The comment here said the detail
 * needed no marker, since the box already stated the shortfall per ordered item —
 * true while every box spoke. Under THE ONE-DELIVERY PREMISE a normal box says
 * nothing at all, so the detail's chip was the only thing left describing the
 * invoice and it described it as complete. The two levels answer different
 * questions now: the marker says SOMETHING disagrees, the box says which ordered
 * item and by how much.
 *
 ...
 * same measurement. `matched` rather than `names` since #232, one word for one
 * fact across both surfaces and the section's own empty state.
 */
const MISMATCH_REASON = "this invoice bills more than the delivery matched to it delivered";
...
export const STATUS_COPY = {
    ...
    detail: {
         ...
         * still arrive or the arrival may still be recorded. `MISMATCH_REASON` is
         * the reference vocabulary and both extend its phrasing.
         *
