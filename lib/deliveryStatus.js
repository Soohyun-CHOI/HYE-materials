 ...
 *
 * WHERE THAT BREAKS, AND IT IS NOT A HYPOTHETICAL ORDERING. Deleting a delivery
 * removes its `Delivery Items` rows, and deletion is the ONLY way to correct an
 * item or a quantity — `lib/deliveryDelete.js` says so in its own header, since
 * item, quantity, vendor and packing-list PO are not editable and there is no
 * allocation-editing UI. So the reachable shape is: an over-delivery row is
 * attached to a full line, and the delivery carrying that line's within-order
 * rows is then deleted. THE CONDITION IS PRECISE — it misreads only when the
 * over-delivery rows still attached to a line sum to at least that line's `Qty`.
 * Worked example: order 10, delivery A brings 10, delivery B brings 10 more which
 * attaches to the same line entirely as surplus, delete A. `Delivered Qty` is 10,
 * `Qty` is 10, so this reports `Delivered` while nothing arrived against the
 * order at all. The direction of the error is the bad one: material that has not
 * come reads as arrived, and nobody goes looking for it. #20 reads
 * `Delivery Items` directly and separates the two, so it is the screen where the
