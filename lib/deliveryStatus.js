...
// DIFFERENT question — which bill's invoice item carries an over-delivered excess — and
 ...
 * bill's invoice item carries an over-delivered excess — and still has to infer,
 * because reading that off the stored pairing needs #167's `spansInvoices`
 * refusal rethought alongside it and is #210's explicit non-goal. It stays
 * exported here, next to `INFERRED_PREMISE`, so the two halves of that one
 * inference keep one home until the issue that retires it moves both.
 *
 ...
 * #167's "which bill's invoice item carries the excess" — and #210 answered the
 * first from the stored pairing, so only #167's use is left.
 *
 ...
 *
 * `status` is null for an invoice item that named no ordered item — the box still
