...
// with the flag untouched. Every charge names an ordered item now, so the
 ...
 * named no ordered item — and it is unreachable now that every charge names one.
 *
 ...
 * and the only thing it ever counted was a charge with no ordered item. Nothing
...
export const STATUS_COPY = {
    ...
    detail: {
         ...
         * not wear a shortfall's tone without dressing a free-text charge as an
         * error. That charge is gone (#278) and it was `unjudged`'s only producer, so
        ...
        verdict: {
            ...
            // sentence in this module about a charge with nothing behind it, and the
 ...
 * delivery, so it was true whatever the pairing was; with every charge naming an
 ...
 * level up, and that predicate is gone with the charge behind it: no invoice this app
 ...
 * deleted. What still separates the two figures is the pairing — an invoice the rule
 * refuses for any other reason wears the chip and has no row. `docs/briefs/invoices.md`
 * says the same thing for the design side.
 *
