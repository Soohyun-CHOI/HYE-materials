...
// `hasDelivery` and withholds the verdict where the fact is the invoice's and not
// the ordered item's; and `STATUS_COPY.detail.againstOrder` renders unconditionally,
// because after the narrowing it is the only order-scoped thing a box says and its
// label is what tells the two scopes apart.
//
 ...
 *
 * NO `ordered` EITHER, AND THAT ABSENCE IS NOW LOAD-BEARING (#232). `ordered` is
 * what gates the `Against the order:` line — see describeInvoiceLine — so a bare
 * share renders no order-scoped sentence, which is correct: it holds no figure from
 * the order. A caller that wants that line grafts the order's three facts on, the
 * way lib/deliveryReconciliation.js does.
 */
 ...
 * against, so every invoice item trivially shows a shortfall and marking them all would
 ...
 * of nothing. The count decides nothing here any more — it is reported for the
 * detail and stays out of the chip, which is a closed set of values the way an
 * Airtable single select is.
 *
...
export function summarizeInvoiceStatus({ lines, hasDelivery = false, excludedCount = 0 } = {}) {
    ...
    return {
        ...
        covered,
        excludedCount,
        // ONE SHORT INVOICE ITEM IS ENOUGH — it does not average out across
        // invoice items, the same call `estimated` used to make for the same
        // reason: the reader has to open the invoice either way.
...
export const STATUS_COPY = {
    ...
    detail: {
        ...
        verdict: {
            ...
            /**
             * NARROWED BY #232 RATHER THAN REWORDED. It used to fire on an invoice
             * that named no delivery at all, which is what made it misleading: it
             * asserted that nothing had arrived when the fact was that nothing had
             * been MATCHED. That state has left the box — describeInvoiceLine returns
             * no verdict for it and the section says it once — so this key now means
             * only what it says: a delivery IS matched to this invoice and it brought
             * none of THIS ordered item.
             *
             ...
             * A bill of 0 reaches it too, by the clamp.
             */
            "nothing-delivered": () => ({
         ...
         *
         * SINCE #232 IT IS THE BOX'S ONLY ORDER-SCOPED STATEMENT, AND THE LABEL IS
         * WHAT CARRIES THAT — do not make it conditional again. The other figures in
         * the box are the invoice's own now (`Billed` is what THIS bill charges,
         * `Delivered` what the matched delivery brought of it), so `Against the
         * order:` is the whole of what the box says about the document the ordered
         * item belongs to. `ordered` therefore renders ALWAYS, not only when
         * something exceeds: hiding it behind an exception would take the order out
         * of the box entirely on every ordinary row, and the reader would have no
         * denominator for the two figures above. The words `Against the order:` are
         * the mechanism that keeps the two scopes apart, which is why they are a
         * label rather than a courtesy.
         *
         ...
         * side comes first because that is the side this screen is about. The two
         * beyond-order terms stay at ORDER scope on purpose. `billedBeyondOrder` has
         * no honest per-invoice form — two bills of 20 against an order of 30 leave
         * every invoice reading clean while the order is over-billed by 10 — and
         * `arrivedBeyondOrder` could be narrowed to the matched delivery for free
         * and must not be, because then one line would hold two scopes, which is the
         * defect #232 exists to remove one level up.
         */
        againstOrder: (s, unit) => {
            const parts = [`${qtyUnit(s.ordered || 0, unit)} ordered`];
 ...
 *
 * BOTH SLOTS ARE NULLABLE SINCE #232, AND EACH FOR ITS OWN REASON.
 *
 * `verdict` is null when a judged ordered item sits on an invoice that MATCHES NO
 * DELIVERY. There is a delivery fact to state and it is the same one for every box
 * on the invoice, so the section states it once above them; repeating it per box
 * would print one document's status as many times as the invoice has items, which
 * is the shape #233 took off the order's page. It is the CALLER's `hasDelivery`
 * rather than something derived from `status`, because a share with `delivered: 0`
 * cannot tell "no delivery is matched" from "the matched delivery brought none of
 * this one" — the distinction #210's stored pairing created and #232 is about.
 * A NULL `status` still gets its verdict: `Not compared — no ordered item` is a
 * fact about the invoice item, not about any delivery, so it is true either way and
 * belongs where the invoice item is.
 *
 * `againstOrder` is null when the caller carried no figure from the order —
 * `ordered` is the gate. A bare `invoiceShareStatus` is that case and renders no
 * order-scoped sentence, which is correct: it knows nothing about the order.
 */
export function describeInvoiceLine(status, unit = "", { hasDelivery = false } = {}) {
    const speaksOfDelivery = !status || hasDelivery;

    return {
        verdict: speaksOfDelivery
            ? STATUS_COPY.detail.verdict[invoiceVerdictKey(status)](status, unit)
            : null,
        againstOrder:
            status && typeof status.ordered === "number"
