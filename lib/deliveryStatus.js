 ...
 * axis's `poLineDelivery`, and the level `summarizePOInvoicingStatus` folds.
 *
 ...
 * WHICH bill, and at what price — is the per-item question, and the per-item
 * `Order variance` mark (#179) is what carries it.
 *
 * BILLED BEYOND THE ORDER COUNTS AS BILLED, which mirrors the delivery axis
 * exactly: `poLineDelivery` asks only whether the quantity was reached, and the
 ...
 * say the same thing.
 */
export function poLineInvoicing({ orderedQty, invoicedQty, committedQty } = {}) {
    const ordered = orderedQty || 0;
    const invoiced = invoicedQty || 0;
 ...
 */
export function summarizePOInvoicingStatus(lines) {
    const judged = (lines || []).map(poLineInvoicing).filter((line) => line.counts);
    const ordered = judged.length;
    const complete = judged.filter((line) => line.complete).length;
    const anyInvoiced = judged.some((line) => line.anyInvoiced);
...
export const STATUS_COPY = {
    ...
    column: {
         ...
         * than collision: `STATUS_COPY.column.delivery` answers "has this arrival
 ...
 * so the subject agrees in number. Whether a `status` is one bill's share of one
 ...
 * The two orderings stay apart on the question they answer — which bill carries an
 * excess, against which bills to read first — and this one adds no comparator.
 */
...
/**
 * The two states an unmatched bill can be in (#256).
 *
 ...
 * refusal, with nothing having arrived when both writes happened, and with the
 ...
 * bill charges, and that is one batched read of `PO Items."Delivery Items"` rather
 * than the ~5 reads per invoice `getArrivalsForBill` costs — the per-row shape #143
 * ruled out and #162 measured at over 200 calls. So the split is by arrival, not by
 * reason, and the reader is told where to look rather than what happened.
 */
export const AWAITING_DELIVERY_KIND = {
    /** Nothing has been delivered against any ordered item this bill charges. */
    noDeliveryRecorded: "no-delivery-recorded",
    /** Something has, and no delivery is matched to this bill. */
...
export const AWAITING_DELIVERY_COPY = {
    ...
    explain: "Longest wait first. Nothing has confirmed the material these bills charge for.",
    kind: {
...
/**
 * The bills waiting on a delivery, longest first (#256).
 *
 ...
 * this strip is narrower than the chip it selects on. Such a bill can never be
 * paired — it is `fitRefusal`'s `noOrderedItem` — and the arrival question cannot be
 ...
 *
 * THE SHARED TIE-BREAK IS INERT ON THIS AXIS, STATED RATHER THAN LEFT TO BE FOUND.
 * `sortLongestWaitingFirst` breaks a tie on `createdAt` descending, and `Invoices`
 * has no creation timestamp at all — the table carries no `Created At` and the
 * mapper exposes none — so two bills issued on one day hold their input order.
 * `Invoice ID` is what would serve: its date half is the day the record was created
 * rather than the day the vendor issued it, so descending by ID is the same
 * "most recently entered first" the delivery side gets from `createdAt`. Using it
 * needs the tie-break property generalized the way `waitingSince` was, across the
 * same two other call sites, which is a second rename this issue was not asked for.
 * Nothing on a screen moves either way: it decides only the order of same-day rows.
 */
export function selectInvoicesAwaitingDelivery({
    ...
    for (const invoice of invoices || []) {
        ...
        rows.push({
            ...
            // No `createdAt`: the field does not exist on `Invoices`. See above for
            // what that costs and what would serve instead.
