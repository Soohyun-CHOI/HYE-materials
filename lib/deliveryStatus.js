 ...
 * unpaid chips alone.
 *
 * `dueDate < today`, SO THE DUE DAY ITSELF IS NOT YET LATE. Same direction as
 * `lib/authTokenState.js`'s `expiresAt < now`: the boundary instant belongs to the
 * side that still has time. A blank `Due Date` is NOT overdue — there is nothing to
 * have passed — which is #263's call on a null wait carried one field across, and it
 * is reachable: `Due Date` is optional on BOTH invoice write paths (neither form
 * marks the input `required` and neither action refuses a blank, unlike `Issue
 * Date`, which both do), so an invoice this axis cannot judge for lateness is an
 * ordinary record rather than a hand edit.
 */
export function invoicePayment({ paid, dueDate } = {}, today) {
    const isPaid = Boolean(paid);
    return {
        paid: isPaid,
        overdue: !isPaid && Boolean(dueDate) && Boolean(today) && dueDate < today,
    };
}
