 ...
 * seven characters are on every tool item and separate none of them. The floor's
 * own width goes from 25.5 mm to 15.0 mm with it, which leaves 24.81 mm of the
 * label's usable width unclaimed beside the widest symbol this stock absorbs.
 *
 * WHAT THAT DID NOT CHANGE IS WHICH DIMENSION BINDS, and it is worth stating so the
 * next pass does not re-derive it: the height does, and it did before. The widest
 * symbol is 23.37 mm against 23.4 mm of usable height, so every `fits` verdict is
 * unchanged by this number moving — measured at 33, 37, 41 and 45 modules a side.
 * The width is where the room went, and #412 is what spends it.
 */
