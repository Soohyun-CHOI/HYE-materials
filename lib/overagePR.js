 ...
 * `Reattached Delivery Items` reverse-link names the rows whose excess came from THIS
...
export async function getOverageBannerFactsForPO(po, poItems) {
    ...
    const rowIds = [...new Set((poItems || []).flatMap((it) => it.reattachedDeliveryItems || []))];
    ...
    return rows
        .map((row) => {
            ...
            if (!state) return null;
            const bill = selectOverageBill(billsByItem.get(resolveOriginalPOItem(row)) ?? [], row.qty).bill;
            return {
                site: "originalPO",
                state,
                rowId: row.id,
    ...
    for (const row of rows) {
        try {
            ...
            await reattachDeliveryItemToPOItem(row.id, {
                ...
                originalPOItemRecordId: originalItemId,
            });
