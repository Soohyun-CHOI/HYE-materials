...
//     `Not fully invoiced` worklist as well as showing `not-applied`.
// Re-attach runs FIRST for exactly that reason: the reachable middle state is the
// one two things notice rather than one.
//
// No email can be sent — Resend is still in sandbox mode — so those two are the
// whole of the notification story, and both are derived from links rather than
// stored.
//
 ...
 * PO Item record id.
 *
 * Two batched reads whatever the row count — the invoice lines off the ordered
...
export async function getOverageContext(deliveryItems, { deliveryId } = {}) {
    const rows = (deliveryItems || []).filter((r) => r.overDelivery || r.overagePRRecordId);
    const out = new Map();
    if (rows.length === 0) return out;

    // The ORIGINAL ordered item in every state — the row's own link before the
    // apply step, the provenance link after it.
    const orderedItemIds = [...new Set(rows.map(originalPOItemRecordId).filter(Boolean))];
    ...
    for (const row of rows) {
        const orderedItem = poItemById.get(originalPOItemRecordId(row)) ?? null;
        ...
        const bill = eligibility.bill ?? selectOverageBill(bills, row.qty).bill ?? null;
...
export async function getOverageBannerFacts(pr) {
    ...
    const [deliveries, poItems] = await Promise.all([
        ...
        getPOItemsForReconciliation([...new Set(rows.map(originalPOItemRecordId).filter(Boolean))]),
    ]);
    ...
    return rows
        .map((row) => {
            const orderedItem = poItemById.get(originalPOItemRecordId(row)) ?? null;
            const bill = selectOverageBill(billsByItem.get(orderedItem?.id) ?? [], row.qty).bill;
 ...
 *
 * `poItems` are the PO's own lines, which the detail page already holds. Their
 * `overageDeliveryItems` reverse-link names the rows whose excess came from THIS
...
export async function getOverageBannerFactsForPO(po, poItems) {
    ...
    const rowIds = [...new Set((poItems || []).flatMap((it) => it.overageDeliveryItems || []))];
    ...
    return rows
        .map((row) => {
            ...
            if (!state) return null;
            const bill = selectOverageBill(billsByItem.get(originalPOItemRecordId(row)) ?? [], row.qty).bill;
            return {
                site: "originalPO",
                state,
                rowId: row.id,
...
export async function applyOverageToPO({ pr, poRecordId }) {
    ...
    for (const row of rows) {
        try {
            ...
            const originalItemId = originalPOItemRecordId(row);
            if (!originalItemId) {
                skipped.push({ rowId: row.deliveryItemId, reason: "no ordered item" });
                continue;
            }

            // WHICH line of the overage PO. Matched on #18's `Material` link, never
            // on `Item Name` text. A single-item overage PO is unambiguous anyway,
            // which is the shape the button produces and the fallback for a PO whose
            // cache refresh did not run.
            const target =
                newItems.find((it) => it.material?.[0] && it.material[0] === row.material?.[0]) ??
                (newItems.length === 1 ? newItems[0] : null);
            if (!target) {
                skipped.push({ rowId: row.deliveryItemId, reason: "no matching line on the overage PO" });
                continue;
            }
            ...
            await reattachDeliveryItemToPOItem(row.id, {
                ...
                overageOfPOItemRecordId: originalItemId,
            });
...
/**
 * Move the billed excess from the original ordered item's invoice line onto the
 * overage order's.
 *
 ...
 *
 * TWO BRANCHES, because a bill whose whole line IS the excess would otherwise be
 * left at qty 0: such a line is RE-POINTED rather than split, so the invoice keeps
 * one line for it and the items table has nothing to fold.
 */
async function splitInvoiceLineForOverage({ originalItemId, target, excess, overagePORecordId }) {
    ...
    const { bill } = selectOverageBill(bills, excess);
    if (!bill) return { split: false, reason: "no single bill can carry the excess" };

    // The lines whose Variance Flag the split changes, collected as they are
    ...
    // on the original line, and asserting that without measuring it would be a
...
/** One line's Variance Flag, on createInvoiceAction's own terms. */
