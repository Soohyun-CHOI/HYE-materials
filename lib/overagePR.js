// The read and write sides of the overage request (#167).
//
// lib/overage.js decides; this fetches the facts it needs and performs the two
// writes: raising the Draft, and — once its PO exists — moving the
...
import { getMaterialByRecordId } from "./airtable/materials";
...
import { WAIT_COPY, WAIT_STAGE, waitStage } from "./prWait";
import {
    OVERAGE_COPY,
 ...
 *
 * Called only for rows that are flagged or already carry a request, so an
 ...
 * `{ eligibility, awaitsOverageRequest, bannerState, noLongerOverDelivered, invoice,
...
export async function getOverageContext(deliveryItems, { deliveryIds } = {}) {
    ...
    // The requests behind the ordered items, for the chain the overage request copies.
    ...
    for (const row of rows) {
        ...
        out.set(row.id, {
            ...
            awaitsOverageRequest: awaitsOverageRequest({ row, overagePR, overagePO }),
            // #272 — the OTHER half of that selection: a row whose request is still an
            // unsubmitted Draft stays on the strip without being offered again. The two
            // were one test, and the day someone abandoned a draft was the day the
            // excess became invisible (lib/prWait.js).
            stillWaiting: overageStillWaiting({ row, overagePR, overagePO }),
            waitStage: waitStage(overagePR),
            // Who holds that draft, for the chip that says so. FREE: the active users
            // are already read for the signer chain above, so this is a lookup rather
            // than a query — and a requester who is no longer active simply falls back
            // to the nameless voice.
            holderName:
                (overagePR?.requester?.[0] &&
                    activeUsers.find((u) => u.id === overagePR.requester[0])?.userName) ||
                null,
            ...
            originalPR,
            facts: {
                // The chain the request will arrive with, so the preview can say
                ...
                // #217 — which stage a live request has reached, for the refusal
...
/**
 * Every over-delivery on the viewer's jobs still waiting for a request, longest
 * wait first — the strip above `/prs` (#217).
 *
 * STILL WAITING IS WIDER THAN STILL OFFERABLE SINCE #272. A row whose request is
 * an unsubmitted Draft is listed with the holder's name and no button; only a row
 * nothing covers gets the control. See lib/prWait.js for the day this cost an
 * excess its only reader.
 *
 ...
 * recipient of an edit request, none of whom need a job assignment. The
 ...
 */
export async function getOveragesAwaitingRequest(jobs) {
    ...
    // Flagged or already covered, which is getOverageContext's own population: the
    // covered ones are read so `awaitsOverageRequest` can drop them for the right
    ...
    for (const row of flagged) {
        ...
        // #272 — LISTED, not offered. `awaitsOverageRequest` decides the button below;
        // this decides whether the reader sees the row at all, and the difference is a
        // row someone has drafted and not submitted.
        if (!context?.stillWaiting) continue;
        ...
        stripRows.push({
            ...
            // #272 — a draft-held row's chip names the person rather than a refusal,
            // because what a reader does about it is go and ask them. It is deliberately
            // NOT an `alreadyRaised` entry in OVERAGE_COPY.strip.reason: that key means
            // "nothing to do here" and this row is one somebody is still expected to
            // finish, so the two would read alike and mean opposite things.
            reason: eligible
                ? null
                : context.waitStage === WAIT_STAGE.draft
                  ? WAIT_COPY.draftChip(context.holderName)
                  : OVERAGE_COPY.strip.reason[eligibility.blocked] ?? null,
            messages: eligible
...
/**
 * The banner facts for one PR — the overage request's own page.
 *
 * Costs nothing on an ordinary PR: `overageDeliveryItemRowIds` is empty on every
 * PR that is not an overage request, and this returns immediately.
 */
export async function getOverageBannerFacts(pr) {
    ...
    // One banner per covered row. A PR covers one excess in practice — the button
...
export async function getOverageBannerFactsForPO(po, poItems) {
    // The overage side first: this PO's PR is the overage request.
...
/**
 * The signing chain to copy onto the overage request, minus anyone inactive.
 *
...
/**
 * Raise the overage request's Draft for one over-delivery.
 *
...
export async function createOverageDraft({ user, delivery, row, orderedItem, invoice, originalPR }) {
    const pr = await createPR({
        requesterId: user.id,
        // #280 — `discipline`, AND THE READ WAS BROKEN BEFORE THE RENAME REACHED IT.
        // This was `originalPR.line?.[0]` until #227, whose blanket `line` → `item`
        // sweep rewrote it to `originalPR.item?.[0]` — a key `recordToPR` has never
        // had, so the expression has evaluated to `undefined` and every overage draft
        // since has been created with no link and therefore no Job. That is the
        // hazard `airtable-access.md` names ("a blanket substitution also hits a
        // different identifier that happens to share the old name") measured on a
        // real regression rather than on a near-miss, and this branch's own sweep is
        // what found it. Restored to the link, under its new name.
        disciplineId: originalPR.discipline?.[0] ?? null,
        vendorId: originalPR.vendor?.[0] ?? null,
        notes:
            `Overage request. ${row.qty} ${row.unit || ""}`.trim() +
    ...
    try {
        ...
        //
        // THE CATEGORY COMES OFF THE MATERIAL THIS ROW ALREADY LINKS (#356).
        // Every other field here is copied from a record; the category was the
        // one an automatic Draft had no source for, because #355 put the picker
        // on the request form and nobody fills this one in. The row's `Material`
        // is the same link `applyOverageToPO` matches the overage order's item
        // on, so this is the link that is already load-bearing here.
        //
        // WHERE THE GUARANTEE COMES FROM, AND WHERE IT STOPS. Since #356 a
        // material's identity IS its category, so `upsertMaterial` cannot create
        // one without it and every material on the base has one: if there is a
        // material, it has a category. What is not guaranteed is a material at
        // ALL — `collectMaterialsCacheEntries` skips an ordered item with no
        // Unit or no numeric Unit Price, so its `Material` link is never
        // written, and an excess delivered against such an item reaches here
        // with nothing to read.
        //
        // AND THEN THE DRAFT IS RAISED WITHOUT ONE, WHICH IS NOT A SHORTCUT. A
        // half-picked item row is a state #355 built the form to re-open and
        // `createItem` documents as one it must keep accepting; this Draft lands
        // on `/prs/new?draft=…`, so the requester meets the picker with
        // everything else already filled. Refusing instead would drop an excess
        // the company has been invoiced for and has to chase — the state #217's
        // strip exists to surface — in exchange for a field one screen away.
        const material = row.material?.[0] ? await getMaterialByRecordId(row.material[0]) : null;
        ...
        const item = await createItem({
            ...
            categoryRecordId: material?.category?.[0] ?? null,
        });
 ...
 * the invoice usually arrives and is settled before anyone raises the request.
 *
...
async function splitInvoiceItemForOverage({
    ...
    // splits has to be the one the request quoted, or the invoice item moved would
