 ...
 * row: the ordered items, their bills, those bills' invoices, the linked PRs, the
 ...
 * `{ eligibility, awaitsCorrection, bannerState, facts, bill }`.
 */
 ...
 * under `canViewPR`, these rows are arrivals under `canAccessJobDeliveries`, and the
...
export async function getUncorrectedOverages(jobs) {
    ...
    for (const row of flagged) {
        ...
        stripRows.push({
            ...
            messages: eligible
                ...
                : [],
            inferredLabel: eligible ? inferredLabel(eligibility) : null,
        });
    }

    // `Received Date` ASCENDING — the same key and the same function the deliveries
    // list and #216's strip order by, so one arrival appears in the same position on
    ...
    // (measured), and the arrival date wins because the excess is a fact about the
    // arrival and that is the date every other screen shows for it.
