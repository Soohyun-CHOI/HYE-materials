...
// Which store an object belongs to — NOT the ingest signal (see isIngested below for
// that). Used here only to skip a URL that was never ours to delete: a Draft
// re-opened later carries Airtable's own attachment URL instead of a Blob one
// (lib/prDraft.js), and re-saving it must not try to delete anything. The predicate
// lives in lib/fileSource.js since #438, which is also where it stopped admitting
// every Vercel customer's store.
import { isOurBlobUrl } from "./fileSource";
