...
// IT FOLDED, so an invoice whose item an overage split divided showed one material
...
// rather than to an invoice, so two invoice items against ONE ordered item carry
// the same
...
//
// #278 TOOK THE SECOND TONE OUT OF THIS LIST. An invoice item with no ordered item
// behind it
// took `unjudged` and made this list's own distinction — gray says nothing was
// measured, amber says something is wrong — and that is not a state this app
...
export function foldedEntryShare(members) {
    ...
    for (const member of judged) {
        ...
        // An ordered item's own excess, once however many invoice items reach it.
