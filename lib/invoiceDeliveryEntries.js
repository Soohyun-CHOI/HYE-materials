...
// were two — a corrective order exists — is an order fact that #232 removed from this
...
// THE WHOLE DESIGN. A share is CLAMPED at what its bill billed
// (`invoiceShareStatus`), and that clamp is per bill per ordered item: a delivery may
// legitimately bring more of an ordered item than this invoice charges, and that
// surplus is the delivery axis's fact rather than this bill's. So a folded entry ADDS
...
// ITEMS. `billedBeyondOrder` and `arrivedBeyondOrder` belong to a `PO Items` row
// rather than to a bill, so two charges against ONE ordered item carry the same
...
// the section says that once and shows no entries at all — `Not compared — no ordered
// item` included, since it explains an omission from a comparison that is not
// happening. A share with `delivered: 0` cannot tell "nothing is matched" from "the
// matched delivery brought none of this", so the caller states which it is.
//
...
// (`lib/deliveryStatus.js`), so `not-compared` stays `unjudged` and its name is as gray
// as its sentence; an entry that speaks ONLY through the order-scoped aside has no
// verdict to read and is `exception`, since something exceeding an ordered item is what
// put it in a list that holds nothing else. The aside itself stays uncolored: it is a
// fact about the ordered item rather than about this bill, which is #232's distinction
...
import { describeInvoiceLine } from "./deliveryStatus.js";

/**
 * The share for one folded entry: its members' shares, added.
 *
 * `members` are reconciliation rows. Returns null when none of them was judged,
 * which is a not-compared entry — a group of one by construction, since a row with
 * no ordered item has no `Material` and therefore keys on its own record id
 * (`foldKey`). A group is homogeneous for the same reason: a member carrying a
 * material got it from the ordered item behind it, so it was judged.
 *
 * The four bill-scoped fields add per member. The two order-scoped ones add over
...
export function foldedEntryShare(members) {
    ...
    for (const member of judged) {
        ...
        share.billedNotArrived += member.status.billedNotArrived || 0;
        share.arrivedNotBilled += member.status.arrivedNotBilled || 0;

        // An ordered item's own excess, once however many charges reach it. A row
        // whose `PO Item` link resolved to nothing never gets here — it took the
        // not-compared row and carries no status.
        if (countedOrderedItems.has(member.poItemId)) continue;
        countedOrderedItems.add(member.poItemId);
        share.arrivedBeyondOrder += member.status.arrivedBeyondOrder || 0;
        share.billedBeyondOrder += member.status.billedBeyondOrder || 0;
    }
...
/**
 * How many ordered items one folded entry covers — what the order-scoped line's
 * subject agrees with, and the same set its two figures were added over.
 */
...
export function invoiceDeliveryEntries({ folded, rows, hasDelivery = false } = {}) {
    ...
    return (folded || [])
        ...
        .map((group) => {
            ...
            // anything about. Left out rather than allowed to fall through to
            // `Not compared`, which would be a sentence about an invoice item that
            // is not there — the walk returns a row per invoice item, so this is
            // unreachable and is a guard rather than a case.
            if (members.length === 0) return null;
            const status = foldedEntryShare(members);
            const orderedItemCount = orderedItemsCovered(members);
            const lines = describeInvoiceLine(status, group.unit || "", {
                hasDelivery,
                orderedItemCount,
            });
            ...
            return {
                ...
                tone: lines.verdict?.tone ?? "exception",
            };
        })
        .filter((entry) => entry && (entry.lines.verdict || entry.lines.againstOrder));
}
