...
// THE WHOLE DESIGN. A share is CLAMPED at what its invoice billed
...
// ITEMS. `billedBeyondOrder` and `deliveredBeyondOrder` belong to a `PO Items` row
...
// fact about the ordered item rather than about this invoice, which is #232's distinction
...
export function foldedEntryShare(members) {
    ...
    for (const member of judged) {
        ...
        share.billedNotDelivered += member.status.billedNotDelivered || 0;
        share.deliveredNotBilled += member.status.deliveredNotBilled || 0;
