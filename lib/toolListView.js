 ...
 * take the half that divides the read, which `pageOfToolItems` says why.
 *
 ...
 * list on a phone with a real warehouse in it — and it is the design's to move again
 * when the list is drawn again, which is why docs/briefs/tools-toolRecordId.md
 * states it.
 *
 ...
 * four operations, and a fifty-first row is a second query. The print: the screen's
 * print control sends the tool items on the page it is showing, so this is also how
 * many labels one press asks for, and the label screen prints
 * `MAX_LABELS_PER_REQUEST` at a time — a page larger than that is a press that does
 * not print its page. `offline/tool-list-view.mjs` holds the size under both.
 *
 * IT IS NOT `LIST_PAGE_SIZE`, AND SINCE #442 THE TWO ARE BOTH 25 — WHICH IS THE
 * STRONGEST REASON NOT TO FOLD THEM RATHER THAN A REASON TO. This list is read at a
 * phone width (#336), its row is three short facts, and its page is also a print
 * run; the document lists are read at a monitor with six columns. Each figure was
 * set for its own list, and the two landing on one number is two decisions agreeing
 * today. Folding them makes a design change to either move the other. **The one way
 * they become one number is a design decision that they are one reader's page**,
 * taken where #258 names the design's values, with this paragraph and the one on
 * `LIST_PAGE_SIZE` changed in the same commit — the check fails a fold made any other
 * way. `/materials` has a 25 of its own too, and that is a bound on what a search
 * asks the base for rather than a page at all.
 */
