...
// the tool item's own page to be its caller; that page builds one instead, because
// the endpoint's two Airtable operations were a session read and a tool item read it
...
// went. So the two screens that draw a symbol both call this function, and the ONLY
// thing that reads a tool item for the purpose is whichever of them is rendering.
// docs/notes/tools.md records what that invalidated of #351.
 ...
 *
 * MEASURED, AND `M` IS FREE OVER `L`. At the 31-character production address both
 ...
 * 5x5 while `M` still decodes at 7x7. `Q` and `H` cost a version each (29 modules a
 * side, so modules 16% thinner at a fixed sticker size) and buy nothing where a
 * sticker actually fails: one flipped module inside a finder pattern leaves no
 * symbol to find at `L`, `M`, `Q` and `H` alike. Error correction protects the data
 * region; it does not protect the corners that abrade first.
 *
 ...
 * boundary and the shortened one leaves seven characters spare; `H` used to cost
 * TWO versions and now costs one, which puts it level with `Q`. The deciding
 * measurement is untouched by any of that — a finder pattern is outside what every
 * level protects — so the conclusion stands on the argument that reached it rather
 * than on the arithmetic that moved.
 */
 ...
 *
 * SEVEN CHARACTERS OF HEADROOM SINCE #411, WHERE IT WAS ZERO. Version 2 holds
 * exactly 38 alphanumeric characters at this level; the address was exactly 38 and
 * is 31. What that buys is measured rather than described: the daily sequence can
 * reach five digits and the host can grow from `hyeusa.com` to seventeen characters
 * before 39 steps this to version 3. docs/notes/tools.md records what #348 had
 * calculated, what #351 measured instead, and what the headroom is for.
 */
 ...
 * far tighter than byte mode. The same 31-character address in its lowercase form
 * segments into two modes and needs version 3 — so #348's reason for printing the
 * whole URL in capitals is confirmed, at one version, and `/T/` resolves through
 * `next.config.mjs`'s rewrite. **#411 shortened the address and re-measured this
 * rather than inheriting it**: a shorter string buys no relief here, because what
 * costs the version is the mode split and not the length.
 *
 ...
 * label already glued to a tool. `labelPath` is also what drops the family token,
 ...
 * `https://hyeusa.com` explicitly. docs/notes/tools.md records that a label may not
 ...
 * now. A Vercel preview domain is long enough to still step a version, so the tell
 * survives for the case it was least needed in and is gone for the case it was most
 * needed in. **What stands between a person and a sheet of stickers pointing at the
 * wrong host is the sentence `/tool-items/labels` prints above the sheet**, which is
 * why that sentence is not decoration and may not be moved below the fold.
 *
