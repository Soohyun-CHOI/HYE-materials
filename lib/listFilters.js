...
// `app/components/ListFilterBar.js` is what draws it from all of that.
 ...
 *
 * `page` WAS STILL RESERVED AND #326 SPENT IT. It is an entry in
 * `offline/url-parameters.mjs`'s inventory on all four routes now, under the SLICE
 * heading `/tools/[toolRecordId]` already occupies — the name was inherited from that
 * screen rather than coined, so the app has one name for one thing across five screens.
 * It stays out of `FILTER_PARAMS` because it is not an axis: it has no control kind, it
 * narrows nothing, and `parseFilters` has no options list to check it against. What it
 * divides is rows nobody asked to lose.
 */
export const FILTER_PARAMS = ["q", "job", "vendor", "status", "kind", "mine", "over"];
export const RESERVED_PARAMS = { page: "page" };

/**
 * How many rows one page of a document list holds.
 *
 * THE NUMBER APPEARS HERE AND NOWHERE ELSE, WHICH IS WHY `pageOfRows` EXISTS RATHER
 * THAN FOUR SCREENS SLICING FOR THEMSELVES. No list page reads this constant — each
 * calls that function and renders what comes back — so changing the size is one edit
 * with nothing to sweep, and `offline/list-filters.mjs` derives every length it
 * asserts from this rather than pinning 25 by value.
 *
 * IT IS A DESIGN VALUE AND THE DESIGN MAY MOVE IT. Nobody has held these lists at a
 * monitor with a year of documents in them; what is settled is that the four agree,
 * not what the figure is.
 *
 * IT IS NOT `TOOL_PAGE_SIZE`, AND SINCE #442 THE TWO ARE BOTH 25 — WHICH IS THE
 * STRONGEST REASON NOT TO FOLD THEM RATHER THAN A REASON TO. That screen is used at a
 * phone width (#336), its row is three short facts, and its page is also how many
 * labels one press of its print control asks for; these four are read at a monitor
 * and their rows are six columns. Each figure was set for its own list, and the two
 * landing on one number is two decisions agreeing today. `/materials` has a 25 of its
 * own and that is a THIRD thing again — a bound on what Airtable is asked for, not a
 * slice of rows already in hand. Folding any two makes a design change to one move
 * the other. **The one way two of them become one number is a design decision that
 * they are one reader's page**, taken where #258 names the design's values, with this
 * paragraph changed in the same commit; `offline/tool-list-view.mjs` fails a fold of
 * the two page sizes made any other way, and `docs/notes/deliveries-and-invoices.md`
 * records all three.
 */
export const LIST_PAGE_SIZE = 25;

/**
 * Which rows one page holds, and where that page sits.
 *
 * ONE FUNCTION FOR THE SLICE AND THE CLAMP, which is `pageOfToolItems`' shape (#339)
 * and the same reason: a caller that slices for itself can slice past the end and
 * render an empty screen for a page that exists.
 *
 * IT TAKES ROWS RATHER THAN IDS, WHICH IS THE HALF THIS ISSUE CANNOT COPY FROM THERE.
 * That screen chooses its page before anything is fetched, so only that page is read.
 * These lists cannot: most admit rows through `canViewPR`, whose ordered rules are not
 * expressible as a query, so a page of RECORDS read from the base is not a page of
 * ROWS a reader sees. The read therefore stays whole and this divides what survived
 * it — after the server's gate and after the bar has narrowed.
 *
 * A PAGE PAST THE END IS THE LAST ONE AND AN UNREADABLE PAGE IS THE FIRST. A URL is
 * typed, edited and copied, so the parameter is a request rather than a promise, and
 * answering it with an empty screen would show a reader nothing and tell them nothing.
 * **The caller then has to mirror the RESOLVED page back into the address** — see
 * `useListFilters`, which is where this differs from the tools screen: clamping alone
 * leaves `?page=99` in the bar over a screen showing page 2.
 *
 * AN EMPTY LIST IS PAGE 1 OF 1, not page 0 of 0. There is one page and it is empty,
 * which is the empty state's business rather than the paging's.
 */
export function pageOfRows(rows, rawPage) {
    ...
    return { page, pageCount, total, rows: all.slice(from, from + LIST_PAGE_SIZE) };
}
...
/**
 * What the foot of a paged list says (#326).
 *
 * TWO SUBJECTS, TWO LINES, AND THEY ARE NOT THE COUNT IN THE BAR ABOVE. `count` up
 * there is the FILTER's own arithmetic — what the bar removed — and renders only while
 * something is active. These are about the LIST: how many rows it holds and which slice
 * of them is on screen. A reader who has filtered nothing gets no `N of M` and still
 * has to be able to tell a whole list from the beginning of one, which is the defect
 * #326 names and the reason these render unconditionally.
 *
 * `total` COUNTS WHAT IS BEING PAGED, WHICH IS THE ROWS AFTER THE BAR. Handing it the
 * rows before would put a figure on screen that no amount of paging reaches, and the
 * two are equal exactly when nothing is filtered.
 *
 * THE POSITION IS STATED WHETHER OR NOT THERE IS A SECOND PAGE, which is the tools
 * screen's reading of the same requirement: a position that appears only once a list
 * is long leaves the short case saying nothing, and the short case is the one where a
 * reader most needs to know they are seeing everything.
 *
 * THE TWO STEPS ARE ABSENT AT THE ENDS RATHER THAN DRAWN DEAD, so a one-page list
 * carries neither and the foot is two sentences.
 */
export const LIST_PAGE_COPY = {
    total: (route, n) => `${n} ${n === 1 ? LIST_NOUN[route].one : LIST_NOUN[route].many}`,
 ...
 */
/**
 * SINCE #326 IT CARRIES BOTH NUMBERS, because the footer that says how many rows a
 * list holds can be handed a one. The narrowing sentences below read `many` and are
 * byte-identical to the ones #324 shipped; nothing about them moved.
 */
const LIST_NOUN = {
    "/prs": { one: "purchase request", many: "purchase requests" },
    "/pos": { one: "purchase order", many: "purchase orders" },
    "/deliveries": { one: "delivery", many: "deliveries" },
    "/invoices": { one: "invoice", many: "invoices" },
};

/** The shape all three narrowing sentences share: what was asked for, and nothing left. */
const narrowed = (route, asked) => `No ${LIST_NOUN[route].many} match ${asked}.`;
...
/**
 * The whole query string a screen's state produces: the bar, then the page (#326).
 *
 * THE PAGE GOES LAST AND ONLY WHEN IT IS NOT THE FIRST, which is #324's own convention
 * rather than a new one — absence is the unapplied state, nothing ever encodes "all",
 * and a bare URL is the whole list from the top. So `?page=1` is a third spelling of a
 * screen that already has two, and this normalizes it away.
 *
 * IT IS A SEPARATE FUNCTION RATHER THAN A BRANCH INSIDE `filterQuery`, because that one
 * answers "what did the bar ask for" and has four callers' worth of meaning riding on
 * it. A page is not an axis and putting it in there would make the axis walk carry
 * something that is not one.
 *
 * THAT THE PAGE IS APPENDED TO A FRESHLY BUILT BAR QUERY IS WHAT RESETS IT. Nothing
 * here carries a page forward: the caller passes the page it resolved, and every
 * setter in `useListFilters` puts that back to 1 before this is called again.
 */
export function listQuery(route, state, page) {
    const qs = filterQuery(route, state);
    const n = Number(page);
    if (!Number.isInteger(n) || n <= 1) return qs;
    return qs ? `${qs}&${RESERVED_PARAMS.page}=${n}` : `${RESERVED_PARAMS.page}=${n}`;
}
