 ...
 * these four documents holds, and the reader's own is the third wherever the document
 * has an author to compare against. Then the list's own state axes.
 *
 * `/invoices` HAS NO READER'S-OWN TOGGLE, AND THAT IS A FACT ABOUT THE SCHEMA RATHER
 * THAN A JUDGMENT ABOUT THE SCREEN. `Deliveries` carries `Recorded By` and
 * `Purchase Requests` carries `Requester`; `Invoices` carries neither, and #382 is
 * the issue that would give it one. Nothing here should grow a toggle for it before
 * that field exists — there is no axis to read.
 *
 ...
 *
 * `Recorded by me` IS A DIFFERENT WORD BECAUSE IT IS A DIFFERENT FIELD.
 * `Deliveries."Recorded By"` is not a requester, and the participle follows the
 * field name, which is `docs/notes/naming.md`'s convention for a checkbox.
 */
