 ...
 *
 * IT IS NOT `TOOL_PAGE_SIZE` AND THE TWO ARE DELIBERATELY APART. That screen is used
 * at a phone width (#336) and its own note left this issue the choice; these four are
 * read at a monitor and their rows are six columns rather than three short facts, so
 * the two are not one reader's page. `/materials` has a 25 of its own and that is a
 * THIRD thing again — a bound on what Airtable is asked for, not a slice of rows
 * already in hand. `docs/notes/deliveries-and-invoices.md` records all three, because
 * two of them carrying the same value today is the kind of coincidence a later reader
 * folds into one constant.
 */
