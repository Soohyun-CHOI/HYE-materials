// Picking an address — how a form asks, what it offers, and every word it says.
//
// TWO SCREENS SINCE #387, AND THE HEADER SAID ONE. This was `/prs/new`'s alone
// (#385): where a request's material goes. `/deliveries/new` now asks the same
// question of the same table — where a delivery was delivered — and takes
// `addressOptions` and six of the strings below. **What it does NOT take is the
// branch machinery**: `jobDefaultAddressId`, `addressChoiceFromStored` and
// `chosenAddressId` are all about a default that comes from the JOB, and a
// delivery's comes from the orders it attaches to (`lib/deliveryAddress.js`). So
// the two forms share the list, the grouping and the vocabulary, and each owns its
// own answer to "what should this open on".
//
