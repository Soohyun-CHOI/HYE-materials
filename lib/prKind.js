...
/**
 * The kinds `/prs` can be filtered to, each with the mark it already wears (#324).
 *
 * TWO AND NOT THREE, BECAUSE ORDINARY HAS NO WORD AND MUST NOT ACQUIRE ONE HERE. The
 * chip above is deliberately `null` for it — "a word on every row makes the
 * exceptional rows ordinary" — and a select offering `Ordinary` would coin exactly
 * that word, in the one place a reader would then meet it. What the filter offers is
 * the two marks the list actually renders, which is also why the labels are read off
 * `chip` rather than written again: a mark and the option that narrows to it cannot
 * be two words for one thing.
 *
 * `/prs`'s status select already offers four of five values for the same class of
 * reason — a Draft never reaches this list — so a select that is not a partition of
 * the rows is this screen's existing shape rather than a new one.
 */
export const PR_KIND_CHOICES = [PR_KIND.overage, PR_KIND.directPurchase].map((kind) => ({
    value: kind,
    label: PR_KIND_COPY.chip[kind],
}));
