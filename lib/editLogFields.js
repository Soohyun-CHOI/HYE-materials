...
// both checks rest on. `PR Edit Log."Field"` is a singleSelect with no `typecast`
...
// with it (see lib/airtable/prEditLog.js on the blast radius). An enumeration that
 ...
 *
 * `itemName` LEFT AND `category` TOOK ITS PLACE (#367), AND THE SWAP IS TWO
 * THINGS AT ONCE. What a signer can change is the CATEGORY — the name is
 * composed from it and written in the same update — so a turn that changed an
 * item writes one row saying the item changed rather than two saying a string
 * was retyped. Its `Old Value` and `New Value` are still the two names, because
 * that is the text a reader of the history knows the item by and the one the
 * purchase order would have printed.
 *
 * AND DIFFING `itemName` WAS A HOLE, WHICH IS WHY THE KEY HAD TO GO RATHER THAN
 * MERELY STOP BEING USED. #355 made the form's name field read-only and left the
 * diff reading `itemsJson.itemName` — but a Server Action is directly callable,
 * so the value the action wrote was still whatever the submission carried. The
 * lock was the screen's, not the code's. With the key gone, the only thing that
 * can write `Item Name` on this path is the label of the category that was
 * picked.
 *
 * `Item Name` IS STILL A FIELD AND ITS LOG ROWS WOULD STILL BE ITS OWN. Nothing
 * here says otherwise: the enumeration below is what the CODE CAN SEND, and a
 * label leaves it when no code path can write it. #181's rule about a choice
 * with rows behind it is what governs the Airtable side — measured at the swap,
 * `PR Edit Log` held no rows at all, so the choice was renamed in place rather
 * than left as history nothing points at.
 */
