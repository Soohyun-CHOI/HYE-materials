...
// the same import. #40 added `describeBytes` from `lib/uploadLimit.js` on the same
// terms: import-free, and already in the client bundle through five upload forms.
// #440 added `isRequester` from `lib/prRequester.js`, on the same terms again: the
// requester half of the gate below is that module's question.
import { formatUSD } from "./format.js";
import { isRequester } from "./prRequester.js";
import { describeBytes } from "./uploadLimit.js";
...
export function canSendPOToVendor(user, pr) {
    ...
    return isRequester(user, pr);
}

/**
 * The largest order document the send will try to mail, in bytes (#40).
 *
 * DERIVED FROM RESEND'S LIMIT, NOT CHOSEN. Resend's documentation, read 2026-09-24:
 * "Emails can be no larger than 40MB (including attachments after Base64
 * encoding)." Base64 writes 4 bytes for every 3, so 40 MB encoded is 30 MB of
 * document. Taking the strictest reading of "40MB" — 40,000,000 bytes — and allowing
 * for the line break a MIME part puts after every 76 encoded characters, which is 78
 * bytes on the wire per 57 of document:
 *
 *     40,000,000 × 57 / 78 = 29,230,769 bytes of document
 *
 * less the mail's body and headers, a few hundred bytes. 27 MiB, 28,311,552 bytes, is
 * the largest whole number of MiB under that, and it encodes to 38,742,124 bytes. The
 * figure is not measured: Resend documents no error for an oversized mail, and
 * measuring one needs a real send.
 *
 * WHY IT EXISTS AT ALL. Until #40 no order document could come near this: one
 * quotation of at most `MAX_UPLOAD_BYTES` (20 MiB) behind the order's own two pages
 * encodes to about 28 MB. Every quotation is appended now, so two large ones or a
 * handful of phone photographs pass it — and past it the send failed with a sentence
 * saying to try again, recorded nothing, and left the button to fail the same way on
 * every press.
 *
 * STRICTLY GREATER refuses, `lib/uploadLimit.js`'s convention: a document of exactly
 * this size is sent.
 */
export const MAX_SEND_DOCUMENT_BYTES = 27 * 1024 * 1024;

/**
 * Why an order cannot be sent, keyed so the page and the action cannot disagree.
 *
 * FIVE SINCE #40, AND `already-sent` IS NOT ONE OF THEM. Being already sent is not a failure —
 * it is the answer to the question the presser asked, and it has its own voice below.
 *
 * `too-large` IS THE ONE NOBODY ON THE PAGE CAN CURE, which is why it says what to do
 * instead rather than what to fix. The document is what it is — it carries the
 * vendor's own quotations, and #281 refuses to replace a document that exists — so
 * the order reaches the vendor some other way or not at all. And the app records
 * only what it sent itself (#281 declined a manual mark), so the order goes on
 * reading as not sent, which the sentence says rather than leaving a reader to find.
 */
export const SEND_REFUSAL = {
    ...
    "too-large":
        `This PO's document is larger than the ${describeBytes(MAX_SEND_DOCUMENT_BYTES)} an email to the vendor can ` +
        "carry, so it can't be sent from here. Download it above and send it another way — the order will still " +
        "show as not sent, since nothing here sent it.",
    "no-address":
 ...
 *
 * `too-large` COMES BEFORE `no-address` (#40), because a reader who added the
 * vendor's address only to be told the document can never be mailed from here would
 * have done work for nothing. It reads the attachment's own `size`, which the order
 * record already carries, so the judgment costs no read; a `size` Airtable has not
 * filled in yet refuses nothing, and the send is left to find out.
 *
...
export function getPOSendEligibility({ po, vendorEmail }) {
    ...
    if (po.poPdfFile[0].size > MAX_SEND_DOCUMENT_BYTES) {
...
export const SEND_COPY = {
    ...
    /**
     * What a reader who cannot send is told instead. Three of the five are reachable
     * on the page; the other two are answered higher up by the page's own shape and
     * are kept for a caller that does not come through it.
     */
