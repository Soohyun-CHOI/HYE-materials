...
//
// THIS FILE'S NAME IS NARROWER THAN WHAT IT HOLDS SINCE #362, AND THE DIVERGENCE
// IS RECORDED RATHER THAN FIXED. `assignedJobsFor`, `jobUnchosen` and
// `jobNotYours` are not about registering anything — they are the tools axis's
// rule for which job an actor may file an event against, and lib/toolTransition.js
// is the second consumer. Narrow is not wrong, which is the test docs/notes/
// naming.md applies to a file name (#181): the name makes no claim the contents
// contradict, it just stops short of them. Moving them costs a module of their
// own plus its line in CLAUDE.md's service-layer list, in an issue that already
// spends two trimmed lines to add one — and it would not take `noJob` with it,
// since that sentence names the act and the two acts differ.
//
// THE CONDITION FOR MOVING IS A THIRD CALLER, AND #363 IS IT. That is
// `countsAsOrdered`'s mechanism exactly (docs/notes/verification.md): it sat in
// lib/materialPriceView.js with this condition written down until #169 was the
// third consumer and moved it to lib/poItemQty.js. Retiring a tool item writes a
// `Tool Log` row, every row carries a job from the actor, so that issue reaches
// this function — and it inherits a trigger that has already fired rather than
// having to re-derive one.
...
/**
 * The jobs this person may register a tool item against, out of a loaded list.
 *
 ...
 * no job has no such job, so there is nothing for them to register against and
 * widening this would invent one.
 *
...
export const TOOL_REGISTRATION_COPY = {
    ...
    // Measured in a browser: with the label reused, the control read
    ...
    jobNotYours: "Pick a job you are assigned to.",
