// Which order an invoice item was billed against, where that differs (#237).
//
// AN INVOICE'S PAGE NAMES THE ORDERS IT BILLS AND USED TO STOP THERE, so a bill
...
// rule that way is that it needs no case for a corrective order:
//
//   one order                     {A}, {A}       — agree, silent
//   corrective order, all split   {A,B}, {A,B}   — agree, silent
...
// can bill orders this page does not show. So a line carries the quantity, the one
 ...
 * `qtyByOrder`, the quantity this item was billed for against each. An item with no
 * ordered item behind it has an EMPTY `orderRecordIds` — which is what takes it out
 * of the list and out of the comparison below.
 */
export function ordersNamedByFoldedItem({ folded, items } = {}) {
    ...
    return (folded || []).filter(Boolean).map((group) => {
        ...
        for (const rowId of group.rowIds || []) {
            ...
            // No ordered item behind it, so there is no order to name. The `PO` link
            // it does carry is not read here on purpose — see this file's header.
 ...
 * them — one order, or a corrective order every item is split across. It is also
 ...
 * `shown` first.
 */
export function billedItemsByOrder({ folded, items } = {}) {
...
export const ORDER_BREAKDOWN_COPY = {
     ...
     * invoice billed against THAT order. No price and no amount — see the header.
     */
    billed: (f) => ({
        key: "order-billed",
