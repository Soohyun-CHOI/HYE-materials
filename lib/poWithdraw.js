...
/**
 * The only two statuses a withdrawal can start from. "Sent to Vendor" is
 * deliberately absent: by then the order has gone out and cancelling it
 * involves the vendor, which is not something this app can model (see
 * CLAUDE.md's follow-up note). Policy, not vocabulary — the Status option
 * names themselves live with the table module (purchaseOrders.js).
 */
 ...
 * one voice can't quietly leave the other describing the old behaviour.
 *
 ...
 *
 * Status is checked FIRST, and that ordering is load-bearing: a Sent to
 * Vendor PO that also has invoices must not be told "ask an Admin to
 ...
 * PO, not per line). The Invoice Items reverse-link only contributes to the
 ...
 * join row, and a PO with invoice lines pointing at it is not withdrawable
 ...
 * would refuse a Sent to Vendor PO (not withdrawable, but the most normal
 * thing to invoice) and refuse a second invoice against a partly invoiced
 * PO (partial invoicing is routine). What they share is the status name,
 * not the rule.
 */
