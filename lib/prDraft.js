...
export async function loadPRDraft(prId) {
    ...
    return {
        ...
        // #385 — the stored link, left as a record id. Which BRANCH of the form
        // wrote it is not stored and cannot be: the request keeps an address
        // either way, so `lib/addressChoice.js:addressChoiceFromStored` derives
        // the branch against the job's own default when the form seeds itself.
        deliveryAddressId: pr.deliveryAddress?.[0] || "",
        shippingFee: pr.shippingFee ?? "",
        notes: pr.notes || "",
        items: orderedItems.map((it) => ({
            // #355 — the stored link, left as a record id for the form to
            // resolve against the tree it already holds. Resolving it here
            // would mean a read per draft item for a value the client has in
            // memory; `lib/airtable/materialCategories.js` says why there is
            // deliberately no reader for it.
            categoryRecordId: it.category?.[0] || "",
            // Kept for a row saved BEFORE the catalog, which carries a typed
            // name and no link. The form has no name input any more, so this is
            // what tells such a row apart from a blank one.
