...
export const DELETE_COPY = {
    plain: {
        ...
        body: (deliveryId, lineCount) =>
            `${deliveryId} and its packing list photo are removed for good, and the ${lineCount} ordered ` +
            `${lineCount === 1 ? "item" : "items"} it was recorded against ${lineCount === 1 ? "goes" : "go"} ` +
    ...
    invoiced: {
        ...
        body: (deliveryId, lineCount) =>
            ...
            `${lineCount === 1 ? "this ordered item" : "these ordered items"} is not changed — deleting this only removes the ` +
            `record that the material arrived, so until you enter it again ${lineCount === 1 ? "that ordered item" : "those ordered items"} ` +
            `will read as invoiced with nothing delivered. This cannot be undone.`,
    },
    paid: {
        title: "Delete this delivery?",
        body: (deliveryId, lineCount) =>
            `${deliveryId} and its packing list photo are removed for good. ${lineCount === 1 ? "This ordered item is" : "These ordered items are"} ` +
 ...
 *
 * `paid` IS PRESIDENT-OR-ADMIN (#211), AND THIS IS WHERE THAT LINE WAS LEAKING.
 * Deletion is author-or-Admin on a Job-scoped record, so a site recorder deleting
 * their own arrival was being told, in a modal, that the vendor had already been
 * paid — the one invoice fact #211 keeps from them. So `seesPayment` decides
 * whether the third voice is reachable at all, and for everyone else the answer
 * stops at `invoiced`, which is still TRUE and still the strongest statement they
 * may be given: the invoice is not changed and the lines will read as invoiced with
 * nothing delivered. Nothing becomes uncomputable — the escalation simply stops one
 * step earlier, and it also stops one read earlier, since the invoices behind the
 * lines are then never fetched.
 *
 ...
 */
export async function resolveDeleteCopy(delivery, items, { seesPayment = false } = {}) {
    ...
    try {
        ...
        if (invoicedPoItemIds.length > 0) {
            voice = "invoiced";
            // Only asked when the answer may be given. Defaulting to false means a
            // caller that forgets the flag under-discloses rather than over-.
            if (seesPayment) {
                const invoiceLines = (
...
export async function deleteDeliveryAsUser({ deliveryRecordId, actingUser }) {
    ...
    try {
        ...
        // would have the flags recomputed against a line that is not what is
    ...
    await recomputeFlagsForLines(touchedPOItemIds);

    return { ok: true, deliveryId: delivery.deliveryId, jobRecordId: delivery.job?.[0] ?? null };
}

/**
 * Redraw the within/over boundary on every line the deleted delivery touched
 * (#206).
 *
 ...
 * `create()` per straddling line — at most one per line, since a strictly
 * increasing total crosses the ordered quantity once. An ordinary one-line
 * delete pays two reads and no writes at all. NOTHING IS EVER DELETED here: no
 * row is removed and no row is merged into another, so the only records this
 * touches are the ones it resizes and the ones it adds.
 *
 * RESIZE BEFORE CREATE, and the order is the failure mode rather than style. If
 * the create fails after the resize, the line has lost the excess from its
 * recorded total: material that arrived reads as not arrived, and the next
 ...
 * leaves the line reading as MORE delivered than arrived, which #169 records as
 ...
 * the same line redraws the boundary again.
 */
async function recomputeFlagsForLines(poItemRecordIds) {
    if (poItemRecordIds.length === 0) return;

    try {
        const lines = await getPOItemsForReconciliation(poItemRecordIds);
        const rowIds = [...new Set(lines.flatMap((line) => line.deliveryItems || []))];
        const stored = await getDeliveryItemsByRecordIds(rowIds);
        const rowById = new Map(stored.map((row) => [row.id, row]));

        for (const line of lines) {
            const lineRows = (line.deliveryItems || [])
                .map((id) => rowById.get(id))
                .filter(Boolean);

            const plan = recomputeOverDelivery({ orderedQty: line.qty, rows: lineRows });
