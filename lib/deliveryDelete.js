...
export async function resolveDeleteCopy(delivery, items) {
    ...
    try {
        ...
        if (invoicedPoItemIds.length > 0) {
            ...
            // #318 — the date, not a flag: `Invoices."Paid"` is gone and an invoice
            // with a `Paid Date` is the paid one. Same question, same answer on every
            // record this base carries.
            if (invoices.some((inv) => inv?.paidDate)) voice = "paid";
        }
    } catch (err) {
