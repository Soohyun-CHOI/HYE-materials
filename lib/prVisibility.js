...
// neither a signer nor an edit-request recipient is guaranteed to be assigned
// to the PR's Job, and a participant who cannot open the PR cannot take their
// turn — the chain stops there. Both are deliberately **status-agnostic**: a
// signer who already approved, one pushed back to Pending by an edit request,
// and the recipient of one that is already Resolved all keep access. Two
...
// user signs this PR. Same shape for `PR Edit Requests (Sent To)`. That
...
// want anyway (above); it would cost one read per edit-request row otherwise.
//
// Note on `Sent To` ambiguity: CLAUDE.md records that it holds a single user
// id and cannot say whether an edit request went to someone *as a signer* or
// *as the Requester* when they are both. That ambiguity is about the recipient's
// capacity, not their identity, and 6 only asks "is this user the recipient",
// so it does not reach this decision.

// #440 — who raised the request is `lib/prRequester.js`'s question and nothing here
// reads the field itself. Spelled with its extension: this module is offline-safe, and
// the tier runs under plain `node` with no loader to resolve an extensionless import.
...
function requireChainFields(user, pr) {
    const missing = [
        ...
        Array.isArray(user.editRequestRowIds) ? null : "user.editRequestRowIds",
        Array.isArray(pr.signerRowIds) ? null : "pr.signerRowIds",
        Array.isArray(pr.editRequestRowIds) ? null : "pr.editRequestRowIds",
    ].filter(Boolean);
...
export function canViewPR(user, pr) {
    ...
    if (pr.status === "Draft") return isRequester(user, pr);
    ...
    // sees their own.
    if (isRequester(user, pr)) return true;
    ...
    if (intersects(user.editRequestRowIds, pr.editRequestRowIds)) return true;
