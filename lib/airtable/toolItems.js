import { base, TABLES, findByFieldValues, findByRecordIds, findChildRecords, getLinkedRecords } from "./client";
 ...
 *
 * THE READER A SCANNED LABEL ARRIVES AT, through `/t/[toolItemId]`, which is
 * what a QR code carries and what redirects to `/tool-items/[toolItemId]` (#348).
 * Either way the id in the URL is the PRINTED one, so the lookup is by that value
 * and not by a record id — nothing on the sticker knows Airtable's own identifier.
 *
 ...
 * non-canonical segment to the stored id, so one tool item keeps one address.
 * **`/t/` canonicalizes before it hands the id over (#348)**, so traffic from a
 * label never reaches that redirect and a scan costs one hop rather than two;
 * what still reaches it is somebody typing this address directly.
 */
...
/**
 * Many tool items by PRINTED id, batched — one query per 50 (#353).
 *
 * `/tool-items/labels` IS THE CALLER, AND THE PRINTED ID IS WHAT IT HAS. A label
 * sheet is named by the ids a registration minted or a person read off a sticker,
 * so the reader beside this one — `getToolItemByToolItemId` — would be one query
 * per label, which is exactly the per-record shape #193 forbids. There is no parent
 * whose reverse-link would serve instead, since a printed id names no parent, so
 * this is `findByFieldValues`' second call site on the same reasoning CLAUDE.md
 * records for `getMaterialPrice`.
 *
 * **IT DOES NOT PRESERVE THE ARGUMENT'S ORDER AND IT DOES NOT THROW ON A MISS**,
 * which is where it parts from `findChildRecords`. A `filterByFormula` `OR` returns
 * rows in the table's own order, and a printed id that resolves to nothing is a
 * reachable state rather than a bug: somebody typed one, or a link was edited. The
 * caller matches the rows back onto its own list and says which ids it could not
 * find — the label screen names them rather than printing a short sheet in silence.
 */
export async function getToolItemsByToolItemIds(toolItemIds) {
    const records = await findByFieldValues(TABLES.TOOL_ITEMS, "Tool Item ID", toolItemIds);
 ...
 * `/tools/[toolRecordId]` chooses its page from the link array before
...
/**
 * Move the two cached fields onto the `Tool Log` row that was just written (#362).
 *
 * ONE CACHE OF ONE ROW, WHICH IS WHY THIS IS ONE FUNCTION AND NOT TWO. `Status`
 * and `Job` are both the last log row's — the status that event left behind and
 * the job it happened on — so writing one without the other is a tool item
 * describing two different moments. They go in one update, which is also one
 * operation.
 *
 * IT DOES NOT WRITE THE LOG ROW AND MUST NOT. `createToolLogEntry`'s own header
 * carries the argument from the other side: Airtable has no cross-table
 * transaction, so the row and the cache are two writes whichever way they are
 * arranged, and hiding one inside the other would make a pair that can half-fail
 * look atomic. The caller owns the ordering, and #338 and #362 are the two
 * callers that have one.
 *
 * THE STATUS COMES FROM `statusAfterEvent` AND NEVER FROM A LITERAL, the rule
 * `createToolLogEntry` already has for `Event` and `createEditLogEntry` for
 * `Field`. `Status` is a singleSelect written with no `typecast`, so a value
 * outside the option list fails the write rather than minting a fourth choice
 * off the color palette — and the Metadata API cannot repair an option list
 * afterwards (422, measured), so failing loudly is the only recovery there is.
 *
 * IT TAKES A JOB RECORD ID RATHER THAN DEFAULTING, because `Job` is required and
 * app-enforced: every event carries one, so an update with none is a caller bug
 * and not a blank to write.
 */
export async function updateToolItemCache({ toolItemRecordId, status, jobRecordId }) {
    if (!toolItemRecordId) throw new Error("updateToolItemCache: a Tool Item is required");
    if (!status) throw new Error("updateToolItemCache: a Status is required");
    if (!jobRecordId) throw new Error("updateToolItemCache: a Job is required");

    const record = await base(TABLES.TOOL_ITEMS).update(toolItemRecordId, {
        Status: status,
        Job: [jobRecordId],
    });
