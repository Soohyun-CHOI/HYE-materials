 ...
 *
 * #339's SHAPE, AND THE REASON IS RECORDED BECAUSE THIS READER HAS NO CALLER
 * YET. That issue's kind list needs a count per status, and `docs/notes/tools.md`
 * settled where it comes from: the `Tool Items` link array `getAllTools` already
 * returns for free. Flattening those arrays across every tool and reading them
 * here is `1 + ceil(total/50)` operations, against one call per tool otherwise —
 * so this is the function that makes the no-rollup decision affordable. A sweep
 * for uncalled readers should leave it; #340 deleted `getToolItemByRecordId` in
 * the same commit precisely because nothing was going to reach for that one.
 */
