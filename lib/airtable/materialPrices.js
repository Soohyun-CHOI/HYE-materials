...
function recordToMaterialPrice(record) {
    return {
        id: record.id,
        // `priceLabel` STOOD HERE AND IS GONE (#182). It is this table's primary, a
        // formula over the two links, and nothing ever printed it — a price row is
        // reached through its material and its vendor, both of which the caller
        // already holds by the time it has the row.
 ...
 * are two records and must not serialize against each other, while two ordered items
