...
function recordToDeliveryItem(record) {
    return {
        ...
        // #167 — where the excess came from, written in the same update as the
        // re-attachment. Before that update it is empty and `poItem` IS the
        // original, so every reader wants `overageOfPOItemRecordId ?? poItem[0]`.
        overageOfPOItemRecordId: (record.get("Overage Of PO Item") || [])[0] ?? null,
    };
}
 ...
 *
 * `Overage Of PO Item` is the provenance the re-attachment would otherwise destroy:
...
export async function reattachDeliveryItemToPOItem(deliveryItemRecordId, {
    poItemRecordId,
    overageOfPOItemRecordId,
}) {
    const record = await base(TABLES.DELIVERY_ITEMS).update(deliveryItemRecordId, {
        "PO Item": poItemRecordId ? [poItemRecordId] : [],
        "Overage Of PO Item": overageOfPOItemRecordId ? [overageOfPOItemRecordId] : [],
        "Over Delivery": false,
    });
    return recordToDeliveryItem(record);
}
