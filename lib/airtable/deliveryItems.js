...
/**
 * One allocated slice of a delivery (#162).
 *
 ...
 * ALWAYS there to compare a delivery against, and an invoice usually is but not
 * always. The reason is availability, not order of delivery — a vendor emails the
 * invoice when it ships and the material turns up afterwards, so the invoice is
...
function recordToDeliveryItem(record) {
    return {
        ...
        // #167 — the overage request raised for this row's excess, if any. Read as
    ...
    const record = await generateChildId(
        ...
        (deliveryItemId) =>
            base(TABLES.DELIVERY_ITEMS).create({
                ...
                // IT SAID "Empty only on an unattributable over-delivery row" AND
                // THAT PATH WENT IN #165 — allocation attaches every row it plans and
                // refuses to plan when it cannot, so no caller passes a null link.
                // Corrected in #278, which found the comment describing a producer
                // three issues dead. The coalesce stays: this is the boundary a
                // hand-emptied link is read back through, and `reattachDeliveryItemToPOItem`
                // below shares the shape.
...
/**
 * Link this row's excess to the overage request that covers it (#167).
 *
 ...
 * changing them changes what the delivery was allocated against, and there is no
 ...
 * able to re-point a delivery by hand. This is not that: it is the settlement of a
 * overage purchase order that was approved and signed, so the allocation is
