...
/**
 * Many invoice lines by record id, batched (#166, exported in #167).
 *
 * The ids come from a `PO Items."Invoice Items"` reverse-link, so a whole level of
 * bills is fetched in one query per 50 rather than one per line — the per-row round
 * trip #143 ruled out. #166's join walked this level with a private copy of this
 * reader; #167 needs the same level for the overage affordance, so it moved here
 * rather than becoming a second one that could drift.
 */
...
export async function updateInvoiceItem(
    ...
    // Issue #167 — the PO a line reconciles against can move exactly once: when an
    // overage correction takes over a bill whose WHOLE line was the excess, the
    // line is re-pointed rather than split, so it must follow the ordered item to
    // the new order. Both links move together or the line would name a PO Item
    // belonging to a different PO.
