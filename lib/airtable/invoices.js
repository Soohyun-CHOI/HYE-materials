...
function recordToInvoice(record) {
    return {
        ...
        // Issue #382 — who typed this invoice in, so a reader looking at a figure
        // that seems wrong has somebody to ask. An ARRAY for the reason every link
        // on this base is one: the Metadata API refuses `prefersSingleRecordLink`,
        // so single-record is the app's rule and not the schema's, and `|| []` is
        // what the two tables beside it already return.
        //
        // EMPTY IS A READING RATHER THAN A GAP, and it will stay reachable: every
        // invoice entered before this field existed has none, and nothing backfills
        // them — the base records no fact that would say who.
        ...
        // `signerRowIds`/`editRequestRowIds` and recordToPO for its children: both
 ...
 *
 * Issue #382 — `recordedByUserId` is the session's own user and is written HERE
 * and nowhere else. `updateInvoice` below takes named parameters only, so there
 * is no edit path to this field at all: who entered the invoice is not who last
 * touched it, and the absence of a parameter is what makes that a property
 * rather than a habit. `createDelivery` and `createDirectPurchase` write their
 * own `Recorded By` in exactly this shape.
 *
 * OPTIONAL, AND THE SEEDS ARE WHY. A seed script has no session, so the person
 * it named would be a fiction; omitting it leaves `[]`, which is the state every
 * invoice on this base is already in.
 */
export async function createInvoice({
    ...
    // Issue #405 — the last line before an invoice's header figures are stored, and
    // the backstop `lib/airtable/invoiceItems.js` has carried for the item half since
    // #254. `HEADER_TOLERANCE` is half a cent because both sides of its comparison are
    // whole numbers of cents; three of these four are terms of one side and the fourth
    // is the other side, so a sub-cent figure here is the premise failing rather than a
    // tidiness problem. Throws rather than rounds, for the reason `lib/variance.js`
    // gives: rounding would restate what a caller said the vendor's document says.
    assertWholeCentPrice("createInvoice", shippingFee, "Shipping Fee");
    assertWholeCentPrice("createInvoice", tariff, "Tariff");
    assertWholeCentPrice("createInvoice", salesTax, "Sales Tax");
    assertWholeCentPrice("createInvoice", amountDue, "Amount Due");
    // Issue #438 — the same shape one field over: `createInvoiceAction` refuses an
    // invoice file that is not on our Blob store in a sentence, and this is the last
    // line before the url reaches Airtable, which would fetch whatever it points at.
    assertOurBlobFiles("createInvoice", file);
 ...
 *
 * Issue #382 — `Recorded By` IS ABSENT FROM THIS SIGNATURE ON PURPOSE. It names
 * who typed the invoice in, which an edit does not change, and the way to hold
 * that is to give the edit path no parameter for it rather than to guard one.
 * See `createInvoice` above, which is its only writer.
 */
export async function updateInvoice(
    ...
    // Issue #405 — the same four, and the abstention is what makes one guard serve a
    // PARTIAL update: `isWholeCentPrice` answers true for `undefined`, so the callers
    // that write only `paidDate` or only `varianceFlag` are asking about nothing and
    // are refused nothing. `null` abstains too, which is how `Tariff` and `Sales Tax`
    // are cleared.
    assertWholeCentPrice("updateInvoice", shippingFee, "Shipping Fee");
    assertWholeCentPrice("updateInvoice", tariff, "Tariff");
    assertWholeCentPrice("updateInvoice", salesTax, "Sales Tax");
    assertWholeCentPrice("updateInvoice", amountDue, "Amount Due");
