 ...
 * every bill on an ordered line to order them, including bills the caller never
 * asked about.
 */
...
function recordToInvoice(record) {
    return {
        ...
        // every invoice's lines in ONE batched read instead of one read per row.
        // Same trick recordToPR uses for `signerRowIds`/`correctionRowIds` and
        // recordToPO for its children: both sides of a link are core data with no
        // propagation lag, so the ids are already here and cost nothing.
        invoiceItems: record.get("Invoice Items") || [],
        // Issue #210 — the shipment this bill describes. An ARRAY because the
        ...
        // awaiting delivery, which is the ordinary state of a bill the vendor
        // emailed at shipment.
 ...
 * replaceDeliveryPhoto: the one field a record lets a neighbour change, written by
 ...
 * delivery's own edit page detaches rather than swapping, so a bill never moves
 * between shipments in one write nobody reviewed.
 *
