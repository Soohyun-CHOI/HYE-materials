 ...
 *
 * `Job` IS ON EVERY ROW AND IS NEVER BLANK, holding the job this tool item belongs
 * to immediately AFTER the event. That is the copy `Tool Items."Job"` cannot be —
 * a tool outlives the project it was bought for, so the current job says nothing
 * about where it was in March. And because there are no blanks, the PREVIOUS row's
 * `Job` is unambiguously the previous job: a `Job Changed` row names where it went
 * and the row before it names where it came from, so nothing here stores a
 * `Former Job`. That was the rejected alternative — see docs/notes/tools.md — and
 * it was rejected because it would be one fact in two places, derivable from the
 * ordering the log already has. #340 renders the whole history at once and so
 * holds both rows.
 */
 ...
 * #341 own the ordering, and `statusAfterEvent` owns which value.
 */
