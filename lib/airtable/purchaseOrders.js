...
function recordToPO(record) {
    return {
        ...
        // Lookup — read-only. NO READER AND DELIBERATELY SO (#182): it is in
        // `offline/file-route.mjs`'s `EVERY_ATTACHMENT_KEY` as the key `/api/files`
        // must NOT serve for a purchase order, which is `poPdfFile` one line down.
        // So a check names it, and removing it would take the name out of that
        // enumeration rather than out of the app.
        quotationFile: record.get("Quotation File"),
        ...
        // Issue #281 — the three facts of one event, written in the same operation
        // as Status -> "Sent to Vendor" and never rewritten (a second send is
        // refused). `sentAt` is the one the send predicate reads, because it is the
        // fact where Status is the stage. `sentTo` is what the app actually mailed
        // to, which `Vendors."PIC Email"` cannot answer later if it changes.
        sentAt: record.get("Sent At"),
        ...
        // the per-item reverse-link, used only as a safety net.
        invoicePoLinks: record.get("Invoice-PO Link") || [],
        invoiceItems: record.get("Invoice Items") || [],
        // Issue #19 — the ordered-item reverse-link, same category as the two
        ...
        // Issue #386 — where this order's material goes, a frozen copy of the
        // request's own link taken at generation. A link array like every other
        // one on this mapper, app-enforced single-record, and EMPTY on an order
        // generated from a request raised before #385 required the address.
        //
        // IT REPLACED A SELECT RATHER THAN RETYPING ONE, which named which of a
        // job's two addresses the order used and could name nothing once #384
        // removed the second slot. A type change is refused by the Metadata API,
        // and the code that writes this one has to WRITE it — so an ordering in
        // which the committed code is already correct does not exist, which is
        // what #356's retype had and this did not. It is a new field beside the
        // old one, and the select is inert until it is deleted by hand
        // (docs/notes/purchase-orders.md).
 ...
 * THAT GROUND IS FALSE and the base disproved it: `HYE-PO-260805-02` was Awaiting
 * Signature and carried an invoice. Two normal paths produce it. Site staff order
 ...
 * `Committed Qty` and `Signed Qty` are separate fields. And an overage order (#167)
 * exists *because* material already delivered, so the excess invoice can precede it
 * by construction — the vendor charges for what it shipped.
 *
 ...
 * `getAllVendors()`, same pattern as the PR detail page's Job/Vendor/Discipline
 * resolution.
 */
 ...
 * `Invoice ID`. A PO ID is `HYE-PO-YYMMDD-##`, fixed width and zero-padded
 ...
 * uninvoiced. So it grew with every order the company had ever placed and not
 ...
 *
 * `deliveryAddressId` (issue #386) is the second value of that kind and takes
 * `shippingFee`'s shape exactly: a plain copy of the request's own, frozen here
 * and never rewritten. It is OMITTED when there is none rather than written as
 * `[]`, which is the distinction the two `? […] : []` link parameters above do
 * NOT make — those clear a value the caller is authoritative about, and this one
 * is a value the request never had.
 */
export async function createPO({
    ...
    // #308 — THE FREEZE, ONE TABLE UP FROM `createPOItem`'s. The fee is a frozen copy
    // of the request's and it lands in `Total Amount` beside the item amounts, which
    // is the TOTAL line the vendor reads — so the same cent rule applies to it at the
    // same moment, in a second call site because it is a second table and a second
    // field. One judgment, two doors.
    assertWholeCentPrice("createPO", shippingFee, "Shipping Fee");
    ...
    const record = await generateNextPOId((poId) =>
        base(TABLES.PURCHASE_ORDERS).create({
            ...
            ...(deliveryAddressId ? { "Delivery Address": [deliveryAddressId] } : {}),
...
export async function updatePO(
    ...
    if (poPdfFile !== undefined) {
        // Issue #438 — the one writer of an attachment no caller supplies: the PDF is
        // generated and `put()` by lib/poPdf.js. Held to the same precondition as the
        // other five anyway, so "every attachment url is ours" has no exception to
        // remember when a second path to this field appears.
        assertOurBlobFiles("updatePO", poPdfFile);
        fields["PO PDF File"] = poPdfFile;
    }
    if (withdrawnAt !== undefined) fields["Withdrawn At"] = withdrawnAt;
    // Issue #281 — one send, one write, all four keys together, mirroring the
    // withdrawal's status-plus-timestamp pair. `Sent By` is a link field, so the
    // caller passes a record id and this wraps it: single-record is the app's
    // convention here rather than the schema's, because `prefersSingleRecordLink`
    // is refused on field create (see docs/notes/airtable-access.md).
    if (sentAt !== undefined) fields["Sent At"] = sentAt;
    if (sentBy !== undefined) fields["Sent By"] = sentBy ? [sentBy] : [];
    if (sentTo !== undefined) fields["Sent To"] = sentTo;
