import { base, TABLES, sumQty } from "./client";
...
export async function getAllPOs() {
    const records = await base(TABLES.PURCHASE_ORDERS)
        .select({
            filterByFormula: `{Status} != "Awaiting Signature"`,
        })
        .all();
...
/**
 * Server-side search over the *complete* non-`Awaiting Signature` PO set (issue #57's
...
export async function searchPOs(query) {
    ...
    const records = await base(TABLES.PURCHASE_ORDERS)
        .select({
            filterByFormula: `AND({Status} != "Awaiting Signature", SEARCH(LOWER("${escaped}"), LOWER({PO ID})))`,
        })
        .all();
 ...
 * unfulfilled item early in its list is cheap to confirm; the fetch
 * strategy is deliberately its own, but the `SUM(Qty)` arithmetic itself
 * is client.js's shared sumQty() (#15) — if that formula ever changes,
 * every caller picks it up together.
 */
export async function isPoOpen(poRecordId) {
    ...
    for (const poItemId of poItemIds) {
        ...
        if (invoiceItemIds.length > 0) {
            ...
            invoicedQty = sumQty(invoiceItemRecords);
        }
 ...
 * see isPoOpen()'s comment), so this still fetches every non-`Awaiting Signature` PO's
