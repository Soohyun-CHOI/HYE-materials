...
/**
 * SUM(Qty) over an already-fetched list of records — shared by
 * isPoOpen() (purchaseOrders.js), getInvoicingStatusByPO(), and
 * getInvoicedQtyForPOItem() (both poItems.js), which each fetch their
 * Invoice Item records differently on purpose (early-exit, parallel-per-
 * item, single-PO-Item scope respectively) but ultimately reduce the same
 * "Qty" field the same way — lives here rather than in poItems.js since
 * it's used from both the PO and Invoice sides, not owned by either table.
 */
export function sumQty(records) {
    return records.reduce((sum, record) => sum + (record.get("Qty") || 0), 0);
}
