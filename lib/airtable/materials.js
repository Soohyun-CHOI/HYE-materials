 ...
 * field, `<category path>_Size_Unit` since #356. Matching the LABEL rather than
 * the name alone is what makes `ball valve 2"` find a row regardless of the
 * order the words were typed. The tradeoff is that a token can match the size or
 * unit segment; see buildSearchTokens for why that is acceptable rather than a
 * defect.
 *
 ...
 * somebody had typed it on a request; it holds the catalog's path now, so the
