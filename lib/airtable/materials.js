...
/**
 * Search identity rows for #19's price screen. Tokens come from
 * lib/materialPriceView.js:buildSearchTokens and are AND-ed, each as a
 * case-insensitive substring of `Material Label` — the primary formula field
 * holding `Item Name_Size_Unit`.
 *
 * Matching the LABEL rather than `Item Name` alone is what makes `2" pipe` find
 * `Pipe_2"_EA` regardless of the order the two words were typed. The tradeoff is
 * that a token can match the size or unit segment; see buildSearchTokens for why
 * that is acceptable rather than a defect.
 *
 ...
 * would grow those OR-lists without bound. A truncated result is reported to the
 * caller rather than silently trimmed.
 */
export async function searchMaterials(tokens, { limit = 25 } = {}) {
    if (!Array.isArray(tokens) || tokens.length === 0) return { materials: [], truncated: false };

    const records = await base(TABLES.MATERIALS)
        .select({
            filterByFormula: andSearchAll("Material Label", tokens),
