...
/**
 * The fields a typed query is matched against (#416).
 *
 * `Material Label` is `<item name>_Size_Unit`, and the item name is the
 * category's OWN name since #416 — so the label no longer carries the path the
 * material sits under. **435 of the 777 names hold no word of their own Level 1**,
 * so a search for a branch would stop reaching the rows in it: `stainless` found
 * every SUS row through the label until this issue and would find almost none
 * after it. `Category Path` is the lookup that puts the path back within reach,
 * and it is searched WITHOUT being rendered — `/materials` shows the name, and
 * where a path belongs on a screen is the design pass's.
 *
 * The count is method-dependent — cutting Level 1 into words four reasonable ways
 * gives 425, 430, 431 and 461 — which is why `offline/material-price-view.mjs`
 * holds the PROPERTY rather than the figure: every Level 1's own words still
 * reach the rows under it.
 */
const SEARCHED_FIELDS = ["Material Label", "Category Path"];
 ...
 * each as a case-insensitive substring of `SEARCHED_FIELDS` joined — the label
 * and the category's path since #416. Matching the LABEL rather than the name
 * alone is what makes `ball valve 2"` find a row regardless of the order the
 * words were typed. The tradeoff is that a token can match the size or unit
 * segment; see buildSearchTokens for why that is acceptable rather than a
 * defect.
 *
 * EACH TOKEN MATCHES IN EITHER FIELD AND THE AND IS OVER THE TOKENS, which is
 * what `ball valve 2"` needs once the two are separate: the size is in the label
 * and the branch word may be only in the path, and no row carries both in one
 * field. See `andSearchAll`.
 *
 * WHAT THE TOKENS REACH CHANGED UNDER THIS QUERY AND THE QUERY DID NOT (#357).
 * The label held a requester's own spelling, so a word found a row because
 * somebody had typed it on a request; it holds the catalog's words now, so the
 * reachable words are the tree's. That makes the search more predictable and
 * less forgiving at once, and `docs/notes/materials.md` carries what was
 * measured and why a word-boundary rule was rejected.
 *
...
export async function searchMaterials(tokens, { limit = 25 } = {}) {
    ...
    const records = await base(TABLES.MATERIALS)
        .select({
            ...(browsing ? {} : { filterByFormula: andSearchAll(SEARCHED_FIELDS, tokens) }),
...
function recordToMaterial(record) {
    return {
        ...
        // #416 — the category's composed path, looked up beside the name. READ
        // BUT NOT RENDERED: it is what the search matches so a branch word still
        // reaches its rows, and no screen prints it.
        categoryPath: firstOf(record.get("Category Path")),
        // Single-record by app enforcement and by the hand toggle, a raw array
        // of record ids like every other link.
        category: record.get("Category") || [],
        size: record.get("Size"),
        unit: record.get("Unit"),
        // A reverse-link, written from the far side. `materialPrices` was its pair
        // and is gone (#182): nothing walked it, because the price level is read by
        // `getPricesForMaterials`, which queries `Material Record ID` for a whole
        // set of materials at once rather than following one row's array.
