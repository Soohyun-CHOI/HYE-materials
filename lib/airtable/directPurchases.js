...
// `getDirectPurchaseByRecordId` STOOD HERE AND IS GONE (#182). It was written as
// "the claim action's re-read" and that action never called it: `lib/
// directPurchaseClaim.js` reads through `getDirectPurchasesByRecordIds` below,
// which answers the same question for a whole strip in one query per 50 ids.
// So the per-row find was #193's shape from before #193, and nothing it claimed
// to cover is uncovered — the batched reader covers it for one id as readily as
// for fifty. This is the third instance of the defect #182 was raised over: a
// function written before its consumer, whose consumer then arrived by another
// route and left it standing.
 ...
 * never here (#140). Held here as well as by the action since #438, and to this
 * app's own store rather than to any Vercel customer's.
 *
...
export async function createDirectPurchase({
    ...
    assertOurBlobFiles("createDirectPurchase", file);
