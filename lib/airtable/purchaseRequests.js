 ...
 * record ids the API hands back. **#248 kept a row whose array CONTAINED the reader,
 * to answer exactly what the walk it replaced answered, and #440 moved it to
 * `lib/prRequester.js`'s first linked user**, the one meaning every screen and action
 * compares a reader against — the two differ only on a row naming two requesters,
 * which no write path produces. Sorted here rather than by the base, so a blank
 * `Created At` goes last.
 *
...
export async function getDraftsByRequester(userRecordId) {
    ...
    return records
        .map(recordToPR)
        .filter((pr) => requesterOf(pr) === userRecordId)
