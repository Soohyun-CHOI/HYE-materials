 ...
 * passed in by the caller. Total Amount is a rollup (read-only).
 */
export async function createPR({ requesterId, jobId, vendorId, notes }) {
    const prId = await generateNextPRId();

    const record = await base(TABLES.PURCHASE_REQUESTS).create({
        "PR ID": prId,
