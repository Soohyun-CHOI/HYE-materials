...
export async function createDelivery({
    ...
    const record = await generateNextDeliveryId((deliveryId) =>
        base(TABLES.DELIVERIES).create({
            ...
            // The counter in generateNextDeliveryId reads this field, so it must
            // be written in the same create() that consumed the count — see
            // lib/ids.js on why it is not Received Date.
 ...
 *
 * The item, the quantity, the vendor and the PO are absent for a different
