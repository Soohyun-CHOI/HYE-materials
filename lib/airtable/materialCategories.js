...
// THERE IS DELIBERATELY NO `getCategoryByRecordId`. A stored `PR Items` row
// carries a `Category` link, and resolving it looks like it needs a read — but
// every screen that shows one has already fetched the whole tree for its
// pickers, so the row's category is found in memory for 0 operations. Adding a
// reader for it would be the caller-less service function the header is about,
// one issue after CLAUDE.md's example of the same mistake.
