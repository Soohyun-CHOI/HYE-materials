 ...
 * is for.
 */
export async function getSignersByPR(prRecordId, { rowIds } = {}) {
    const records = rowIds
        ? await findChildRecords(TABLES.PR_SIGNERS, rowIds)
        : await getLinkedRecords(TABLES.PURCHASE_REQUESTS, prRecordId, "PR Signers", TABLES.PR_SIGNERS);

    // Neither read path sorts server-side — the batched one returns the parent's
    // link-array order and the walk it replaced returned the same (#193) — so
    // ordering by Sequence Order happens client-side here.
    return records
        .map(recordToSigner)
...
function recordToSigner(record) {
    return {
        id: record.id,
        // NO READER TODAY, AND KEPT for the reason `prEditLog.js` states at the
        // same position (#182): a row's minted id is what names it outside this
        // process, and every mapper here carries its own.
