...
export async function createPOItem({
    ...
    // #308 — THE FREEZE, AND THE ONE PLACE OUR OWN CODE MULTIPLIES THESE TWO. `Amount`
    // below is a STATIC currency value computed here, not a live formula like the
    // invoice item's, and it rolls into `Purchase Orders."Total Amount"`, which is the
    // TOTAL line on the PDF the office sends the vendor. Unreachable through this app
    // once `createItem` guards the request — which is exactly the point: the one way a
    // fraction still arrives is a hand edit in the Airtable UI, named in
    // `docs/notes/deliveries-and-invoices.md` as the case nothing catches, and this is
    // the last door before it leaves the building. A throw here rolls the generation
    // back and leaves the request Approved with no order, which `/pos`'s #176 strip
    // already surfaces and `generatePOAction` already retries — and that retry names
    // this refusal rather than saying `Please try again.`
    assertWholeQty("createPOItem", qty);
    assertWholeCentPrice("createPOItem", unitPrice);
