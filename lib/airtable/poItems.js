...
function recordToPOItem(record) {
    return {
        ...
        // Issue #167 — the Delivery Items rows whose excess came from THIS ordered
        // item, the reverse of Delivery Items."Overage Of PO Item". Delivery data,
        // not invoice data, so it is admissible on this employee-facing path for
        // the same reason `material` above is. It is what lets the ORIGINAL PO
        // render the overage banner precisely rather than walking the shared
        // Delivery, which would put the banner on an order that was not itself
        // exceeded whenever one arrival filled two of them.
        overageDeliveryItems: record.get("Overage Delivery Items") || [],
    };
}
 ...
 * which is why lib/deliveryReconciliation.js decides whether to walk this level at
 * all rather than fetching it and letting a page choose not to render it.
 *
 * #167 NARROWED THAT, and the narrowing is worth stating rather than leaving as a
 * contradiction between this comment and a caller. The overage affordance on a
 * delivery is Job-scoped — raising the corrective PR is site work, per the issue —
 * and deciding whether it can be raised needs the bills on the over-delivered
 * ordered item. So lib/overagePR.js reads this projection on a path site staff
 * reach. What #166 withholds is unchanged: the deliveries LIST's invoicing column
 * is still President-or-Admin and still not fetched for anyone else. What #167
 * exposes on the delivery DETAIL is narrower and deliberate — that this ordered
 * item is billed, by which invoice, and at what unit price.
 *
