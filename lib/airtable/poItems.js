...
function recordToPOItem(record) {
    return {
        ...
        // than bent. #211 held that what a vendor billed is readable by anyone who
        ...
        // already here in full — the same argument `deliveryItems` arrived on.
        //
        ...
        // fetch a whole level of arrivals in one batched read instead of none at
        ...
        //
        // THE `Invoice Items` ARRAY IS NOT HERE, and the line is #132's. An id
        // array carries no money, but its LENGTH answers "has this been billed,
        // and in how many pieces" — which is invoice-derived by the same standard
        // that keeps `Invoiced Qty` out of this mapper. It sits in
        // getInvoicingStatusByPO instead, which is where the privileged path
        // already reads `Invoiced Qty`. getPOItemsByRecordIds below draws the same
 ...
 * an arrival (#162) — so widening the shared mapper would push fields onto every
...
export async function getPOItemsByRecordIds(poItemRecordIds) {
    ...
    return records.map((record) => ({
        ...
        // #211 settled: what a vendor billed is readable by anyone who may read the
        ...
        // Delivery Items in one batched read. The `Invoice Items` array is still NOT
        // here — a chip needs the total, not the rows, and the level below belongs to
        // the caller that renders it — see getPOItemsForReconciliation below.
        deliveryItems: record.get("Delivery Items") || [],
    }));
}
...
export async function getPOItemsForReconciliation(poItemRecordIds) {
    ...
    return records.map((record) => ({
        ...
        // summing only the invoice in hand would report material as unbilled when
        // it is billed twice over.
        invoicedQty: record.get("Invoiced Qty"),
 ...
 * over-allocate the NEXT arrival to an ordered item that is already full.
