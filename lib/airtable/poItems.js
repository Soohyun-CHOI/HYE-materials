...
function recordToPOItem(record) {
    return {
        ...
        // employee path. "Invoiced Qty"/"Committed Qty"/"Signed Qty" are
        // deliberately absent: the first is invoice data (#132), and the other
        // two exist to feed the Materials rollups inside Airtable, with no
        // reader in the app.
...
/**
 * PO Item lines by record id, batched, in the shape the item axis needs (#19).
 *
 ...
 * on the document axis reads them. The material axis reads both — the status to
 * LABEL a line and Committed Qty to JUDGE whether it counts as ordered — so
 * widening the shared mapper would push two fields onto every PO detail render
 * that page has no use for.
 *
 * Batched by RECORD_ID() via findByRecordIds rather than by the `PO Record ID`
 * lookup, which on this table is misconfigured and returns the PO's Invoice
 * Items link array instead of its record id (measured, #19 — see CLAUDE.md).
 */
