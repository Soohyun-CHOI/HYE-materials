import { base, TABLES, getLinkedRecords, sumQty } from "./client";
...
/**
 * Sum of Qty across all Invoice Items currently linked to a single PO
 * Item, scoped to one PO Item and read fresh right after a new Invoice
 * Item is linked, so that line is already included. Kept as its own small
 * fetch rather than reusing getInvoicingStatusByPO()/isPoOpen()'s fetch
 * strategies (see isPoOpen's comment on why those two are deliberately
 * separate) — only the sumQty() formula above is actually shared.
 */
...
export async function getInvoicingStatusByPO(poRecordId) {
    ...
    return Promise.all(
        items.map(async (item) => {
            ...
            const invoicedQty = sumQty(invoiceItems);
