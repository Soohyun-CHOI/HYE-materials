...
export async function getJobByRecordId(recordId) {
    ...
    return {
        ...
        // #384 — the job's DEFAULT delivery address, and the only address field
        // left on this table. `Alternate Delivery Address` was the second slot and
        // is gone: a job uses as many addresses as it needs and they are reached
        // from `Addresses."Jobs"`, so adding a third place to ship to no longer
        // means adding a field. `lib/addressCreation.js:addressesOnJob` is the one
        // rule that reads both links together.
        deliveryAddress: record.get("Delivery Address"),
        ...
        // fetch. `disciplines` is the first step of the Job -> Disciplines -> PRs ->
        // PO Items walk that finds a Job's ordered items, since PO Items carries
 ...
 * (Requester picks a Job first, then a Discipline under it — see
 * lib/airtable/disciplines.js:getAllDisciplines()).
 */
export async function getAllJobs() {
    ...
    return records.map((record) => ({
        ...
        // callers would otherwise have paid to find the same ids. `disciplines` is
        // the first step of the Job -> Disciplines -> PRs -> POs -> PO Items walk, and
        // having it here is what lets that walk batch across every accessible Job
        // at once instead of per Job.
        disciplines: record.get("Disciplines") || [],
        deliveries: record.get("Deliveries") || [],
        // Issue #272 — a third reverse-link on the same terms, and the same
        // saving: the strip above `/prs` lists the direct purchases waiting for a
        // request on the jobs a viewer can reach, and `filterByFormula` cannot
        // match a link field, so the ids have to come off the Job. This select
        // already returns them.
        //
        // NOT ON `getJobByRecordId`, which has no caller for them. `disciplines` and
        // `deliveries` are on both because #162 gave both readers; a reverse-link
        // nobody reads is a field to keep in step for nothing.
        directPurchases: record.get("Direct Purchases") || [],
        // #384 — the job's default address, on the same terms: this select passes
        // no `fields`, so the link array is already on every record and exposing it
        // costs no query. `/addresses/new` reads it beside `Addresses."Jobs"` to
        // list what a job already ships to, which is the pair `addressesOnJob`
        // takes the union of.
        deliveryAddress: record.get("Delivery Address") || [],
    }));
}
 ...
 * doesn't take Discipline info — Disciplines are created separately under a Job,
 * via lib/airtable/disciplines.js:createDiscipline().
 */
