...
export async function getAllJobs() {
    ...
    return records.map((record) => ({
        ...
        jobCode: record.get("Job Code"),
        jobName: record.get("Job Name"),
        // Issue #162 — the reverse-link the deliveries list reads. This select
        // passes no `fields`, so Airtable already returns it on every record:
        // exposing it costs no query and saves one per Job, which is what the
        // list would otherwise have paid to find the same ids.
        deliveries: record.get("Deliveries") || [],
    }));
}
