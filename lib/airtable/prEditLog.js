 ...
 *
 * THE TABLE WAS `Edit Log` UNTIL #333, AND ONLY THE PREFIX MOVED. `edit` is the
 * word that survived — `PR Edit Requests` beside this table is the request and
 * this is the result of the same act — so nothing here is renamed for the word.
 * What the prefix reaches is everything that ADDRESSES the table: the `TABLES`
 * key, this file's own name, the primary field, and the two reverse links on
 * `Purchase Requests` and `Users`. What it deliberately does not reach is the
 * exported names: `lib/airtable/purchaseRequests.js` exports `getPRById` and
 * `recordToPR` rather than spelling its table out, so a function here is named
 * for the concept in the app's own words and `getEditLogByPR` already says whose.
 * `lib/editLogFields.js`, `EDIT_LOG_FIELD_LABELS` and `createEditLogEntry` keep
 * their names for the same reason, which is what left only the request side to
 * move.
 *
 ...
 * is for.
 */
export async function getEditLogByPR(prRecordId, { rowIds } = {}) {
    const records = rowIds
        ? await findChildRecords(TABLES.PR_EDIT_LOG, rowIds)
        : await getLinkedRecords(TABLES.PURCHASE_REQUESTS, prRecordId, "PR Edit Log", TABLES.PR_EDIT_LOG);
...
function recordToEditLogEntry(record) {
    return {
        id: record.id,
        // NO READER TODAY, AND KEPT (#182). A row's own minted id is the only
        // handle that survives leaving this process — an Airtable URL, a support
        // question, a log line. Every other mapper on this base carries its row's
        // id for that reason, and the one that dropped it would be the one whose
        // rows cannot be named. The audit's other findings had no such reason and
        // went; this is the reason, written down rather than assumed.
        editLogId: record.get("PR Edit Log ID"),
        pr: record.get("PR"),
 ...
 * PR Edit Log ID is backend-generated as {PR ID}-{seq}.
 */
    ...
    const record = await generateChildId(
        {
            ...
            parentLinkFieldName: "PR Edit Log",
            childTableName: TABLES.PR_EDIT_LOG,
            prefix: prId,
        },
        (editLogId) =>
            base(TABLES.PR_EDIT_LOG).create(
                {
                    "PR Edit Log ID": editLogId,
                    PR: [prRecordId],
                ...
                // the PR Item field, `Shipping Fee` when #69 added it — sat off
                // the palette the other six walk until someone fixed them BY
                // HAND. Measured on this field: the Metadata API refuses
                // `options.choices` outright (422), so no code here can clean up
                // after itself, which is what made the mess outlive two issues.
                //
                ...
                // and two checks are the pin, one per direction (#181):
                // offline/edit-log-fields.mjs fails in CI when a label is ADDED
                // without the choice, and verify-edit-log-fields-181.mjs is the
                // only thing that can see a choice DELETED in Airtable — which
                // has happened, by hand, since no API can do it.
            )
    );
