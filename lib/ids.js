 ...
 * given parent, then returns "{PREFIX}-{seqPrefix}{padded count+1}".
 *
...
export async function generateChildId({
    ...
    // A link field itself can't be compared to a record ID in
    // filterByFormula (Airtable formulas see a link field as its linked
    // record's display text, not its record ID) — so every child table has
    // a "{parentFieldName} Record ID" lookup field (via a RECORD_ID()
    // formula on the parent) specifically so this filter can be precise.
    const matching = await base(tableName)
        .select({
            filterByFormula: `{${parentFieldName} Record ID} = "${parentRecordId}"`,
        })
        .all();
