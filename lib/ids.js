...
/**
 * Generates the next Delivery ID: HYE-DL-YYMMDD-##, resetting each new day.
 * Top-level daily-reset counter, same shape as PR ID/PO ID/Invoice ID, and the
 * same lock-then-count-then-create for the same race reason.
 *
 * IT COUNTS `{Created At}`, WHICH THE APP WRITES, AND NEVER `{Received Date}`.
 * That is the whole reason Deliveries carries a Created At at all. Received Date
 * is the date on the packing list — human-entered, and routinely earlier than
 * the moment of entry, since material arrives late afternoon and gets recorded
 * the next morning. Counting on it would mean the ID's date (today) and the
 * counted population (rows whose Received Date is today) are different sets, so
 * the count would almost always be 0 and every delivery entered on one day would
 * get -01.
 *
 * That is not hypothetical: generateNextInvoiceId above counts `{Issue Date}`,
 * the vendor's own date off their document, and has exactly that defect — see
 * CLAUDE.md's unfiled-work entry. It has not surfaced only because every invoice
 * in the base so far happens to have been entered on its issue date. Deliveries
 * would surface it immediately, so this counter reads the field nobody can
 * backdate, matching generateNextPRId.
 */
export async function generateNextDeliveryId(createFn) {
    const dateStr = todayYYMMDD();
    const lockKey = `${TABLES.DELIVERIES}:${dateStr}`;
    ...
    return withKeyLock(lockKey, async () => {
        ...
        const nextSeq = records.length + 1;
        const deliveryId = `HYE-DL-${dateStr}-${pad(nextSeq, 2)}`;
        return createFn(deliveryId);
    });
}
