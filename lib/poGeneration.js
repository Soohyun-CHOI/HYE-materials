...
export async function generatePOForApprovedPR(pr) {
    ...
    try {
        po = await createPO({
            ...
            // THE ONLY VALUE ANY CODE PATH HAS EVER WRITTEN, AND NOTHING UPDATES
            // IT AFTERWARDS. This comment said the President could change it on
            // the signing screen before signing (issue #12); there is no such
            // control and no update path — `deliveryAddressUsed` is a parameter of
            // `createPO` and appears nowhere else that writes. Corrected per #181
            // by #384 rather than filed, and measured: 34 of 34 orders on this base
            // hold `Primary`.
            //
            // AND SINCE #384 THE OTHER OPTION NAMES NOTHING. `Jobs."Alternate
            // Delivery Address"` is gone, so a job has one default address and the
            // select has one value it can mean. The field and its 34 values stay
            // for #386, which converts it to a link to `Addresses` copied off the
            // request — no screen reads it in the meantime.
