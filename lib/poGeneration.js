 ...
 * Material Price, and each PO line's Material link), once the PO and its items
...
export async function generatePOForApprovedPR(pr) {
    ...
    // Delivery Items row moves onto this PO's own ordered item and the invoice line
    // that billed the excess splits onto it.
    //
    // AFTER the cache and outside the same rollback, deliberately twice over. After,
    // because it matches the overage row to a line of this PO on #18's `Material`
    // link, which the cache is what writes. Outside, for #165's reason — a derived
    // artifact must not undo the approval that produced it, and this one touches an
    // invoice that may already be paid. A failure therefore leaves the PO standing
    // and an asymmetry behind; lib/overagePR.js names the two places it shows,
    // since no email can be sent.
    try {
