...
import { orderDeliveryAddressId } from "./poDeliveryAddress";
import { applyOverageToPO } from "./overagePR";

/**
 * Creates the PO + PO Items snapshot for an Approved PR. Rolls back
 * everything it created (PO Items first, then the PO record itself) on any
 * failure, rather than trying to "resume" a partial attempt — the same
 * create-then-delete pattern as PR creation (issue #5). This is deliberate:
 * PO Item ID's sequence number is the highest one among the PO's existing
 * siblings plus one (see lib/ids.js:generateChildId), so a fresh retry after
 * a rollback starts a brand-new PO with an empty reverse-link array and seq
 * restarts at 1 with no risk of colliding with a half-finished attempt.
 *
 * IT DOES NOT PROMISE ZERO TRACE, AND THIS SENTENCE SAID IT DID. The rollback
 * discards its `Promise.allSettled` results and deletes the PO regardless, so
 * an item delete that FAILS leaves a `PO Items` row whose `PO` is gone —
 * `docs/notes/backlog.md` carries that as open work, with #206's fix to
 * `deleteDeliveryAsUser` as the model. What the rollback does promise is that
 * nothing it created is left REACHABLE from the request.
 *
...
export async function generatePOForApprovedPR(pr) {
    ...
    try {
        po = await createPO({
            ...
            // Issue #386 — where this order's material goes, frozen off the
            // request in the same create as everything else it snapshots. This
            // wrote the literal `"Primary"` into a select until then, the only
            // value any code path ever gave it.
            //
            // THE JOB IS IN HAND HERE AND IS DELIBERATELY NOT ASKED. `job` is
            // loaded two statements up for `Our PIC` and `Our Manager`, so
            // falling back to its default address would cost nothing and is
            // exactly what must not happen: the document would follow a job the
            // requester never named, and the order's whole purpose is to keep
            // what they did. `lib/poDeliveryAddress.js` owns that, which is why
            // the value is a call and not `pr.deliveryAddress?.[0]` written here.
            deliveryAddressId: orderDeliveryAddressId(pr),
        ...
        for (const item of items) {
            ...
            // #356 — the request item's category, carried in memory rather than
            // stored on `PO Items`. The cache keys the item axis on it, and this
            // loop is the one place the two rows are in hand together: the PO
            // item is a frozen copy of THIS request item, so pairing them here
            // needs no read and cannot pair the wrong two. `PO Items` gains no
            // field for it — see lib/materialsCache.js for the condition under
            // which it should.
            createdPOItems.push({ ...poItem, categoryRecordId: item.category?.[0] ?? null });
        }
    } catch (err) {
