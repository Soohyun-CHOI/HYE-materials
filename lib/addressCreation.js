 ...
 *
 * A SECOND CALLER ARRIVED IN #385 AND THE MOVE CONDITION IS WRITTEN HERE RATHER
 * THAN LEFT TO BE RE-DERIVED. `lib/addressChoice.js:addressOptions` groups the
 * request form's picker with this at the top of it, which is the same question
 * this answers and not a second one — so it CALLS this rather than copying it,
 * because "which addresses does this job use" is a union of two links and a
 * second implementation would go wrong silently. **What moves it is a third
 * caller, or a caller that wants this answer without the create screen's
 * framing**: then it belongs in a module named for the question. Two callers
 * under a name that still describes what the function computes is not drift, and
 * `assignedJobsFor` (#362 → #363) is the worked example of that condition firing
