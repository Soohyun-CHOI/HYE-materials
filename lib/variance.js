 ...
 * paragraph (#254): a charge's `Amount` is `{Qty} * {Unit Price}`, and it is a
 ...
 *
 * `charge` rather than `item`, per the vocabulary; the neighbouring refusals in
 * `createInvoiceAction` still say `item` and are not reworded here.
 */
export const CHARGE_PRECISION_COPY = {
    qty: "Every charge's quantity has to be a whole number.",
    unitPrice: "Every charge's unit price has to be a whole number of cents.",
};
