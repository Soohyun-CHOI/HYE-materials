 ...
 *
 * Returns `{ tokens, materials, truncated }`. An empty `tokens` means nothing was
 * typed — the caller shows a prompt rather than every material, since returning
 * the whole catalogue is not a search result.
 */
export async function searchMaterialPrices({ user, query, limit = 25 }) {
    const tokens = buildSearchTokens(query);
    if (tokens.length === 0) return { tokens, materials: [], truncated: false };
 ...
 * and "we ordered this and then cancelled" is information a buyer wants. Whether
 * it counts toward the order book is #18's `Committed Qty`, carried on the row
 * and read by lib/materialPriceView.js:countsAsOrdered — not re-derived here.
 *
