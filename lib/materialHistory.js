...
export async function searchMaterialPrices({ user, query, limit = 25 }) {
    ...
    // The source POs behind those prices, then exactly those POs' ordered items — not
    // every ordered item each material ever had.
    ...
    // (po, material) -> the ordered item that priced it. A PO can hold two
    // ordered items of one material (split quantities), and materialsCache
    // caches the LAST ordered item's price, so the last match is the one that
    // agrees with the cached figure.
    const orderedItemByPoAndMaterial = new Map();
    for (const item of poItems) {
        const key = `${item.po[0]}::${item.material[0]}`;
        orderedItemByPoAndMaterial.set(key, item);
    }
    ...
    const groups = materials.map((material) => {
        const rows = prices
            ...
            .map((price) => {
                ...
                const orderedItem = po ? orderedItemByPoAndMaterial.get(`${po.id}::${material.id}`) : null;
                ...
                return {
                    ...
                    // pricing ordered item cannot be located — see the note below.
                    qty: orderedItem?.qty,
                    ...
                    poStatus: orderedItem?.poStatus || po?.status || "",
                    identifiers: visible
                        ? {
                              poId: po?.poId ?? null,
...
/**
 * One material's full purchase history, newest first — every ordered item recorded
 * for it, including ordered items whose PO was withdrawn.
 *
 * Withdrawn ordered items are INCLUDED and marked rather than filtered out, for
 * the same reason the comparison rows show status: the ordered item is a real
 * thing that happened, and "we ordered this and then canceled" is information a
 * buyer wants. Whether it counts toward the order book is #18's `Committed Qty`,
 * carried on the row and read by lib/poItemQty.js:countsAsOrdered — not
 * re-derived here.
 *
