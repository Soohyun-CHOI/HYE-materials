// Whether an order is still unsigned: the signal that says so wherever one is offered
// for an invoice (#198), and since #292 the mail that asks the President to sign it.
//
// WHY THIS IS A SIGNAL AND NOT A FILTER, which is the whole shape of the issue. On the
// normal path an order is placed by sending the signed PO to the vendor, so a vendor
// invoice against a PO still at `Awaiting Signature` always means something happened
// outside that rule — the site ordered directly and the office is recording it after
// the fact, or an overage order (#167) exists precisely because material already
// delivered. **This read "Site can only order once the President has signed and the PO
// PDF has gone out", which is circular** (#281): sending the PO IS placing the order,
// so that sentence made the act its own precondition. The premise it was reaching for
// survives the correction — an unsigned order has not been placed through the app —
// and that is what makes an invoice against one worth a signal. #168 stopped excluding those POs from the invoice-side queries for
...
//
// PURE, so a client component can import it and so
// scripts/tests/offline/po-unsigned.mjs can pin it. It was import-free until #281,
// which added the signature axis's other half below and takes the one status name it
// needs from `lib/poSend.js` rather than spelling it twice. #292 adds `formatUSD` for
// the mail's total. Both of those are pure too, so every part of this file stays safe
// for a browser bundle and for the offline tier; the extensions are spelled out
// because that tier runs under plain `node` with no loader.
import { PO_SENT_STATUS } from "./poSend.js";
...
/**
 * The statuses that mean the President HAS signed, and #281 is the sweep the note
 * above was inviting — for the other axis of the same field.
 *
 * WHY THIS EXISTS AT ALL: `Sent to Vendor` (#281) is the second status past the
 * signature, so `status === "Signed"` stopped being the same question as "has this
 * been signed". Three places asked it that way and two of them were wrong the moment
 * the option came back — `app/materials/page.js`'s unsettled-price caveat called a
 * sent order unsettled, and `statusTag` below tagged it `PO: Sent to Vendor` where a
 * signed order gets no tag at all. Both read this now.
 *
 * AN ALLOWLIST, LIKE `PO_WITHDRAWABLE_STATUSES` AND FOR THE SAME REASON: a status
 * option added later must not silently count as signed. What that costs is a change
 * here when one is, which is the point rather than the price.
 *
 * NOT `presidentSigned`, WHICH WOULD BE THE BETTER TEST AND IS NOT AVAILABLE. Both
 * callers hold a `PO Status` lookup string off a material-price row and no checkbox,
 * which is also why `isPOUnsigned` above reads `status` rather than the flag.
 */
export const PO_SIGNED_STATUSES = ["Signed", PO_SENT_STATUS];

/** Has the President signed this order, judged from a `PO Status` string? */
export function isPOSigned(poStatus) {
    return PO_SIGNED_STATUSES.includes(poStatus);
}
...
/**
 * The mail that asks the President to sign a newly generated order (#101).
 *
 * MOVED OUT OF `lib/email.js` BY #292, WORD FOR WORD. Nothing it says changed: the
 * issue is what its one FIGURE looked like. `notifyPOAwaitingSignature` handed the
 * body `po.totalAmount` raw, so the President read `260` where every screen in the app
 * reads `$260.00`, and #233's rule that money reaches a reader through `formatUSD` had
 * simply never been applied to the mail path.
 *
 * IT IS HERE BECAUSE THE OFFLINE TIER CANNOT IMPORT `lib/email.js` — that module
 * throws at load without `RESEND_API_KEY`, so a body assembled inside it can be read
 * by a check and never CALLED. That was #290's reason for moving the mail before this
 * one, and it is the rule #292 generalized: a mail that carries a figure has to be a
 * pure builder, and a mail that carries none has nothing for a check to compute.
 *
 * AND IT IS IN THIS MODULE RATHER THAN BESIDE THE OTHER TWO. `lib/poSend.js` holds
 * them and was the obvious home, but its subject is sending the order to the vendor
 * and this mail is not about that act — which is #181's file-name test, does the name
 * make a claim the contents contradict. This module owns the signature axis: the
 * status a signature has not reached yet, the two it has, and now the message that
 * asks for it.
 *
 * THE FIGURE IS A NUMBER AND THIS BUILDER RENDERS IT, for the reason `SEND_COPY.mail`
 * now states: the step a caller can forget is the one that was forgotten here.
 */
export const AWAITING_SIGNATURE_COPY = {
    subject: ({ poId }) => `Action needed: PO ${poId} awaiting your signature`,
