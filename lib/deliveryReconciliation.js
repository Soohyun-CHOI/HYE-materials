 ...
 *     HYE-INV-260819-19  charges 13  ┘ both on HYE-PO-260819-22-001
 ...
 *
 * `orderedQtyByOrderedItem` IS A LEVEL THIS ALREADY READ AND THREW AWAY (#408), the
 * same shape `getInvoiceDeliveryStatus` hands back `orderedItemsByInvoice` in: the
 * `PO Items` rows are fetched on the line below to compare quantities, `Qty` is
 * already on them, and handing the map back costs nothing. Deriving it in the page
 * would be one more query per 50 ids for rows in memory.
 *
 * IT IS A MAP RATHER THAN A FIELD ON THE ROWS, AND THAT IS THE DECISION RATHER THAN
 * A SHAPE. #232 took `ordered` OFF the row deliberately — nothing on the delivery
 * side of this screen states what was ordered, and `/pos/[poId]` is where that is
 * answered. A row carrying it again would put the figure back in front of
 * `lib/invoiceDeliveryEntries.js`, which reads these rows and must not gain it. The
 * map goes to `chargesByOrder` and nowhere else.
 */
export async function getInvoiceReconciliation(invoiceItems, { linkedDeliveryRecordId } = {}) {
    ...
    return {
        ...
        delivery,
        // #408 — what each of those ordered items was ordered for, keyed the way the
        // charge rows name one: an ordered item's record id. Built from `poItems`
        // rather than from `rows`, because one ordered item can be charged by two
        // invoice items and the map is about the ordered item.
        orderedQtyByOrderedItem: new Map(poItems.map((p) => [p.id, p.qty])),
    };
}
