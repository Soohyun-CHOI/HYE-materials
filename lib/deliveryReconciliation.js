...
// reads every slice on those ordered items, because `arrivedBeyondOrder` stays an
 ...
 * were what the detail read to list every arrival that touched an ordered item;
...
function deliveredByPOItem(deliverySlices) {
    ...
    for (const slice of deliverySlices) {
        const lineId = slice.poItem?.[0];
        if (!lineId) continue;
        const acc = byLine.get(lineId) || { within: 0, beyond: 0 };
 ...
 * ARITHMETIC. Every figure a row carries is now this bill's: `billedOnThisInvoice`
 * is what THIS invoice charges, and `status.delivered` is what the delivery it
 * matches brought of that ordered item. The ordered item's own totals used to ride
 * along as `line` — `Billed` being the `Invoiced Qty` rollup across every invoice —
 * and that field is GONE from the row rather than left unread. Two scopes in one row
 * is what produced `HYE-INV-260804-04` reading `Billed 30` while billing 15.
 *
 * THREE FACTS FROM THE ORDER SURVIVE, AND THEY TRAVEL ON `status` WHERE THE COPY CAN
 * SEE THEM: `ordered`, `arrivedBeyondOrder`, `billedBeyondOrder`. They are the ORDER's
 * facts and are rendered as one line that says so by name. Narrowing them was
 * considered and refused in both directions — see STATUS_COPY.detail.againstOrder,
...
export async function getInvoiceReconciliation(invoiceLines, { linkedDeliveryRecordId } = {}) {
    ...
    // `fitRefusal` guarantees for such an invoice on the computed path — a bill
    ...
    // this level did not shrink. `arrivedBeyondOrder` stays an ORDER-scoped fact, and
    ...
    const rows = lines.map((l) => {
        ...
        return {
            ...
            status: {
                ...
                // THE ORDER'S THREE FACTS, CARRIED ON THE SHARE BECAUSE ONE LINE OF
                // COPY STATES THEM TOGETHER. They belong to the ORDER and not to one
                // bill, which is why `STATUS_COPY.detail.againstOrder` names that
                // frame out loud; `ordered` joined them in #232, when the figures
                // beside it became this invoice's own and it stopped having a
                // same-scope line to sit on. `ordered` is also the gate that decides
                // whether that line renders at all, so a bare share stays silent
                // about an order it holds no figure from.
                ordered: lineFacts.ordered,
