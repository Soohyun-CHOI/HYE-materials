 ...
 * An invoice with no lines at all gets a summary too, so a caller never has to
 * distinguish "no entry" from "nothing to compare"; and since the chip comes from
 * the link rather than from the lines, that summary is still an answer.
 */
export async function getInvoiceDeliveryStatus(invoices) {
    ...
    // Free-text lines name no ordered item, so they are dropped from the quantity
    // comparison here and counted per invoice — see countsTowardStatus for why
    // comparing them to nothing would make every invoice with such a line read as
    // short.
    ...
    const rows = lines.map((l) => {
        ...
        return {
            ...
            status: {
                ...
                // so they are carried from the line rather than from the share.
 ...
 * the ordered items to ask whether ANY invoice line hung off them — an existence
 ...
 *
 * Returns a Map of delivery record id -> summary.
 */
export async function getDeliveryInvoicing(deliveries) {
    ...
    // The bills naming these shipments, and their lines. Both levels are empty — and
    // therefore free — for a delivery nobody has billed yet, which is the state the
    // vendor-chasing worklist is made of.
    ...
    for (const slice of slices) {
        ...
        // into it: a delivery can hold two slices of one line — the within-order one
        // and the excess — and counting them as two would make "1 of 2 invoiced" out
        // of a single ordered item.
