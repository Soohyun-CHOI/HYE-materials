...
// caller here takes a privileged flag any more. The withholding was real — a page
// that fetched the invoice levels and then declined to render the column would
// have been hiding rather than withholding, so `getDeliveryInvoicing` was simply
// not called — but the line it drew does not survive the reason for it. The
// deliveries list is Job-scoped, and #211 admits an employee to the invoices of any
// job they are assigned to, so "has this been billed" is no longer a fact kept from
// them one screen away. Payment is what stays behind, and no function here reads it.
//
