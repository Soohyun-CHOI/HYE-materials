...
// delivery matched measures 1 and a delivery nobody has invoiced measures 1 too.
//
...
export async function getInvoiceReconciliation(invoiceItems, { linkedDeliveryRecordId } = {}) {
    ...
    // gave a charge with no ordered item a row of its own carrying `status: null`,
    // which the page rendered as `Not compared — no ordered item`; the early return
    // was the whole-invoice case, an invoice every one of whose charges was like
