 ...
 * a second time. The bills are fetched on the line below to compare quantities, they
 ...
 *
 * It maps to the ordered items a bill CHARGES, free-text invoice items dropped, so a
...
export async function getInvoiceDeliveryStatus(invoices) {
    ...
    // The ordered items each bill charges, from the rows already in hand. Built from
 ...
 * a reader wants answered — was this bill refused a pairing, or has nothing arrived
 ...
 * `getArrivalsForBill` per invoice, which is five reads each and the per-row shape
 ...
 * it should: whether what arrived COVERS what is billed is the matched delivery's
 * question, and this bill has no match, which is the state being reported.
 *
 ...
 * ARITHMETIC. Every figure a row carries is now this bill's: `status.invoiced` is
 ...
 * COPY CAN SEE THEM: `arrivedBeyondOrder` and `billedBeyondOrder`. They are the
    ...
    const rows = lines.map((l) => {
        ...
        return {
            ...
            status: {
                ...
                // ordered item and not to one bill, which is why
