...
//
// Pure apart from three other pure modules, so
// scripts/tests/offline/material-price-view.mjs pins all of it without
// credentials.
//
// NOTE THE EXPLICIT `.js` ON EVERY INTRA-LIB IMPORT BELOW, which is deliberate:
// the offline tier runs under plain `node` with no module loader, and node cannot
// resolve the extensionless intra-lib imports the rest of the app relies on Next
// to resolve. **This paragraph said it was "the only place in lib/ that does it"
// until #325, which was false of its own file — the two imports under it already
// did — and is false of about twenty files under lib/ (#181).** What is true is
// the rule: an offline-pinned module either imports nothing or spells the
// extension, and inlining a dependency to stay import-free is the worse trade —
// a second implementation of somebody else's rule.

// Issue #281 — the signature axis's allowlist, spelled once.
...
// Issue #325 — the token rule itself, which this module owned until the document
// lists needed the same one. See that module for the semantic; what stays here are
// the two arguments that are this screen's and not the rule's.
import { searchTokens } from "./searchTokens.js";
...
/**
 * This screen's two arguments to the app's one token rule (#325).
 *
 * THE RULE MOVED TO `lib/searchTokens.js` AND THIS KEPT ITS NAME. That module holds
 * the fold, the AND, the substring and the empty case — the whole semantic, shared
 * now with the four document lists' box. What is left here is the pair of steps that
 * belong to `Material Label` rather than to searching: the path separator a reader
 * pastes back, and the ceiling the Airtable formula imposes. Neither applies to a box
 * matching in the browser, which is why they are arguments rather than the rule.
 *
 * The normalization is still #18's (`textMatchKey`, one layer down): trim, collapse
 * internal whitespace, then fold case — case-folded at the LOOKUP rather than in
 * storage, because the stored string is printed on the vendor's PO and is the only
 * copy.
 *
...
export function buildSearchTokens(query) {
    return searchTokens(query, { drop: [PATH_SEPARATOR_TOKEN], max: MAX_SEARCH_TOKENS });
}
 ...
 * monotonic within a day by construction (HYE-PO-YYMMDD-##).
 */
...
export const MATERIAL_SEARCH_COPY = {
     ...
     * became a lookup of the category's composed path: nobody types the words this
     * box matched, so naming a name told a reader to search for one.
     *
     * **#416 GAVE THE WORD BACK BY REMOVING THE REASON IT WENT.** A category's item
     * name is a readable name again — `Tube, SUS 304, AP` — so a name is once more
     * a thing a person types, and the lookup behind the label is that field rather
     * than the path.
     *
     * AND `category` STAYS, WHICH IS THE WHOLE ANSWER TO A ROW WITH NO VISIBLE
     * MATCH. The box also searches the category's path, which no screen renders, so
     * a query for a branch returns rows carrying none of the typed words anywhere a
     * reader can see. This label is where that is stated — the reader is already
     * looking at it, and the alternative was putting the path on the row, which is
     * a decision the design pass has and this issue does not.
     */
    boxLabel: "Search by item name, category, size or unit",
