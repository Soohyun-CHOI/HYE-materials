...
//
//   - `Category` compares by its LEAF CODE, and it replaced `Item Name` in #355.
//     A name used to be typed, so two spellings of one thing had to be folded by
//     normalizing and lower-casing both sides; a category is picked, so identity
//     is exact and no normalization applies to it at all. The leaf code rather
//     than the record id because that is what the form carries and what the
//     action resolves — and either would do, since a leaf code names one row and
//     the 777 labels are distinct, so name-equality and category-equality are the
//     same question asked two ways.
//   - **A ROW WITHOUT ALL FOUR LEVELS SETTLED MERGES WITH NOTHING**, the same
//     treatment an empty row gets. It is not yet an item: two half-picked rows
//     agreeing on size and price may still become different items, and folding
//     them would silently take one of the requester's rows away. A Draft is saved
//     with no per-item validation, so this state is reachable and ordinary.
//   - `Size` compares through `normalizeItemText` AND lower case. The stored value
//     keeps its case (#18: the string is printed on the PO PDF and `SCH 40 PVC` is
//     correct as written), but `getMaterialByKey` looks a material up with
//     `LOWER(TRIM(...))` — so two spellings of one size are one material. Leaving
//     them as two rows would produce exactly the two-ordered-items-one-material
//     state this issue exists to remove, which is why the comparison follows the
//     lookup rather than the storage. **Size is where the remaining duplication
//     lives** now that the name axis is a picked category (#356 says so too).
...
// differing only in the case of its Size contributes its quantity and nothing else,
// so the spelling the requester typed first is what reaches the vendor. **The name
// is no longer part of that since #355** — it is composed from the category, so two
// merging rows cannot disagree about it in the first place; Size is the one free
// text left on the key.
//
// AN EMPTY ROW IS NOT MERGED INTO ANYTHING, and neither is one whose category is
// half-picked. `isEmptyItemRow` already drops the first at write time. What the two
// have in common is that there is no `Material` link on a `PR Item` to fall back on
// the way #241's fold does: a PR row is what the requester put in it, and until the
// fourth level is settled that is not yet an item to fold anything into. Both are
// passed through untouched and left for the write to skip or refuse.
//
...
export function isEmptyItemRow(item) {
    return !(
        // Any level picked is a touched row (#355), the same grade of fact as a
        // Unit being chosen below: the requester started on this item even
        // though no name was ever typed into it.
        (item?.categoryCodes && item.categoryCodes.some((c) => c)) ||
...
export function mergeKey(row) {
    ...
    // The leaf level, and nothing merges until it is settled — see the header.
    const leaf = row.categoryCodes?.[3] ?? "";
    if (!leaf) return null;
    ...
    return [leaf, size, unit, priceKey, remark, quotation].join("::");
}
 ...
 *
 * APPLIED IN `app/prs/new/actions.js:parseFormState`, which is the one place a
 * submission becomes items, so both write paths through it merge; the form calls
 * `describeMerge` below to preview the same rule before anybody submits.
 */
