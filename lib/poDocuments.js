...
// to the last ordered item the arrival filled, so the within piece and the excess
...
// lib/invoiceItemFold.js's key at this scope — two charges billed at different
// prices are two facts and stay two rows, so a folded charge's `@ price` is exact
...
/**
 * One charge's fold key: the ordered item it bills, plus the unit price.
 *
 * `lib/invoiceItemFold.js:foldKey` AT THIS SCOPE, with the ordered item where that
 * one has `Material` — a charge here is listed and sorted under an ordered item, and
...
/**
 * The invoices charging this order, one entry each, and one charge per ordered item
 * and price within an entry.
 *
 ...
 * folded charge is whether something is wrong with it, not which half.
 */
export function foldInvoicesOnOrder({ orderedItems, invoiceItems, invoices } = {}) {
    ...
    // ordered item are two charges and tie here; first appearance breaks the tie,
 ...
 * `overQty` sums only the FLAGGED members, because a folded line holds the within
