...
// is the page's, because it is a decision about who is looking.
...
//
// TWO VARIANCE WORDS, BECAUSE THE TWO FLAGS COMPARE DIFFERENT THINGS ON ONE
// SCREEN. `Invoices."Variance Flag"` is the invoice's stated total against its
// calculated one, so it reads `⚠ Total variance`; `Invoice Items."Variance Flag"`
// is this charge against what the order agreed, and it keeps the bare
// `⚠ Variance` the invoice items table already uses for that same field. The page
// this replaces said `Header` and `Line` — one a database word, the other the
// word this repository reserves for a Job's `Lines` row.
    ...
    invoices: {
        ...
        charge: (f) => ({
            ...
            text:
                ...
                `${typeof f?.unitPrice === "number" ? ` @ ${f.unitPrice}` : ""}`,
        }),
    },
    deliveries: {
        heading: "Deliveries",
