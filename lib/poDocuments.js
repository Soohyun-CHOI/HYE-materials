    ...
    for (const item of invoiceItems || []) {
        ...
        if (!entries.has(invoiceRecordId)) {
            entries.set(invoiceRecordId, {
                ...
                // `paidDate` STOOD HERE AND IS GONE (#309). Its only reader was the
                // paid badge's builder above, which no longer prints a date, so
                // carrying it would be an entry field with no consumer — the shape
                // #182 exists to remove. The Airtable mapper still reads
                // `Invoices."Paid Date"`, because the invoice detail states it and
                // the Admin form writes it; nothing on an ORDER's page needs it.
                //
                // **AND #318 READS THAT DATE HERE WITHOUT CARRYING IT.** The `Paid`
                // checkbox is gone from the base and a `Paid Date` is the whole of the
                // payment, so the entry's own field is derived from the date's presence
                // rather than copied from a flag. The paragraph above still holds:
                // nothing on an order's page prints the date, so nothing on this entry
                // stores it — what changed is which field the boolean is read from.
                // #281's shape, one axis over: the mapper carries the date and each
                // site tests whether it is there.
                paid: Boolean(invoice.paidDate),
                charges: new Map(),
            });
        }
...
// stopped coining a word for it — the page had said `⚠ Line Variance`, putting a
// barred word into new screen text where #227's sweep cannot read it. At the time
// the word was barred because `Lines` was a table; #280 renamed that table and the
// bar survived on #303's rule instead, so the sentence holds and its reason moved.
//
...
export const PO_DOCUMENTS_COPY = {
     ...
     */
    badge: {
        // A PLAIN STRING SINCE #309, NOT A BUILDER. It read `✓ Paid {paidDate}` when
        // the invoice carried a date, so one badge said two different things and the
        // shorter one looked like missing data. A badge marks that the vendor was
        // paid; WHEN is stated rather than marked, by `Paid on {date}` on the
        // invoice's own page — the one screen whose subject is the payment itself.
        // Its argument went with it, and so did the `paidDate` the fold copied in for
        // no other reader.
        paid: "✓ Paid",
        notPaid: "Not paid",
