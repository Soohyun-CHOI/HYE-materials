...
//
// A SECOND DATA POINT, AN ORDER OF MAGNITUDE WORSE, measured while verifying #217.
// Six navigations to `/prs` produced NINETY-ONE renders of it: 31 at the page's
// pre-change cost of 8 operations, 60 at its post-change 17, one at 5. That is
// roughly 1,270 operations for six page views a person actually asked for — about
// 2% of the 60,000 this base spent in six days, from one afternoon on one screen.
// The mechanism is the same stash-and-unstash the #176 note describes, so the two
// measurements are the same phenomenon at two sizes; what the second adds is that
// the multiplier grows with how much a change is measured, which is exactly the
// activity these labels exist to encourage.
//
// ─────────────────────────────────────────────────────────────────────────────
 ...
 * Server Action, the filename for a script, and for a Route Handler the HTTP
 * METHOD IN CAPITALS before the route template (`POST /api/invoices/upload`).
 *
 * THE METHOD IS PART OF A HANDLER'S LABEL BECAUSE A route.js FILE IS NOT ONE
 * ENTRY POINT. It may export GET and DELETE together, and those two have nothing
 * in common: one lists and one deletes, and folding them into a row named for the
 * path would average a read against a write and hide whichever is the expensive
 * one. Every route.js here exports a single method today, so the rule looks like
 * ceremony — write it anyway, because the day one file exports two is the day the
 * figure silently stops meaning anything, and nothing about the label would
 * change to say so. None of the four conventions is enforced by this function:
 * offline/airtable-ops.mjs derives the expected string from the file's path and
 * the export's name and fails on a mismatch (#224), which is what makes a typo a
 * build failure rather than a new bucket nobody notices.
 */
