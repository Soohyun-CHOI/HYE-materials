...
export function planPairings({ arrival, bills, agreedPrices } = {}) {
    ...
    // ONE PASS, NO RUNNING STATE, AND THAT IS A PROPERTY RATHER THAN A SHORTCUT.
    // An earlier version folded each decision back into the pool as an attachment
    // and deducted its room, because a transcribed invoice number could be attached
    // first and then compete with a computed bill for the same ordered item. With
    // the entry form's invoice control gone there is no such first attachment, and
    // the fold became provably inert: a bill only attaches when NO other unplaced
    // bill charges any ordered item it charges, so any two that attach are
    // disjoint — they are not each other's rival and they draw on different room.
    // Folding disjoint decisions in changes no later answer, which is why this is a
    // filter now. `offline/delivery-invoice-match.mjs` asserts the disjointness the
    // simplification rests on, so if an override ever returns the fold has to
    // return with it and that check is where it will be noticed.
    for (const bill of bills || []) {
        if (!bill?.invoiceRecordId) continue;
        const refusal = pairingRefusal({ bill, arrival: asArrival, bills, agreedPrices });
        if (refusal) {
