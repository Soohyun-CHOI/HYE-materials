 ...
 *
 * Only `sharedOrder` is ever spoken. The other three are the ordinary shape of a
 * bill that simply is not this shipment's, and saying so for every invoice on the
 * vendor would bury the one answer that matters.
 */
export const PAIRING_REFUSED = {
    ...
    /** Another bill on this arrival charges an ordered item this one also charges. */
...
export function orderedItemsDelivered(arrival) {
    return [...new Set((arrival?.orderedItemRecordIds || []).filter(Boolean))];
}
 ...
 * rival is any other bill charging the same ordered item that nothing has placed
 * elsewhere, which needs no price at all and refuses rather than guesses. Measured
 * on this base 2026-08-13, both rules attach the same 6 pairings and block the same
 * `166-DEMO Coupling` pair, so the width costs nothing that can be observed here.
 *
 * A RECORDED PAIRING BEATS A COMPUTED ONE, which is #219's tier rule one level up:
 * a rival naming this arrival counts as a fact rather than a candidacy, and one
 * naming a DIFFERENT shipment is not a rival at all — it has been placed, so it is
 * competing for nothing. That is the whole of `placedElsewhere` below.
 *
 * `arrival.deliveryRecordId` IS NULL ON THE ENTRY PATH, where the delivery does not
 * exist yet — #210's reading of the same null. It needs no clause of its own: no
 * bill can name a record that does not exist, so `placedElsewhere` is simply true
 * of every bill some shipment holds and false of every bill none does. An earlier
 * version carried a `held &&` guard against a `null === null` comparison, which was
 * left over from a narrower rival rule and could not change an answer under this
 * one — removed rather than kept with a comment, the call this repo makes on
 * `arrived-more` and `resolveDeliveryFilters`.
 */
export function pairingRefusal({ bill, arrival, bills, agreedPrices } = {}) {
    ...
    const rival = (bills || []).some((other) => {
        if (!other || other.invoiceRecordId === bill.invoiceRecordId) return false;
        if (!chargesSameOrderedItem(bill, other)) return false;
        const placedElsewhere =
            other.pairedDeliveryRecordId && other.pairedDeliveryRecordId !== held;
        return !placedElsewhere;
    });
...
export const PAIRING_COPY = {
    preview: {
        ...
        /**
         * One bill fits, and another already on this shipment charges the same
         * ordered item. NAMES BOTH, because a rival is always visible to whoever is
         * reading this — see the module header for why that holds by construction.
         */
        [PAIRING.sharedOrder]: (f) => ({
            key: PAIRING.sharedOrder,
            text:
                `${f?.invoiceId ?? "An invoice"} bills an ordered item another invoice on this ` +
                "delivery also bills, so nothing records which of them this delivery answers. " +
                "Pick the number on the packing list.",
        }),
    },
    ...
    banner: {
        ...
        [PAIRING.sharedOrder]: () => ({
            key: PAIRING.sharedOrder,
            text:
                "A delivery brought everything this invoice bills, but another invoice on it " +
                "charges the same ordered item, so none was attached. Attach the right one from " +
