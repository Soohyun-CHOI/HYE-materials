...
// either invoice the same #210 mismatch marker, so no figure this app computes comes
...
// is stable. Deliberately NOT #166's oldest-invoice-first, which asserts that the
// earlier invoice has the better title to the delivery — the assertion this case
// exists to say cannot be made.
//
// A DIFFERENT QUANTITY IS OBSERVED, SO IT STILL REFUSES. Invoices of 10 and 5 against
...
// invoice axis still works. A vendor invoicing 13 and shipping 10 is exactly the
...
// written: `HYE-INV-260804-07` invoiced 13 against a delivery of 10 and paired.
//
...
// invoiced against 0 delivered) carried #210's mismatch marker, which fires on the same
...
export const PAIRING_REFUSED = {
    ...
    /** It charges an ordered item this delivery did not bring. */
    notContained: "not-contained",
    /** Some delivery already holds it. The single side is taken. */
    alreadyPaired: "already-paired",
    /** A invoiced unit price was compared with the order's and is not it. */
    ...
    /**
     * Invoices already on this delivery have claimed everything it brought of an
 ...
 * of its two callers. Both directions build their invoices through here, which is also
...
export function orderedItemsInvoiced(invoice) {
 ...
 */
function qtyOnOrderedItem(document, poItemRecordId) {
 ...
 * delivery brought, and that would drop the very invoices #210's mismatch marker
 * exists for — a vendor invoicing 13 against a delivery of 10. This asks whether the
 * delivery's capacity for that ordered item has been spoken for by SOMEBODY ELSE.
 * So the comparison is `> 0`, never `>= invoiced`: 13 against 10 with nothing else
...
export function chargesSameOrderedItem(a, b) {
    const other = new Set(orderedItemsInvoiced(b));
    return orderedItemsInvoiced(a).some((p) => other.has(p));
}
...
export function chargeSignature(invoice) {
    return orderedItemsInvoiced(invoice)
        .sort()
        .map((poItemRecordId) => {
 ...
 * that reads this for the tie-break message does not go through `fitRefusal`. Both
 * stay after #278 for the reason that refusal gives: they assert the property
 * instead of resting on the state being unwritable.
 */
 ...
 * vacuously contained in every delivery, so containment alone would attach it to
 * whichever delivery was recorded next. It named the two hand-entered free-text
 * invoices this base carried; #278 removed the state and both rows, and the refusal
 * STAYS AND IS NOT COPY — `PAIRING_REFUSED` values are never spoken, so keeping it
 * names nothing on a screen. What it buys is that the vacuous-subset property is
 * asserted here rather than resting on a hand-emptied link being impossible.
 * Nothing downstream may rely on a later clause happening to catch it.
 *
...
export function fitRefusal(invoice, delivery, agreedPrices) {
    const invoiced = orderedItemsInvoiced(invoice);
    if (invoiced.length === 0) return PAIRING_REFUSED.noOrderedItem;
    ...
    if (!invoiced.every((p) => brought.has(p))) return PAIRING_REFUSED.notContained;

    for (const charged of invoice.orderedItems || []) {
        // A crash guard on a hand-emptied link since #278: a null key would look up
        // no agreed price and fail closed on a row this rule has no business
        // judging. It was the mixed-invoice case #96's hidden option made reachable.
 ...
 *
 * `invoices` is the whole vendor-narrowed pool, not the candidates — an invoice that
...
export function pairingRefusal({ invoice, delivery, invoices, agreedPrices } = {}) {
    ...
    const noRoom = orderedItemsInvoiced(invoice).some(
        (poItemRecordId) =>
            roomOnOrderedItem({
                delivery,
                poItemRecordId,
 ...
 *
 * THE POOL'S ORDER IS STILL IRRELEVANT, BUT NO LONGER FOR FREE. Among untied invoices
...
export function planPairings({ delivery, invoices, agreedPrices } = {}) {
    ...
    // invoice re-enters it holding THIS delivery, so it stops being an unplaced rival and
 ...
 * ordered items each invoiced twice at matching figures; the sentence's work is to
 ...
 *
 * `deliveries` are the ones that brought any of the ordered items this invoice
 * charges for; anything else cannot contain it. `invoices` is the same vendor-wide
...
export const PAIRING_COPY = {
    preview: {
        ...
        [PAIRING.matched]: (f) => ({
            key: PAIRING.matched,
            text:
                `${f?.invoiceIds?.[0] ?? "One invoice"} charges ordered items this delivery brought ` +
                "and no other invoice charges them, so it is attached. If the packing list names a " +
        ...
        [PAIRING.severalAttached]: (f) => ({
            key: PAIRING.severalAttached,
            text:
                `${(f?.invoiceIds || []).join(" and ") || "Several invoices"} each charge ordered ` +
                "items this delivery brought that no other invoice charges, so all of them are " +
        ...
        [PAIRING.sharedOrder]: (f) => ({
            ...
            text:
                ...
                "both charge an ordered item this delivery brought, so nothing records which of " +
    ...
    banner: {
        ...
        [PAIRING.sharedOrder]: () => ({
            key: PAIRING.sharedOrder,
            text:
                "A delivery brought everything this invoice charges, but another invoice nobody " +
    ...
    tieBreak: {
        preview: (f) => ({
            ...
            text:
                ...
                `${(f?.tiedWith || []).join(" and ") || "another invoice"} charge the same ordered ` +
