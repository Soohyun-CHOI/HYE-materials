...
export const ALLOCATION_COPY = {
    ...
    banner: {
        ...
        overAttached: (qty, unit, poId, label = null) => ({
            key: "over-attached",
            text:
                `Over-delivery — ${qty}${unit ? " " + unit : ""}${label ? " of " + label : ""} ` +
                `arrived beyond what ${poId} ordered.`,
        }),
        /**
        ...
        overUnattached: (qty, unit, label = null) => ({
            key: "over-unattached",
            text:
                `Over-delivery — ${qty}${unit ? " " + unit : ""}${label ? " of " + label : ""} arrived ` +
...
export function describeDelivery(rows) {
    ...
    // "3 EA arrived beyond what was ordered" does not say beyond what.
