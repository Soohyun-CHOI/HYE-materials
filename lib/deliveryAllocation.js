...
/**
 * How much of an ordered item has not arrived yet.
 *
...
/** Whether this ordered item can still absorb an arrival. */
export function hasUndeliveredQty(line) {
    return undeliveredQty(line) > 0;
}

/**
 * The ordered items this arrival could possibly belong to — the NARROWED
 * ORDERED ITEM SET.
 *
 ...
 *                   { line, qty, over }. EVERY row names an ordered item; see the
 *                   invariant below.
 ...
 * FILL ORDER: the last ordered item this arrival actually filled, or — when it filled
 ...
 * inside ONE ordered item and never revisits which ordered item an arrival
...
export function planDelivery({ lines, vendorRecordId, materialRecordId, poRecordId = null, qty }) {
    ...
    if (left > 0) {
        // The end of the fill order: the last ordered item this arrival filled,
 ...
 * That function answers "what arrived", for a headline and a list row that name no
 ...
 * state what arrived against that order AND how much of it fell beyond, which is
